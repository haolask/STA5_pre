...
#define PTR_JPGC   pJpegDecCont
       ...
       (PTR_JPGC->ppInstance != NULL && !PTR_JPGC->ppControl.usePipeline))
    {
		...
		TRACE_ERR("allocate luminance output : %d bytes / 0x%x",PTR_JPGC->image.sizeLuma, PTR_JPGC->asicBuff.outLumaBuffer.virtualAddress);
                if(tmp == -1)
                {
                    return (JPEGDEC_MEMFAIL);
                }
            ...
            if(PTR_JPGC->info.givenOutChroma.virtualAddress == NULL)
            {
                if(PTR_JPGC->info.operationType != JPEGDEC_BASELINE)
                {
                    if(PTR_JPGC->asicBuff.outChromaBuffer.virtualAddress == NULL)
                    {
                        ...
                        if(tmp == -1)
                            return (JPEGDEC_MEMFAIL);
		        TRACE_ERR("allocate chrominance output : %d bytes / 0x%x",(PTR_JPGC->image.sizeChroma / 2), PTR_JPGC->asicBuff.outChromaBuffer.virtualAddress);
                    }
                    ...
                    if(PTR_JPGC->asicBuff.outChromaBuffer2.virtualAddress == NULL)
                    {
                        ...
                        if(tmp == -1)
                            return (JPEGDEC_MEMFAIL);
		        TRACE_ERR("allocate chrominance output : %d bytes / 0x%x",(PTR_JPGC->image.sizeChroma / 2), PTR_JPGC->asicBuff.outChromaBuffer2.virtualAddress);
                    }
                }
                else
                {
...
#define JPG_FRM     pJpegDecCont->frame
    ...
    VlcTable *pTable1 = NULL;
    __attribute__((unused)) VlcTable *pTable2 = NULL;

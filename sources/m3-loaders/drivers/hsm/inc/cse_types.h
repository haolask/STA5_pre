...
/**
 * @brief Type definition for a pre-allocated memory buffer that is required
 * by some functions
 */
struct membuf_stt {
	/*
	...
	uint16_t  mSize;
	/*
	 * !< Currently used portion of the buffer, should be initialized
	 * by user to zero
	 */
	uint16_t  mUsed;
};
...
/**
 * @brief  Structure type describing a byte array
 */
struct ByteArrayDescriptor_stt {
	const uint8_t  *address;    /*!< pointer to byte buffer */

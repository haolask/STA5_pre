...
//	uart_send_data("+chk_mounted\n");
	...
	{
//		uart_send_data("chk_mounted: Invalid drive\n");
	...
	*rfs = fs = FatFs[vol];			/* Return pointer to the corresponding file system object */
	if (!fs)
	{
//		uart_send_data("chk_mounted: FS not enabled\n");
	...
	if (fs->fs_type) {				/* If the logical drive has been mounted */
		...
		if (!(stat & STA_NOINIT)) {	/* and the physical drive is kept initialized (has not been changed), */
			...
			{
//				uart_send_data("chk_mounted: FDISK is write protected\n");
				return FR_WRITE_PROTECTED;
			}
#endif
//			uart_send_data("chk_mounted: File system object is valid\n");
			return FR_OK;			/* The file system object is valid */
		}
	}
	/* The logical drive must be mounted. Following code attempts to mount the volume (initialize the file system object) */

//	uart_send_data("chk_mounted: step 2\n");
...
//	uart_send_data("chk_mounted: step 3\n");
#endif
#if !_FS_READONLY
...
//	uart_send_data("chk_mounted: step 4\n");
...
//			uart_send_data ("CH_STEP7B\n");
			fmt = check_fs(fs, bsect);					/* Check the partition */
		}
	}
//	uart_send_data("chk_mounted: step 4\n");
	if (fmt == 3) return FR_DISK_ERR;
//	uart_send_data("chk_mounted: step 5\n");
...
//	uart_send_data("chk_mounted: step 13\n");
...
//	uart_send_data("f_open: step 1\n");
	...
	if (res == FR_OK) {
		...
		{
//			uart_send_data("f_open: invalid name\n");
			res = FR_INVALID_NAME;
		}
#if _FS_SHARE
		else
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (res != FR_OK) {				/* No file, create new */
			...
			if (mode & FA_CREATE_NEW) {			/* Cannot create new */
				res = FR_EXIST;
//				uart_send_data("f_open: file exists\n");
			} else {

...
void mmap_add_range(const mmap_region_t *mm_start, const mmap_region_t *mm_end)
{
	const mmap_region_t *mm, *mm_prev;
	bool duplicate;

	for (mm = mm_start; mm < mm_end; mm++) {
		duplicate = false;
		mm_prev = mm_start;
		while (!duplicate && mm_prev < mm) {
			duplicate = !memcmp(mm, mm_prev, sizeof(mm));
			mm_prev++;
		}
		if (!duplicate)
			mmap_add_region(mm->base_pa, mm->base_va, mm->size,
					mm->attr);
	}
}

...
static int mmc_enumerate(unsigned int clk, unsigned int bus_width)
{
	...
	if (get_boot_dev_base() == 0) {
		...
		/* Get saved context */
		memcpy(&mmc_csd, get_mmc_csd(), sizeof(mmc_csd));
		mmc_ocr_value = get_mmc_ocr();
		rca = get_mmc_rca();
		/* FIXME: Perhaps partially call mmc_fill_device_info() */
	}
...
size_t mmc_read_blocks(int lba, uintptr_t buf, size_t size)
{
	...
	while (!hsem_trylock(HSEM_MMC_ID, 0))
		;
...
{
	/* FIXME: boot partition config is re-initialized */
	mmc_set_ext_csd(CMD_EXTCSD_PARTITION_CONFIG,
			PART_CFG_PART_RPMB_ACCESS);
}

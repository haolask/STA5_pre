...
static tS32 read_xrun_recovery(snd_pcm_t *handle, tS32 err)
{
	...
	else if (err == -ESTRPIPE)
	{
		while ((err = snd_pcm_resume(handle)) == -EAGAIN)
		{
			...
			OSALUTIL_s32TracePrintf(0, TR_LEVEL_ERRORS, TR_CLASS_OSALIO, "audioin: read_xrun_recovery EAGAIN");
		}
		if (err < 0)
		{
			err = snd_pcm_prepare(handle);
...
static tS32 read_loop(snd_pcm_t *handle, tS16 *samples, tU32 size_in_bytes)
{
	...
	while (err == -EAGAIN)//(cptr > 0) do not wait alsa buffer filling
	{
		...
		if (err == -EAGAIN)
		{
		 	// -EAGAIN when non-blocked mode is used and the ring buffer is full (playback) or empty (capture)
		 	// we are here in capture, so buffer is empty
		 	// just return and skip the loop
			OSALUTIL_s32TracePrintf(0, TR_LEVEL_ERRORS, TR_CLASS_OSALIO, "audioin: read_loop EAGAIN, i.e. buffer empty");
			// set the err to 0 for an empty buffer
			err = 0;
		}
		if (err < 0)
		{
...
tS32 AudioIOIN_s32IOControl(OSAL_tIODescriptor fd, tS32 s32fun, tS32 s32arg)
{
	...
	switch (s32fun)
	{
		...
		case OSAL_C_S32_IOCTRL_AUDIO_IO_ENQUEUE_BUFFER:
		{
			...
			{
#if defined(CONFIG_ENABLE_CLASS_OSALIO) && (CONFIG_ENABLE_CLASS_OSALIO >= TR_LEVEL_ERRORS)
				OSALUTIL_s32TracePrintf(0, TR_LEVEL_ERRORS, TR_CLASS_OSALIO, "AudioIOIN_s32IOControl snd_pcm_avail handle = %p, =(0x%x, %s)", handle, err, snd_strerror(err));
#endif
				...
				if (err == 0)
				{
					...
					if (err < 0)
					{
						SamplesSize = 0;
#if defined(CONFIG_ENABLE_CLASS_OSALIO) && (CONFIG_ENABLE_CLASS_OSALIO >= TR_LEVEL_ERRORS)
						OSALUTIL_s32TracePrintf(0, TR_LEVEL_ERRORS, TR_CLASS_OSALIO, "AudioIOIN_s32IOControl retry after recovery snd_pcm_avail handle = %p, =(0x%x, %s)", handle, err, snd_strerror(err));
#endif
					}
				}
			}

...
#define	ETAL_STECI_EVENT_DATA_TO_TX				0

// Data Rx event bit 0
#define ETAL_STECI_EVENT_DATA_RX				1


// FLAGS
#define	ETAL_STECI_EVENT_DATA_TO_TX_FLAG		((tU32)0x01 << ETAL_STECI_EVENT_DATA_TO_TX)	

#define ETAL_STECI_EVENT_DATA_RX_FLAG			((tU32)0x01 << ETAL_STECI_EVENT_DATA_RX)


// WAKEUP FULL FLAGS & MASK
#define ETAL_STECI_EVENT_FLAGS					(ETAL_STECI_EVENT_DATA_TO_TX_FLAG | ETAL_STECI_EVENT_DATA_RX_FLAG)

#define ETAL_STECI_EVENT_WAIT_MASK				(ETAL_STECI_EVENT_FLAGS)

#define ETAL_STECI_EVENT_WAIT_ALL				(0xFFFFFFFF)

#define ETAL_STECI_EVENT_WAIT_TIME_MS			OSAL_C_TIMEOUT_FOREVER
//#define ETAL_STECI_EVENT_WAIT_TIME_MS 			ETAL_STECI_THREAD_SCHEDULING

#define ETAL_STECI_NO_EVENT_FLAG				0x00

OSAL_tEventHandle ETAL_Steci_TaskEventHandler;


//static tVoid ETAL_STECI_TaskClearEvent (tU32 event);
static tVoid ETAL_STECI_TaskClearEventFlag(tU32 eventFlag);
static tVoid ETAL_STECI_TaskWakeUpOnEvent (tU32 event);
#endif
...
#ifndef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
...
#if 0
//!
//! \brief      <i><b> tVoid ETAL_STECI_TaskClearEvent (tU32 event) </b></i>
...
static tVoid ETAL_STECI_TaskClearEvent (tU32 event)
{
	 ...
	 ETAL_TaskClearEvent (ETAL_Steci_TaskEventHandler, event);
}
#endif
//!
//! \brief      <i><b> tVoid ETAL_STECI_TaskClearEventFlag (tU32 event) </b></i>
//! \details    Functions to clear possible pending event in the middlewar task on event.
//!           
//! \param[in]  tU32            Event 
//! \return     tVoid           None
...
static tVoid ETAL_STECI_TaskClearEventFlag(tU32 eventFlag)
{
		...
		// Clear old event if any (this can happen after a stop)
	OSAL_s32EventPost (ETAL_Steci_TaskEventHandler, 
						  ~eventFlag, OSAL_EN_EVENTMASK_AND);
}

//!
//! \brief      <i><b> ETAL_STECI_TaskWakeUpOnEvent </b></i>
...
static tVoid ETAL_STECI_TaskWakeUpOnEvent (tU32 event)
{
	...
	if ((ETAL_STECI_EVENT_DATA_TO_TX == event)
		|| (ETAL_STECI_EVENT_DATA_RX == event)
		)
	{
		OSAL_s32EventPost (ETAL_Steci_TaskEventHandler, 
...
{
	STECI_tracePrintSystem(TR_CLASS_STECI, "ETAL_STECI_DataToTx");

	ETAL_STECI_TaskWakeUpOnEvent(ETAL_STECI_EVENT_DATA_TO_TX);
}

tVoid ETAL_STECI_DataRx(tVoid)
{
	STECI_tracePrintSystem(TR_CLASS_STECI, "ETAL_STECI_DataRx");

	ETAL_STECI_TaskWakeUpOnEvent(ETAL_STECI_EVENT_DATA_RX);
}

#endif // CPU_IMPROVEMENT
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
	tSInt vl_res;
	OSAL_tEventMask vl_resEvent = ETAL_STECI_NO_EVENT_FLAG;
    ...
    while (TRUE) 
    {
		...
		switch (steciStatus.status)
        {
            ...
            case STECI_WAIT_REQ_PIN_TOGGLE:		
				...
				STECI_tracePrintSystem(TR_CLASS_APP_ETAL_COMM, "STECI_ProtocolHandle: wait event handler %d", ETAL_Steci_TaskEventHandler);

				// if we are here it means a transaction may just have ended..
				// put a wait in order to be sure not to restart a new one too early so that DCOP detects the request
				// 
				OSAL_s32ThreadWait(ETAL_STECI_THREAD_TIME_BETWEEN_REQUESTS);
				
		        vl_res = OSAL_s32EventWait (ETAL_Steci_TaskEventHandler,
			                                 ETAL_STECI_EVENT_WAIT_ALL, 
			                                 OSAL_EN_EVENTMASK_OR, 
			                                 ETAL_STECI_EVENT_WAIT_TIME_MS,
			                                 &vl_resEvent);


				STECI_tracePrintSystem(TR_CLASS_APP_ETAL_COMM, "STECI_ProtocolHandle: scheduling event vl_res = %d, vl_resEvent = 0x%x",
						vl_res, vl_resEvent);

				if (OSAL_ERROR == vl_res)
				{
					// Event wait failure ==> break;			
					STECI_tracePrintError(TR_CLASS_APP_ETAL_COMM, "STECI_ProtocolHandle: wait error");
					break;
				}
				else if ((vl_resEvent == ETAL_STECI_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
				{
					// this is a timeout : no even received
				}
				else if (OSAL_OK == vl_res)
				{
					// This is a ETAL_STECI_EVENT_DATA_TO_TX_FLAG call the Tx event handler
					if (ETAL_STECI_EVENT_DATA_TO_TX_FLAG == (ETAL_STECI_EVENT_DATA_TO_TX_FLAG & vl_resEvent))
					{	
						// clear event now that it will be processed
						ETAL_STECI_TaskClearEventFlag(ETAL_STECI_EVENT_DATA_TO_TX_FLAG);
						// process		
					}
					
					// This is a ETAL_STECI_EVENT_DATA_RX_FLAG call the Rx event handler
					if (ETAL_STECI_EVENT_DATA_RX_FLAG == (ETAL_STECI_EVENT_DATA_RX_FLAG & vl_resEvent))
					{
						// clear event now that it will be processed
						ETAL_STECI_TaskClearEventFlag(ETAL_STECI_EVENT_DATA_RX_FLAG);
						// process	
						// Manage data reception
					}
					
					// unexepected event case
					if (!(ETAL_STECI_EVENT_WAIT_MASK & vl_resEvent))
					{
						// just clear the event
						ETAL_STECI_TaskClearEventFlag(vl_resEvent);
					}
				}
				else
				{
				...
				// FOR NOW : keep normal processing whatever the wake-up...
				//
#endif // CONFIG_ETAL_CPU_IMPROVEMENT_STECI
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
				else if (ETAL_STECI_EVENT_DATA_RX_FLAG == (ETAL_STECI_EVENT_DATA_RX_FLAG & vl_resEvent))
				{
//				else if (true == STECI_CheckDeviceRequest (&deviceInfo, &steciStatus))
					// specific check for the collision case
					// if both Rx and Tx are to be done simultaneously, the Rx is hanlded and Tx processed later
					// as a consequence, in CPU improvement, Tx request need to be reposted
					// in theory, in FULL DUPLEX, we could avoid that by processing the Tx...
					// which will result in Rx data reception together with Tx sent.
					//
					if (ETAL_STECI_EVENT_DATA_TO_TX_FLAG == (ETAL_STECI_EVENT_DATA_TO_TX_FLAG & vl_resEvent))
					{
						ETAL_STECI_DataToTx();
...
//  				OSAL_pvMemorySet ((tVoid *)(paramsPtr->inDataBuffer+startLocation), 0x00, (STECI_MAX_PACKET_LENGTH- startLocation));
            ...
            case STECI_TX_FRAME:
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
					// we need to repost a data to be sent event, else the STECI will remained waiting without knowing that there is a data to be sent
					//
					ETAL_STECI_DataToTx();
#endif
				...
				else if (STECI_STATUS_DEV_NOT_READY == steciStatus.error)
				{
					...
					// Warning message
					STECI_tracePrintError(TR_CLASS_STECI, "WARNING: STECI_STATUS_DEV_NOT_READY during STECI_TX_FRAME");
            ...
            case STECI_ERROR:
				...
				if (true == txRxInfoPtr->dataToBeTx)
				{
					if (txRxInfoPtr->numberOfRetry < STECI_DATA_SEND_RETRY_MAX_NUM)
					{
	                         ...
	                         tU8 specific0, tU8 specific1, tU8 specific2, tU8 specific3, tU16 BytesNumber)
{
	...
	OSAL_tMSecond vl_timeout = STECI_MAX_TIME_TO_WAIT_FOR_MSG_SENDING;
#if 0
	tU32 vl_index;
#endif
    ...
    else 
    {
		...
		STECI_tracePrintComponent(TR_CLASS_STECI, "STECI_SendMessageStart Tx:req");
		...
		else if (STECI_FLASH_CMD == specific0)
		{
			...
			// for flash operation : the timeout should be set to infinitie...
			// because the command is lasting the flash duration
			// a 50 s is set later..
			// so here no timeout
			vl_timeout = OSAL_C_TIMEOUT_FOREVER;
		}
		else
		{
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
		// indicate a message is to be sent
		ETAL_STECI_DataToTx();
#endif

		// wait the semaphore to gurantee message is sent
    	if (OSAL_s32SemaphoreWait(STECI_ProtocolHandle_MsgSentSem, vl_timeout) != OSAL_OK)
		{
			// errorr in semaphore...
			// should not happen
			STECI_tracePrintError(TR_CLASS_STECI, "STECI_SendMessageStart: error in Tx no post of STECI_ProtocolHandle_MsgSentSem");
	    }
		...
		STECI_tracePrintComponent(TR_CLASS_STECI, "STECI_SendMessageStart Tx done");
...
//	tBool currentLevel;
	...
	if (levelToWaitFor == currentLevel)
	{
		ret = TRUE;
	}
	else
#endif
	{
		...
		STECI_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "STECI_ProtocolHandle: STECI_EventWaitReqReady ");
		
		vl_res = OSAL_s32EventWait (ETAL_Steci_TaskEventHandler,
			                            	ETAL_STECI_EVENT_DATA_RX_FLAG, 
			                                OSAL_EN_EVENTMASK_OR, 
			                                WAIT_REQ_TIMEOUT,
			                                &vl_resEvent);

		STECI_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "STECI_EventWaitReqReady: scheduling event vl_res = %d, vl_resEvent = 0x%x", vl_res, vl_resEvent);

		if ((vl_resEvent == ETAL_STECI_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
		{
			// this is a timeout : no even received
			ret = FALSE;	
		}
		else
		{
			...
			// cross check if correct even but there is no reason it is not :)
			if (levelToWaitFor == currentLevel)
			{
				ret = TRUE;
			}
			...
			{
				// req received but wrong event !!
				ret = FALSE;
			}
#endif			
		
			//STECI_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "STECI_EventWaitReqReady: levelToWaitFor %d currentLevel %d ret %d", levelToWaitFor, currentLevel, ret);
		}
	}

	...
	while (1)
	{
		...
		vl_res = OSAL_s32EventWait (ServiceFollowing_TaskEventHandler,
			...
			if (OSAL_ERROR == vl_res)
			{
				...
				ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_ServiceFollowing_WaitingTask: wait error");
				break;
			}
...
//		DABMW_SF_PRINTF(TR_LEVEL_USER_1, "ETALTML_ServiceFollowing_Task : vl_res = %d, vl_resEvent = %d\n", vl_res, vl_resEvent);
		...
		else if (OSAL_ERROR == vl_res)
		{
			// Event wait failure ==> break;			
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_ServiceFollowing_Task: wait error");
			break;
		}	
		else if ((vl_resEvent == DABMW_SF_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
		{

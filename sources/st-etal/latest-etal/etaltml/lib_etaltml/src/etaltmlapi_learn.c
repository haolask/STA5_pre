...
static tVoid ETALTML_StopLearnAndSendEvent(ETAL_HANDLE hReceiver, etalLearnConfigTy *learnCfgp, etalLearnEventStatusTy status)
{
    ...
    if(ETAL_seek_stop_internal(hReceiver, learnCfgp->terminationMode) != ETAL_RET_SUCCESS)
    {
...
static tVoid ETALTML_ReportLearnInfo(ETAL_HANDLE hReceiver, EtalLearnStatusTy* learnStatusp)
{
	...
	if (learnStatusp->m_status == ETAL_LEARN_FINISHED)
	{
		...
		tuneStatusInternal.m_Frequency				 = learnStatusp->m_frequency;
		tuneStatusInternal.m_syncInternal			 = ETAL_TUNESTATUS_SYNCMASK_FOUND;
		tuneStatusInternal.m_externalRequestInfo	 = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalLearnRequestInProgress);
...
#endif //CONFIG_HOST_OS_TKERNEL
{
    ...
    while (TRUE)
    {
        ...
        if (OSAL_ERROR == vl_res)
		{
			...
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_LearnWaitingTask: wait error");
			break;
		}
		else if ((vl_resEvent == ETALTML_LEARN_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
        {
        ...
        * normal processing or event return flag contains EVENT_CMD.. so normal processing */
        ...
        {
	        if (ETALTML_LEARN_EVENT_KILL_FLAG == (ETALTML_LEARN_EVENT_KILL_FLAG & vl_resEvent))
	        {
	        	/* Kill the learn */
	        	break;
	        }
...
static tVoid ETALTML_LearnThreadEntry(tPVoid dummy)
{
    ...
    while (TRUE)
    {
       ...
       ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "ETALTML_LearnThreadEntry :  awaked, event = 0x%x, vl_res = %d\n",
        ...
        {
			// Event wait failure ==> break;			
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_LearnThreadEntry: wait error");
			break;
		}
		else if (ETALTML_LEARN_EVENT_SLEEP_FLAG == (vl_resEvent & ETALTML_LEARN_EVENT_SLEEP_FLAG))
        {
		...
		{
	        if ((vl_resEvent == ETALTML_LEARN_NO_EVENT_FLAG) ||
    ...
    if (ETAL_receiverIsValidHandle(hReceiver))
    {
        switch(learnCfgp->state)
        {
            ...
            case ETALTML_LEARN_START:
                ...
                //if (ETAL_mute(hReceiver, TRUE) == ETAL_RET_SUCCESS)
                {
                    if (ETAL_receiverGetBandInfo(hReceiver, &bandInfo) == OSAL_OK)
                    {
                        ...
                        if (ETALTML_CheckTuneReturn(hReceiver, ret) == OSAL_OK)
                        {
                            if(ETAL_seek_start_internal(hReceiver, cmdDirectionUp, learnCfgp->step, cmdAudioMuted, dontSeekInSPS, TRUE) == ETAL_RET_SUCCESS)
                            {
            ...
            case ETALTML_LEARN_CONTINUE:
                ...
                if(ETAL_seek_continue_internal(hReceiver) == ETAL_RET_SUCCESS)
                {
                    /* Change learn state */
                    learnCfgp->state = ETALTML_LEARN_STARTED;
                }

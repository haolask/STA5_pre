	...
	*extAutoSeekStatus = RdsSeekCfgp->seekStatus;
    ...
    switch(extAutoSeekStatus->m_status)
    {
        ...
        case ETAL_SEEK_ERROR_ON_STOP :
		  ...
		  if (true == ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress))
            {
                /* Send ETAL_INFO_RDS_SEEK event */
                ETAL_callbackInvoke(ETAL_COMM_EVENT_CALLBACK_HANDLER, cbTypeEvent, ETAL_INFO_RDS_SEEK, (tVoid *)extAutoSeekStatus,  sizeof(EtalSeekStatus));
            }
        ...
        case ETAL_SEEK_FINISHED :
			...
			tuneStatusInternal.m_externalRequestInfo	 = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress);
        	...
        	if (true == ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress))
            {
                /* Send ETAL_INFO_RDS_SEEK event */
                ETAL_callbackInvoke(ETAL_COMM_EVENT_CALLBACK_HANDLER, cbTypeEvent, ETAL_INFO_RDS_SEEK, (tVoid *)extAutoSeekStatus,  sizeof(EtalSeekStatus));
				ETAL_receiverSetSpecial(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress, cmdActionStop);
            }
			else
			{
				// strange no seek external in progress ??
				printf("ETALTML_Report_RdsSeek_Info, no cmdSpecialExternalRDSSeekRequestInProgress in progress \n");
			}
...
ETAL_STATUS etaltml_RDS_AF(ETAL_HANDLE hReceiverA, ETAL_HANDLE hReceiverB, tBool AFOn)
{
	...
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "APP->ETAL: etaltml_RDS_AF(recA: %d, recB: %d mode: %d)", hReceiverA, hReceiverB, AFOn);
	...
	if ((ETAL_receiverIsValidHandle(hReceiverA) == FALSE) 
		|| (ETAL_receiverIsValidRDSHandle(hReceiverA) == FALSE)
		)
	{
		ret = ETAL_RET_INVALID_RECEIVER;
	}
	else
	{
		...
		switch (std)
		{
			...
			case ETAL_BCAST_STD_HD_FM:
				...
				RdsFuncPara.F_TAEnable   |= ETALTML_RDS_Strategy_GetFunctionStatus(hReceiverA, RDS_FUNC_TA);
				RdsFuncPara.F_EONEnable = ETALTML_RDS_Strategy_GetFunctionStatus(hReceiverA, RDS_FUNC_EON);
				RdsFuncPara.F_REGEnable = ETALTML_RDS_Strategy_GetFunctionStatus(hReceiverA, RDS_FUNC_REG);
				...
				if (TRUE == AFOn)
				{
					...
					//
					if (ETAL_intCbIsRegisteredPeriodic(ETALTML_RDSStrategyPeriodicFunc, hReceiverA) == FALSE)
					{
				...
				else
				{
					...
					and no more needed */
					if ((RdsFuncPara.F_AFEnable == FALSE)
						&&
	...
	// Receiver B is the background receiver
	// for now it is mandatory to provide if TUNER_APP_BACKGROUND activated
	//
#ifdef TUNER_APP_BACKGROUND

	//Did not check hReceiverB at this moment
	if ((ETAL_receiverIsValidHandle(hReceiverB) == TRUE) 
		&& (ETAL_receiverIsValidRDSHandle(hReceiverB) == TRUE)
		)
	{
       	std = ETAL_receiverGetStandard(hReceiverB);
		switch (std)
		{
#ifdef CONFIG_ETAL_SUPPORT_CMOST_STAR
			case ETAL_BCAST_STD_FM:
			...
			case ETAL_BCAST_STD_HD_FM:
				...
				if (AFOn)
				{
							ETALTML_RDS_Strategy_Init_BgReceiver(hReceiverA, hReceiverB);
	               			ETAL_receiverSetSpecial(hReceiverB, cmdSpecialRDSStrategy, cmdActionStart);
				}
				else
				{
				            ETAL_receiverSetSpecial(hReceiverB, cmdSpecialRDSStrategy, cmdActionStop);
							ETALTML_RDS_Strategy_Init_BgReceiver(hReceiverA, ETAL_INVALID_HANDLE);
				}
#endif
...
exit:
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "ETAL->APP: etaltml_RDS_AF() = %s", ETAL_STATUS_toString(ret));
...
ETAL_STATUS etaltml_RDS_TA(ETAL_HANDLE hReceiver, tBool TAOn)
{
	...
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "APP->ETAL: etaltml_RDS_TA(rec: %d, mode: %d)", hReceiver, TAOn);
		...
		switch (std)
		{
			...
			case ETAL_BCAST_STD_HD_FM:
...
#if defined (CONFIG_ETAL_SUPPORT_CMOST_STAR) && defined (CONFIG_ETALTML_HAVE_RDS_STRATEGY)
				// why dependance with AF function ??
				...
				//{
					...
					RdsFuncPara.F_AFEnable   = ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_AF);
					RdsFuncPara.F_TAEnable   = TAOn;
					RdsFuncPara.F_EONEnable = ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_EON);
					RdsFuncPara.F_REGEnable = ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_REG);				
					
					ret = ETALTML_RDS_Strategy_Init(hReceiver, RdsFuncPara);
					...
					if (TRUE == TAOn)
					{
						// if not already registered
						//
						if (ETAL_intCbIsRegisteredPeriodic(ETALTML_RDSStrategyPeriodicFunc, hReceiver) == FALSE)
						{
							ret = ETAL_intCbRegisterPeriodic(ETALTML_RDSStrategyPeriodicFunc, hReceiver, 10);
						}

						// enable rds flags
						ETAL_receiverSetSpecial(hReceiver, cmdSpecialRDSStrategy, cmdActionStart);
					}
					else
					{
						/* deregister periodic function if already registered 
						...
						if ((RdsFuncPara.F_AFEnable == FALSE)
							...
							(RdsFuncPara.F_TAEnable == FALSE))
						{					
							if (ETAL_intCbIsRegisteredPeriodic(ETALTML_RDSStrategyPeriodicFunc, hReceiver) == TRUE)
							{
								ret = ETAL_intCbDeregisterPeriodic(ETALTML_RDSStrategyPeriodicFunc, hReceiver);
								if (ret != ETAL_RET_SUCCESS)
								{
									return ret;
								}
							...
							// disable rds flags
							ETAL_receiverSetSpecial(hReceiver, cmdSpecialRDSStrategy, cmdActionStop);
						}
					}	
				//}
				//else
...
exit:
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "ETAL->APP: etaltml_RDS_TA() = %s", ETAL_STATUS_toString(ret));
...
static tVoid ETALTML_RdsSeekIntCbFunc(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context)
{
    ...
    etalRdsSeekConfigTy *RdsSeekCfgp = NULL;
 	...
 	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RdsSeekIntCbFunc, receiver = %d\n", hGeneric);
	...
	if (ETAL_handleIsReceiver(hGeneric))
	{
		...
		// because receiver is already assumed lock by caller
			...
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RdsSeekIntCbFunc, receiver = %d, status = %d, frequency %d, freq found = %d full cycle = %d\n", 
				hGeneric, pl_autoSeekStatus->m_status, pl_autoSeekStatus->m_frequency, 
				pl_autoSeekStatus->m_frequencyFound, pl_autoSeekStatus->m_fullCycleReached);
...
#endif //CONFIG_HOST_OS_TKERNEL
{
    ...
    while (TRUE)
    {
        ...
        if (OSAL_ERROR == vl_res)
		{
			...
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_RdsSeekWaitingTask: wait error");
			break;
		}
		else if ((vl_resEvent == ETALTML_RDS_SEEK_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
        {
        ...
        * normal processing or event return flag contains EVENT_CMD.. so normal processing */
        ...
        {
	        if (ETALTML_RDS_SEEK_EVENT_KILL_FLAG == (ETALTML_RDS_SEEK_EVENT_KILL_FLAG & vl_resEvent))
	        {
	        	/* Kill the learn */
	        	break;
	        }
...
static tVoid ETALTML_RdsSeekThreadEntry(tPVoid dummy)
{
    ...
    while (FALSE == vl_completed)
    {
       vl_res = OSAL_s32EventWait(v_ETALTML_RdsSeek_TaskEventHandler,
		...
		{
			// Event wait failure ==> break;			
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETALTML_RdsSeekThreadEntry: wait error");
			break;
		}
		else if (ETALTML_RDS_SEEK_EVENT_SLEEP_FLAG == (vl_resEvent & ETALTML_RDS_SEEK_EVENT_SLEEP_FLAG))
        {
	    ...
	    {
			if ((vl_resEvent == ETALTML_RDS_SEEK_NO_EVENT_FLAG) ||
...
//					if (FALSE == RdsSeekCfgp->autoSeek_complete)
					{
						...
						ETAL_seek_stop_internal(hReceiver, lastFrequency);
					...
					// clear RDS info
					// this is to be done by the registration on end of rds seek
					//                    
					vl_complete = TRUE;
					...
					// move to IDLE
	                    ...
	                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_FINISHED, switch to ETALTML_RDS_AUTO_SEEK_IDLE.\r\n");
#endif
				...
				case ETALTML_RDS_AUTO_SEEK_WAIT_AUTOSEEK:
	             ...
	             case ETALTML_RDS_AUTO_SEEK_WAITRDS:
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_EXT_AUTO_SEEK_WAITRDS, switch to ETALTML_EXT_AUTO_SEEK_WAITPI.\r\n");
#endif
	                ...
	                else if(ETALTML_RDS_Strategy_RDSTimer_Get(hReceiver, RDS_TIMER_10MS) == 0)
	                {
	                    ...
	                    //
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_EXT_AUTO_SEEK_WAITRDS, time out.\r\n");
#endif
	                    ...
	                    ETAL_seek_continue_internal(hReceiver);
	                }
	            ...
	            case ETALTML_RDS_AUTO_SEEK_WAITPI:
	                    ...
	                    ( currentPI != 0))
	                {
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                        ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITPI, switch to ETALTML_EXT_AUTO_SEEK_WAITPTY.\r\n");
#endif
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
		                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITPI, switch to ETALTML_RDS_AUTO_SEEK_WAITTP.\r\n");
#endif
		                ...
		                else if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_PISeek))
		                {
		                	if(ETALTML_RDS_Strategy_IsPISeekOk(hReceiver))
		                    {
								...
								RdsSeekCfgp->state  = ETALTML_RDS_AUTO_SEEK_FINISHED;
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
								ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITPI, PI Station Found, switch to ETALTML_RDS_AUTO_SEEK_FINISHED.\r\n");
#endif
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
								ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITPI, switch to ETALTML_RDS_AUTO_SEEK_WAIT_AUTOSEEK.\r\n");
#endif
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
							ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITPI, Rds Station Found, switch to ETAL_SEEK_FINISHED.\r\n");
#endif
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
						ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: Timeout, ETALTML_RDS_AUTO_SEEK_WAITPI, switch to ETALTML_RDS_AUTO_SEEK_WAIT_AUTOSEEK.\r\n");
#endif
						// Request seek to continue 
			            ETAL_seek_continue_internal(hReceiver);
	                }
					...
					{
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
							ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: state = %d, timer = %d \r\n", RdsSeekCfgp->state, ETALTML_RDS_Strategy_RDSTimer_Get(hReceiver, RDS_TIMER_10MS));
#endif
					}
	            ...
	            case ETALTML_RDS_AUTO_SEEK_WAITTP:
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	               ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITTP, TP: %d, TA: %d  m_validityBitmap:%x\r\n", rdsData.m_TP, rdsData.m_TA, rdsData.m_validityBitmap);
#endif
	               ...
	               {
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	               	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITTP, TP Found.\r\n");
#endif
	                ...
	                {
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_RDS_AUTO_SEEK_WAITTP, timeout.\r\n");
#endif
	 					// no TP within timer
						vl_RdsSeekStatus.m_status = ETAL_SEEK_RESULT;
                   		ETALTML_Report_RdsSeek_Info(hReceiver, &vl_RdsSeekStatus);
	            ...
	            case ETALTML_RDS_AUTO_SEEK_WAITPTY:
				 	...
				 	if ((rdsData.m_validityBitmap & ETAL_DECODED_RDS_VALID_PTY) == ETAL_DECODED_RDS_VALID_PTY)
	                {
	                    ...
	                    {
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                    	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_EXT_AUTO_SEEK_WAITPTY, TPY Found.\r\n");
#endif
	   						// 
							RdsSeekCfgp->state	= ETALTML_RDS_AUTO_SEEK_FINISHED;
	                ...
	                {
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	                    ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "RDS Seek status: ETALTML_EXT_AUTO_SEEK_WAITPTY, timeout.\r\n");
#endif
	                   	/* Send event and notify */
						vl_RdsSeekStatus.m_status = ETAL_SEEK_RESULT;
						ETALTML_Report_RdsSeek_Info(hReceiver, &vl_RdsSeekStatus);
...
ETAL_STATUS ETALTML_rds_seek_start(ETAL_HANDLE hReceiver, etalSeekDirectionTy direction, tU32 step, etalSeekAudioTy exitSeekAction, EtalRDSSeekTy rdsSeekOption)
{
  ...
  etalRdsSeekConfigTy *RdsSeekCfgp = &(recvp->RdsSeekCfg);
	...
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_rds_seek_start: (rec: %d, dir: %d, ste: %d, exiSeeAct: %d) (tpSeek %d, taSeek %d, ptySeek %d, ptyCode %d, piSeek %d, pi %d,)",
		hReceiver, direction, step, exitSeekAction,
		rdsSeekOption.tpSeek, rdsSeekOption.taSeek, rdsSeekOption.ptySeek, rdsSeekOption.ptyCode, rdsSeekOption.piSeek, rdsSeekOption.pi);
	...
	// start normal seek
	// do not send event, it will be done on ETAL_SEEK reception
	/* Send event and notify */
	...
	// 
	if(ETAL_seek_start_internal(hReceiver, direction, step, cmdAudioMuted, dontSeekInSPS, TRUE) == ETAL_RET_SUCCESS)
	{
...
exit:
	...
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_rds_seek_start: (ret = %d)", ret);
	    ...
	    if ((ret = ETAL_receiverGetLock(hReceiver)) == ETAL_RET_SUCCESS)
	    {
			...
			else
			{
				...
				else
				{
					...
					//
					ETAL_receiverSetSpecial(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress, cmdActionStart);

...
static tBool ETALTML_RDSIsAFQualityOk(RDS_TunerQualInfoTy *p_I_AFQual, tU8 vI_AFStrengthFast);
...
static tVoid ETALTML_RDSTA_MainProc(ETAL_HANDLE hReceiver, tSInt slot);
static tVoid ETALTML_RDSPISeekStrategy_MainProc(ETAL_HANDLE hReceiver, tSInt slot);
static tVoid ETALTML_RDSAF_MainProc(ETAL_HANDLE hReceiver, tSInt slot);
static tVoid ETALTML_RDS_Strategy_RdsSeekCbFunc(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context);

/***************************
 *
 * ETALTML_RDS_Strategy_RdsSeekCbFunc
 *
 **************************/
static tVoid ETALTML_RDS_Strategy_RdsSeekCbFunc(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context)
{
	ETAL_HANDLE hReceiver, hReceiverMain, vl_selectedReceiver ;
	tSInt slot;
	
	hReceiverMain = (ETAL_HANDLE) context;
	
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDS_Strategy_RdsSeekCbFunc : hGeneric = %d, receiver Main = %d\r\n", 
							hGeneric, hReceiverMain);
#endif

	if (ETAL_handleIsReceiver(hGeneric))
	{
		hReceiver = hGeneric;
		vl_selectedReceiver = hReceiver;
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDS_Strategy_RdsSeekCbFunc : slot = %d, freq = %d\r\n", 
							slot, ETAL_receiverGetFrequency(hReceiver));
#endif		

		// deregister call back
		(LINT_IGNORE_RET)ETAL_intCbDeregister(callAtEndOfRdsSeek, ETALTML_RDS_Strategy_RdsSeekCbFunc, hReceiver);

#ifdef TUNER_APP_BACKGROUND
		// in case of background, the seek has been done on a different receiver
		// retrieve the main receiver
		// 
		
		if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiverMain, RDS_FUNC_BG_PROC_ACTIVE)
			&&
			(hReceiverMain != hReceiver))
		{
			// but processing is on main receiver :
			vl_selectedReceiver = hReceiverMain;
		}
#endif

		// reset on-going procedure status
		// if we are call it means Seek is ended : 
		// we disable the on-going request : 
		ETALTML_RDS_Strategy_SetFunctionStatus(vl_selectedReceiver, RDS_FUNC_TASeek, FALSE);
	    ETALTML_RDS_Strategy_SetFunctionStatus(vl_selectedReceiver, RDS_FUNC_PISeek, FALSE);

		// new frequency available
		// so notification
			
		// clear RDS info
				
		ETALTML_RDS_Strategy_Data_Init(vl_selectedReceiver);
		ETALTML_RDSresetData(vl_selectedReceiver);  //  CLEAR RDS DATA, reset
			...
			(hReceiverMain != hReceiver))
		{
			...
			// it was search on background only !
			//
			
			ETALTML_RDS_SetFreq(hReceiverMain, RDSMgr[slot].AFInfo.AFFreq[0]);
		}
#endif
		...
		// so notification
		// data have been init
		// so we are now on AF[0]

		ETALTML_RDS_DataNotification(vl_selectedReceiver, slot, NULL, TRUE, RDSMgr[slot].AFInfo.AFFreq[0], FALSE, FALSE);
...
static tVoid ETALTML_RDS_Strategy_Timer_hdr(tSInt slot)
{
	...
	RDS_Timer[slot].T10ms_count++;
//	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "ETALTML_RDS_Strategy_Timer_hdr in RDS_Timer[%d].Timer10ms %d\n", slot, RDS_Timer[slot].Timer10ms);
	...
	if(RDS_Timer[slot].T10ms_count >= 10)
	{
		RDS_Timer[slot].T10ms_count  = 0;
		...
		RDS_Timer[slot].T100ms_count++;
		if(RDS_Timer[slot].T100ms_count >= 10)
		{
			RDS_Timer[slot].T100ms_count = 0;
...
ETAL_STATUS ETALTML_RDS_DataNotification(ETAL_HANDLE hReceiver, tSInt slot, RDS_TASwitchSystemMode * pTASwitchMode, tBool AFSwitched, tU8 afFreq, tBool TASeekStarted, tBool PISeekStarted)
{
	...
	{
		RDSStrategyStatus.m_AFSwitchedFreq = ETAL_INVALID_FREQUENCY;
		...
		if (AFSwitched)
		{
			...
			RDSStrategyStatus.m_AFSwitchedFreq = FREQ_AF_TO_KHZ(afFreq);
		}
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT)
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDS_Strategy_IsPISeekOk, receiver = %d, PI found status = %d, PI %x \n",
...
tBool ETALTML_RDS_Strategy_GetFunctionStatus(ETAL_HANDLE hReceiver, RDS_FuncType rdsRuncType)
{
	...
	{		
		...
		if (rdsRuncType == RDS_FUNC_TASeek) 	return RDSMgr[slot].ProcessFlag.F_InTASeek;
		if (rdsRuncType == RDS_FUNC_PTYSELECTTYPE) 	return RDSMgr[slot].FunctionInfo.PTYSelectType;
		if (rdsRuncType == RDS_FUNC_RDS_ENABLED)	return(ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialRDS));
		if (rdsRuncType == RDS_FUNC_BG_PROC_ACTIVE) return RDSMgr[slot].FunctionInfo.F_BgProcEnable;
	}
...
tVoid ETALTML_RDS_Strategy_SetFunctionStatus(ETAL_HANDLE hReceiver, RDS_FuncType rdsRuncType, tBool value)
{
	...
	{		
		...
		if (rdsRuncType == RDS_FUNC_TASeek) 	RDSMgr[slot].ProcessFlag.F_InTASeek = value;
		if (rdsRuncType == RDS_FUNC_BG_PROC_ACTIVE) 	RDSMgr[slot].FunctionInfo.F_BgProcEnable = value;
		

		// not Set for RDS_FUNC_RDS_ENABLED : the ETAL receiver info is used.
		// 
	}
}
...
ETAL_STATUS ETALTML_RDS_SetFreq(ETAL_HANDLE hReceiver, tU8 setfreq)
{
	...
	{
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
		ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDS_SetFreq, recevier %d, set frequency to : %d\r\n", hReceiver, FREQ_AF_TO_KHZ(setfreq));
#endif
...
/**
* ETALTML_RDSIsAFQualityOk
* @desc: This is the sub routine used to check if AF is acceptable quality
* @param: *p_I_AFQual 
* @param: vI_AFStrengthFast // Smeter limit
* @return tBool :  TRUE if quality OK, else FALSE
* @note:
*/

static tBool ETALTML_RDSIsAFQualityOk(RDS_TunerQualInfoTy *p_I_AFQual, tU8 vI_AFStrengthFast)
{
	tBool vl_qualityOk = FALSE;

#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSIsAFQualityOk threshold : Smeter %d, Detuning %d, Multipath %d, AdjChannel %d, Mpx %d, SNR %d\r\n",
								vI_AFStrengthFast,
								ETAL_CMOST_GET_AMFMMON_FM_DETUNING(ETALTML_RDS_STRATEGY_THRESHOLD_DETUNING),
								ETAL_CMOST_GET_AMFMMON_FM_MULTIPATH(ETALTML_RDS_STRATEGY_THRESHOLD_MP), 
								ETAL_CMOST_GET_AMFMMON_FM_ADJACENT_CHANNEL(ETALTML_RDS_STRATEGY_THRESHOLD_ADJ),
								ETAL_CMOST_GET_AMFMMON_FM_MPX_NOISE(ETALTML_RDS_STRATEGY_THRESHOLD_MPX),
								ETAL_CMOST_GET_AMFMMON_FM_SNR(ETALTML_RDS_STRATEGY_THRESHOLD_SNR));
#endif	

	if (p_I_AFQual->Smeter < vI_AFStrengthFast
		|| p_I_AFQual->Detuning >= ETAL_CMOST_GET_AMFMMON_FM_DETUNING(ETALTML_RDS_STRATEGY_THRESHOLD_DETUNING)
		|| p_I_AFQual->Multipath>= ETAL_CMOST_GET_AMFMMON_FM_MULTIPATH(ETALTML_RDS_STRATEGY_THRESHOLD_MP)
		|| p_I_AFQual->MpxNoise>= ETAL_CMOST_GET_AMFMMON_FM_MPX_NOISE(ETALTML_RDS_STRATEGY_THRESHOLD_MPX)  
		|| p_I_AFQual->SNR <= ETAL_CMOST_GET_AMFMMON_FM_SNR(ETALTML_RDS_STRATEGY_THRESHOLD_SNR)  
		|| p_I_AFQual->AdjChannel >= ETAL_CMOST_GET_AMFMMON_FM_ADJACENT_CHANNEL(ETALTML_RDS_STRATEGY_THRESHOLD_ADJ)  )
	{
		vl_qualityOk = FALSE;
	}
	else
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 	
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSIsAFQualityOk ret = %d \r\n",vl_qualityOk);
#endif
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSAF_AFCheckBack, rec = %d, slot = %d, set frequency back to : %d\r\n",
			hReceiver, slot, FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]));
#endif
...
ETAL_STATUS ETALTML_RDSAF_AFSearchNext(ETAL_HANDLE hReceiver, tSInt slot, tBool bAFEnd)
{
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSAF_AFSearchNext, rec = %d, slot = %d, set frequency to : %d\r\n",
			hReceiver, slot, FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[i]));
#endif
			ETALTML_RDSresetData(hReceiver);  //  CLEAR RDS DATA, reset rds data
	...
	// this part is not strictly needed in background case :
	// the call is to get back to best frequency here latest one
	// but on the background, not really needed.
	// anyhow keep  it : 
	//
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSAF_AFSearchNext, set frequency to : %d\r\n", FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]));
#endif
...
ETAL_STATUS ETALTML_RDSEON_CheckEON(ETAL_HANDLE hReceiver, tSInt slot)
{
	...
	ETAL_HANDLE vl_selectedreceiver_for_EonTASearch = hReceiver;


	// Check on requested receiver if EON TA activity ok
...
#ifdef TUNER_APP_BACKGROUND
	if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
	{
		vl_selectedreceiver_for_EonTASearch = RDSMgr[slot].AFInfo.AF_Bg_Receiver;
	}
	else
#endif
	{
		vl_selectedreceiver_for_EonTASearch = hReceiver;
	}

	ETALTML_RDS_Mute(vl_selectedreceiver_for_EonTASearch, SOFTON);


	for(i = 0; i < RDSMgr[slot].EONInfo.EONAFNum; i++)
	{
		freq = FREQ_AF_TO_KHZ(RDSMgr[slot].EONInfo.EONAFFreq[i]);
		ret = ETAL_AF_start(vl_selectedreceiver_for_EonTASearch, cmdNormalMeasurement, freq, 0, &afQual);

#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
		ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSEON_CheckEON, rec %d, set frequency to : %d\r\n", 
		vl_selectedreceiver_for_EonTASearch, FREQ_AF_TO_KHZ(RDSMgr[slot].EONInfo.EONAFFreq[i]));
#endif
		if (ret == ETAL_RET_SUCCESS)
		{
			ETALTML_RDS_AFCheckInit(vl_selectedreceiver_for_EonTASearch);		//  CLEAR RDS DATA, reset rds data
	...
	ETALTML_RDSAF_ReadTunerAFQual(vl_selectedreceiver_for_EonTASearch, &TunerAFQual);
	
	if(i > 0)	RDSMgr[slot].EONInfo.EONAFSmeter[i - 1] = TunerAFQual.Smeter;

	freq = FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]);
	ret = ETAL_AF_end(vl_selectedreceiver_for_EonTASearch, freq, &afQual);
	
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
	ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "ETALTML_RDSEON_CheckEON, rec %d set frequency to : %d\r\n", 
	vl_selectedreceiver_for_EonTASearch, FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]));
#endif

	ETALTML_RDS_AFCheckInit(vl_selectedreceiver_for_EonTASearch);		//  CLEAR RDS DATA, reset rds data
	
	ETALTML_RDS_Mute(vl_selectedreceiver_for_EonTASearch, SOFTOFF);
	...
	// in background case : the tuner is not set on target EON frequency : set it here if find
	//
	if (vl_selectedreceiver_for_EonTASearch != hReceiver)
	{
		// set frequency
		ETALTML_RDS_SetFreq(hReceiver, freq);
...
tVoid ETALTML_RDS_Strategy_Data_Init(ETAL_HANDLE hReceiver)
{
	...
	// despite not active reset data
#if 0	
	ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_RDS_ENABLED);
	...
	if ((RDSMgr[slot].ProcessFlag.F_PISeek)
		||(RDSMgr[slot].ProcessFlag.F_InTASeek))
		{
			// Seek is on going, return
			return;
		}
	...
	printf("ETALTML_RDS_Strategy_Data_Init freq = %d \n", currentFreq);
...
ETAL_STATUS ETALTML_RDS_Strategy_Init(ETAL_HANDLE hReceiver, RDS_FunctionInfoTy RdsFuncPara)
{
	...
	if (!RDSMgrLocksInit)
	{
		...
		for (slot = 0; slot < DABMW_RDS_SOURCE_MAX_NUM; slot++)
		{
			...
			RDSMgr[slot].AFInfo.F_RDSStationSlow  		= FALSE;
			RDSMgr[slot].AFInfo.AF_Bg_Receiver	= ETAL_INVALID_HANDLE;
			RDSMgr[slot].AFInfo.AF_Bg_slot 		= 0xFF;
			RDSMgr[slot].FunctionInfo.F_BgProcEnable	= FALSE;
			RDSMgr[slot].AFInfo.AFSmeterCheckFreqTimer = 10;  		//100ms

			// init the timer part
			RDS_Timer[slot].T100ms_count=0;
			RDS_Timer[slot].T100ms_count=0;
			RDS_Timer[slot].Timer10ms=0;
			RDS_Timer[slot].Timer100ms=0;
		}
...
#ifdef TUNER_APP_BACKGROUND
/**
* ETALTML_RDS_Strategy_Init_BgReceiver
* @desc This routine is to initialize the process of RDS for bg
* @param: hReceiver,  RDS_FunctionInfoTy
* @return ETAL_STATUS :  error code.
* @note:
*/
ETAL_STATUS ETALTML_RDS_Strategy_Init_BgReceiver(ETAL_HANDLE hReceiver, ETAL_HANDLE hReceiverBg)
{
	ETAL_STATUS ret = ETAL_RET_SUCCESS;
	tSInt slot, slot_bg;
	...
	else
	{
		...
		// set the background receiver and the slot
		//
		
		if ((ETAL_INVALID_HANDLE == hReceiverBg) || (hReceiverBg == hReceiver))
		{
			// disable the RDS on bg if needed 
			if (ETAL_INVALID_HANDLE != RDSMgr[slot].AFInfo.AF_Bg_Receiver)
			{
				if (ETAL_RET_SUCCESS != ETAL_stop_RDS(RDSMgr[slot].AFInfo.AF_Bg_Receiver))
				{
					// ERROR IN RDS activation
					//
					ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_ERRORS, "ETALTML_RDS_Strategy_Init_BgReceiver : error in bg rds stop rec = %d \n", RDSMgr[slot].AFInfo.AF_Bg_Receiver);	
				}
			}
		...
		{
#ifdef ETAL_RDS_IMPORT
			slot_bg = ETAL_receiverGetRDSSlot(hReceiverBg);	
#else
			slot_bg = DABMW_RdsGetSlotFromSource ((DABMW_storageSourceTy)hReceiverBg);
#endif	
			RDSMgr[slot].AFInfo.AF_Bg_Receiver	= hReceiverBg;
			RDSMgr[slot].AFInfo.AF_Bg_slot = slot_bg;
			RDSMgr[slot].FunctionInfo.F_BgProcEnable	= TRUE;

			// we should make sure to activate the RDS on the bg as well 
	    	...
	    	vl_etalRdsAttr.nbRdsBlockforInteruptNormalMode = 16;    /* for normal mode, indicate the number of decoded blocks which will generate an interrupt for buffer read */

			if (ETAL_RET_SUCCESS != ETAL_start_RDS(hReceiverBg, &vl_etalRdsAttr))
			{
				// ERROR IN RDS activation
				//
				ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_ERRORS, "ETALTML_RDS_Strategy_Init_BgReceiver : error in bg rds start rec = %d \n", hReceiverBg);
	
			}
			

			// if BG enable : change the timings
			RDSMgr[slot].AFInfo.AFCheckDisable1sTimerInitVal 				= ETALTML_RDS_STRATEGY_AFCHECK_DISABLE1STIMERINIT_BG;
			RDSMgr[slot].AFInfo.AFCheckBetweenList1sTimerInitVal 			= ETALTML_RDS_STRATEGY_AFCHECK_BETWEENLIST1STIMERINIT_BG;
			RDSMgr[slot].AFInfo.AFCheckBetweenFreq10msTimerInitVal 		= ETALTML_RDS_STRATEGY_AFCHECK_BETWEENFREQ10MSTIMERINIT_BG;
			RDSMgr[slot].AFInfo.AFFastCheckBetweenList1sTimerInitVal 		= ETALTML_RDS_STRATEGY_AFFASTCHECK_BETWEENLIST1STIMERINIT_BG;
			RDSMgr[slot].AFInfo.AFFastCheckBetweenFreq10msTimerInitVal 	= ETALTML_RDS_STRATEGY_AFFASTCHECK_BETWEENFREQ10MSTIMERINIT_BG;
			RDSMgr[slot].AFInfo.AFDisable100msTimer 	= ETALTML_RDS_STRATEGY_AFDISABLE100MSTIMER_BG;
			RDSMgr[slot].PIInfo.PISeek100msTimerInitVal 	= ETALTML_RDS_STRATEGY_PISEEK100MSTIMERINIT_BG - ETALTML_RDS_STRATEGY_PISEEK_COMP_WAITTIME;
...
tBool ETALTML_RDS_Strategy_RadioIsBusy(ETAL_HANDLE hReceiver)
{
	...
	if (!bBusy)  bBusy = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialRdsSeek);
	...
	if (!bBusy)  bBusy = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalSeekRequestInProgress);
	if (!bBusy)  bBusy = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalRDSSeekRequestInProgress);
	if (!bBusy)  bBusy = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalLearnRequestInProgress);
	if (!bBusy)  bBusy = ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialExternalScanRequestInProgress);
...
/**
* ETALTML_RDSTA_MainProc
* @desc This routine is main function of TA handling
* @param: hReceiver;  slot
* @return: None
* @note:
*/
tVoid ETALTML_RDSTA_MainProc(ETAL_HANDLE hReceiver, tSInt slot)
{
	ETAL_STATUS ret = ETAL_RET_SUCCESS;
	ETAL_HANDLE vl_ReceiverSelected = hReceiver;
		...
		if(RDSMgr[slot].FunctionInfo.F_TAEnable && (!RDSMgr[slot].ProcessFlag.F_InEONTASwitch))
		{
			...
			else
			{
				if(RDSMgr[slot].FuncTimer.AutoTASeekTimer == 1 &&  (!ETALTML_RDS_Strategy_RadioIsBusy(hReceiver)) )
				{
					...
					if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
					{
						vl_ReceiverSelected = RDSMgr[slot].AFInfo.AF_Bg_Receiver;
					}
					else
					{
						vl_ReceiverSelected = hReceiver;
					}
#endif
					ret = ETALTML_rds_seek_start(vl_ReceiverSelected, cmdDirectionUp, ETALTML_RDS_STRATEGY_FM_SEEK_STEP, cmdAudioUnmuted, rdsSeekOption);
					if (ret == ETAL_RET_SUCCESS)
					{
						ETALTML_RDS_DataNotification(hReceiver, slot, NULL, FALSE, INVALID_AF, TRUE, FALSE);
						RDSMgr[slot].ProcessFlag.F_InTASeek = TRUE;

						// Register for end of seek 
	

						if (ETAL_intCbRegister(callAtEndOfRdsSeek, ETALTML_RDS_Strategy_RdsSeekCbFunc, vl_ReceiverSelected, hReceiver) != ETAL_RET_SUCCESS)
                        {
                       		printf("ETALTML_RDS_Strategy_RdsSeekCbFunc not deregistered\n");
...
/**
* ETALTML_RDSPISeekStrategy_MainProc
* @desc This routine is main function of PI SEEK strategy handling
* @param: hReceiver;  slot
* @return: None
* @note:
*/
tVoid ETALTML_RDSPISeekStrategy_MainProc(ETAL_HANDLE hReceiver, tSInt slot)
{
	ETAL_STATUS ret;
	ETAL_HANDLE vl_ReceiverSelected;
...
tVoid ETALTML_RDSAF_MainProc(ETAL_HANDLE hReceiver, tSInt slot)
{
	...
	if(RDSMgr[slot].FunctionInfo.F_AFEnable)
	{
			...
			(recvp->isTuneInProgress) )		
		{
			RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_IDLE;
#ifdef TUNER_APP_BACKGROUND	
			RDSMgr[slot].AFInfo.AF_BG_Checking = 0;
#endif
			return;
		}
		...
		if(RDSMgr[slot].AFInfo.AFDisable100msTimer > 0)
		{
			...
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_2, "RDSMgr[slot].AFInfo.AFDisable100msTimer : %d,  RDSMgr[slot].AFInfo.AFCheckMode : %d, ETALTML_RDS_Strategy_BreakAFCheck. \r\n", RDSMgr[slot].AFInfo.AFDisable100msTimer, RDSMgr[slot].AFInfo.AFCheckMode);
#endif
			(LINT_IGNORE_RET)ETALTML_RDS_Strategy_BreakAFCheck(hReceiver);
		}
			     ...
			     (RDSMgr[slot].AFInfo.AFSmeterCheckFreqTimer == 0))
		{
			...
			if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
			{
				if (RDSMgr[slot].AFInfo.AFCheckMode  == AFCHECK_MODE_IDLE)
				{
					if (RDSMgr[slot].AFInfo.AFCheckBgFreqTimer == 0)
					{
						RDSMgr[slot].AFInfo.AFCheckBgFreqTimer = 30;	  //300 ms
						RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_BG_REQ;
					}
				}
			}
#endif	
	...
	switch(RDSMgr[slot].AFInfo.AFCheckMode)
	{
		...
		case AFCHECK_MODE_CHECK:
			...
			OSAL_s32ThreadWait(2);
#if 0
			// TMP which could be usefull
			// read current freq on bg
			freq = FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]);
			
			ret   = ETAL_AF_check_and_get_AF_quality(vl_ReceiverSelected, freq, 0, &afQual);
			...
			// Update based on RDS China import and new quality parameters
			TunerAFQual.MpxNoise = afQual.EtalQualityEntries.amfm.m_UltrasonicNoise;
			TunerAFQual.SNR = afQual.EtalQualityEntries.amfm.m_SNR;
						
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_CHECK, receiver = %d, freq = %d, Smeter = %d, detuning = %d, Mp = %d, adj = %d, MpxNoise = %d, SNR = %d\r\n",
				vl_ReceiverSelected, freq, TunerAFQual.Smeter, TunerAFQual.Detuning, TunerAFQual.Multipath, TunerAFQual.AdjChannel, TunerAFQual.MpxNoise, TunerAFQual.SNR);
#endif		
#endif
			...
			currentFreq = FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[0]);
			if (ret == ETAL_RET_SUCCESS)
			{
				TunerAFQual.Smeter = afQual.EtalQualityEntries.amfm.m_BBFieldStrength;
				TunerAFQual.Detuning = afQual.EtalQualityEntries.amfm.m_FrequencyOffset;
...
//#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_CHECK, current freq (%d) - smeter: %d, AF(%d) - Smeter = %d, delta: %d, AFStrenghFast %d\r\n", 
								currentFreq,
								RDSMgr[slot].AFInfo.AFSmeter[0], freq, RDSMgr[slot].AFInfo.AFSmeter[RDSMgr[slot].AFInfo.AFCheckPoint], 
								RDSMgr[slot].AFInfo.AFStrengthDeltaReal,  RDSMgr[slot].AFInfo.AFStrengthFast);
...
//#endif				
			...
			{			
				vl_qualityOk = ETALTML_RDSIsAFQualityOk(&TunerAFQual, RDSMgr[slot].AFInfo.AFStrengthFast);
				
				if (TRUE == vl_qualityOk)
				{
					...
					// switch the Frequency to check RDS
					// this is not yet the final switch just temporary to verify RDS
					//

					ETALTML_RDS_SetFreq(vl_ReceiverSelected, RDSMgr[slot].AFInfo.AFFreq[RDSMgr[slot].AFInfo.AFCheckPoint]);

					RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_CHECKRDS;
					
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
					ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, " AFCHECK_MODE_CHECK,  AF station is good,  switch to AFCHECK_MODE_CHECKRDS \r\n ");
#endif				
...
#ifdef TUNER_APP_BACKGROUND
					if (FALSE == ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
#endif						
					{	
				...
				{	
					// EPR Comment : 
					// why CheckBack ? we have not switched in theory  in that state ??
...
#ifdef TUNER_APP_BACKGROUND	
			case AFCHECK_MODE_BG_REQ:
				if (RDSMgr[slot].AFInfo.AFNum > 1)
				{
					RDSMgr[slot].AFInfo.AFCheckPoint = 1;
				
					RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_BG_CHECK;
				
					RDSMgr[slot].AFInfo.AF_BG_Checking = 1;

					// quality on main receiver
					if (ETALTML_RDSAF_ReadTunerCFQual(hReceiver, &TunerCFQual) == ETAL_RET_SUCCESS)
					{
						RDSMgr[slot].AFInfo.AFSmeter[0] += TunerCFQual.Smeter /3 - RDSMgr[slot].AFInfo.AFSmeter[0] /3;
					}
			...
			case AFCHECK_MODE_BG_CHECK:
				if(RDSMgr[slot].AFInfo.AFDisable[RDSMgr[slot].AFInfo.AFCheckPoint] > 0)
				{
					RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_BG_CHECKWAIT;
					break;
				}

				// what we do here is only reading quality for each AF.
				// 
				...
				freq = FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[RDSMgr[slot].AFInfo.AFCheckPoint]);
				ret   = ETAL_AF_check_and_get_AF_quality(RDSMgr[slot].AFInfo.AF_Bg_Receiver, freq, 0, &afQual);
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_BG_CHECK, freq = %d, Smeter = %d, detuning = %d, Mp = %d, adj = %d, MpxNoise = %d, SNR = %d\r\n",
					freq, TunerAFQual.Smeter, TunerAFQual.Detuning, TunerAFQual.Multipath, TunerAFQual.AdjChannel, TunerAFQual.MpxNoise, TunerAFQual.SNR);
#endif				

					RDSMgr[slot].AFInfo.AFSmeter[RDSMgr[slot].AFInfo.AFCheckPoint] += TunerAFQual.Smeter /3 -RDSMgr[slot].AFInfo.AFSmeter[RDSMgr[slot].AFInfo.AFCheckPoint] /3;


					if (RDSMgr[slot].AFInfo.AFCheckPoint < (RDSMgr[slot].AFInfo.AFNum - 1))
					{
						RDSMgr[slot].AFInfo.AFCheckPoint ++;
					}
					else
					{
						RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_BG_CHECKEND;
					}
			...
			case AFCHECK_MODE_BG_CHECKEND:
				if(RDSMgr[slot].AFInfo.AFNum > 1)
				{
					ETALTML_RDSAF_AFSort(slot, 1);
				}
				
				//RDSStrategyData.Timers.REAL_AFCheckStartTimer = RDSStrategyData.AFData.REAL_AFCheckStartTimerSet;
					RDSMgr[slot].AFInfo.AFCheckMode  = AFCHECK_MODE_IDLE;

					// check timers
					if ((RDSMgr[slot].AFInfo.AFSearchWaitTimer < 30)
						&&
						(RDSMgr[slot].AFInfo.F_RDSStationSlow))
					{
						RDSMgr[slot].AFInfo.AFSearchWaitTimer = 30;
					}
					
					
					if ( RDSMgr[slot].AFInfo.AFSmeter[0] < (RDSMgr[slot].AFInfo.AFStrengthFast /2))
					{
						if (RDSMgr[slot].AFInfo.AFSmeterLevel > 0)
						{
							RDSMgr[slot].AFInfo.AFSmeterLevel /= 2;
						}
					}
					
					break;

		case AFCHECK_MODE_BG_CHECKWAIT:
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_CHECKWAIT. \r\n");
#endif
			...
			break;
#endif // TUNER_APP_BACKGROUND

		case AFCHECK_MODE_CHECKRDS:
			bRDSStation = FALSE;

			ETALTML_get_RDSStationFlag(vl_ReceiverSelected, &bRDSStation);

#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_USER_1) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_1, "AFCHECK_MODE_CHECKRDS. rec = %d, rds station : %d\r\n", vl_ReceiverSelected, bRDSStation);
#endif
			if (bRDSStation)
			{
			...
			if(RDSMgr[slot].AFInfo.AFCheckWaitTimer == 0)
			{
				...
				if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
				{
					ETALTML_RDSAF_AFCheckBack(RDSMgr[slot].AFInfo.AF_Bg_Receiver, slot, TRUE);
				}
				else
#endif
				{
		...
		case AFCHECK_MODE_CHECKPI:
			...
			ret = ETALTML_get_PIStatus(vl_ReceiverSelected, &PIStatus, &currentPI, &lastPI, &backupPI);

#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_USER_1) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_1, "AFCHECK_MODE_CHECKPI, rec = %d, currentPI : %x, backupPI: %x  lastPI:%x \r\n", 
			vl_ReceiverSelected, currentPI, RDSMgr[slot].PIInfo.BackupPI, lastPI);
#endif		
			if ( (ETAL_RET_SUCCESS == ret)  && (currentPI != 0))
			{
			         ...
			         (((currentPI & 0xFF) == (RDSMgr[slot].PIInfo.BackupPI  & 0xFF)) && (!RDSMgr[slot].FunctionInfo.F_REGEnable))  )
				{
					...
					if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
					{
						// switch main tuner to this freq
				...
				else
				{
					...
					// no need to change bg tuner.. this is background
#ifdef TUNER_APP_BACKGROUND
					if (FALSE == ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
#endif
					{
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_CHECKEND. \r\n");
#endif
			if(RDSMgr[slot].AFInfo.AFNum > 1)
			{
				ETALTML_RDSAF_AFSort(slot, 1);
		...
		case AFCHECK_MODE_SEARCHREQ:
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_SEARCHREQ, rec = %d \r\n", vl_ReceiverSelected);
#endif
...
#endif
			{
				...
				for(RDSMgr[slot].AFInfo.AFCheckPoint = 0; RDSMgr[slot].AFInfo.AFCheckPoint < RDSMgr[slot].AFInfo.AFNum; RDSMgr[slot].AFInfo.AFCheckPoint++)
				{
					...
					ret = ETAL_AF_start(vl_ReceiverSelected, cmdNormalMeasurement, freq, 0, &afQual);

					if (ret == ETAL_RET_SUCCESS)
					{
						ETALTML_RDS_AFCheckInit(vl_ReceiverSelected);		//  CLEAR RDS DATA, reset rds data
					
						if(RDSMgr[slot].AFInfo.AFCheckPoint > 0)
						{
							// retrieved quality is the one from prior cell 
							// 
				...
				ETALTML_RDSAF_ReadTunerAFQual(vl_ReceiverSelected, &TunerAFQual);

				if(RDSMgr[slot].AFInfo.AFCheckPoint > 0)	 
				{
					RDSMgr[slot].AFInfo.AFSmeter[RDSMgr[slot].AFInfo.AFCheckPoint - 1] = TunerAFQual.Smeter;
				}
				...
				{
					int i;
					ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_2, "AFCHECK_MODE_SEARCHREQ, after AF signal sort. \r\n");
					ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_2, "		frequency	smeter\r\n");					
					for (i = 0; i < RDSMgr[slot].AFInfo.AFNum; i++)
					{
						ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "		%d		%d\r\n", FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[i]), RDSMgr[slot].AFInfo.AFSmeter[i]);
					}
				}
#endif
				RDSMgr[slot].AFInfo.AFCheckPoint = 0;

				ETALTML_RDSAF_AFSearchNext(vl_ReceiverSelected, slot, TRUE);
		...
		case AFCHECK_MODE_SEARCHRDS:
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_SEARCHRDS. rec = %d, freq: %d   RDSSTATION: %d \r\n", vl_ReceiverSelected, FREQ_AF_TO_KHZ(RDSMgr[slot].AFInfo.AFFreq[RDSMgr[slot].AFInfo.AFCheckPoint]), bRDSStation);
#endif			
			if (bRDSStation)			
			{
				RDSMgr[slot].AFInfo.AFCheckMode = AFCHECK_MODE_SEARCHPI;
			...
			if(RDSMgr[slot].AFInfo.AFCheckWaitTimer == 0)
			{
				...
				ETALTML_RDSAF_AFSearchNext(vl_ReceiverSelected, slot, FALSE);
		...
		case AFCHECK_MODE_SEARCHPI:
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_USER_1) 
			ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_USER_1, "AFCHECK_MODE_SEARCHPI.  rec = %d, slot = %d , RDSMgr[slot].PIInfo.BackupPI : %x, currentPI :%x\r\n", 
			vl_ReceiverSelected, slot, RDSMgr[slot].PIInfo.BackupPI, currentPI);
#endif		
			if( (ret == ETAL_RET_SUCCESS) && 
			     (currentPI != 0) &&
			...
			{
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_RDS_STRATEGY >= TR_LEVEL_COMPONENT) 
				ETALTML_RDS_STRATEGY_PRINTF(TR_LEVEL_COMPONENT, "AFCHECK_MODE_SEARCHPI. PI : %x, matched.\r\n", currentPI);
#endif
...
tVoid ETALTML_RDS_Strategy_Main(ETAL_HANDLE hReceiver)
{
	...
	//	if RDS Decoding is stopped, not run RDS strategy
	if (FALSE == ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_RDS_ENABLED))
	{
		return;
	}

	// Add-on : if a background is activated for this slot, the timer for this bg should be managed as well
	//
#ifdef TUNER_APP_BACKGROUND
	if (ETALTML_RDS_Strategy_GetFunctionStatus(hReceiver, RDS_FUNC_BG_PROC_ACTIVE))
	{
#ifdef ETAL_RDS_IMPORT
		vl_bgSlot = ETAL_receiverGetRDSSlot(RDSMgr[slot].AFInfo.AF_Bg_Receiver);
#else
		vl_bgSlot = DABMW_RdsGetSlotFromSource ((DABMW_storageSourceTy)RDSMgr[slot].AFInfo.AF_Bg_Receiver);
#endif

		ETALTML_RDS_Strategy_Timer_hdr(vl_bgSlot);
	}
#endif

	// EPR CHANGE : we can allow strategy even if AF not enabled : 
	// TA / TP PTY can be working
	//
	...
	// TO DO 
	// if seek is on going we should do nothing ??
	//
	...
	// TA Strategy
	if (RDSMgr[slot].FunctionInfo.F_TAEnable)
	{
		ETALTML_RDSTA_MainProc(hReceiver, slot);
	}


	// PI Seek strategy
	ETALTML_RDSPISeekStrategy_MainProc(hReceiver, slot);


	// Specific handling for PTY :
	// may be to remove.
	//

...
typedef struct
{
    ...
    tU8 confidenceThr;
	tU8 wrongPiConsecutivesErrorCount;
    DABMW_storageStatusEnumTy newValue;
} DABMW_piDataTy;
...
typedef struct
{
	...
	tU8 DABMW_nbConsecutivePIErrorAllowed;
...
tVoid DABMW_RdsDataEvent (DABMW_storageSourceTy source, DABMW_eventTy event)
{
    ...
    if (DABMW_EVENT_FREQUENCY_TUNED == event)
    {
		...
		OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
				"DABMW_RdsDataEvent : New freq tuned received slot = %d\n", slot);
        ...
        if (DABMW_INVALID_SLOT != slot)
        {
			...
			DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount= 0;
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)
				OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
						"DABMW_RdsDataEvent : DABMW_EVENT_RDS_DISABLED received slot = %d\n", slot);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)    
...
tVoid DABMW_RdsMain (DABMW_storageSourceTy source, tU32 data, tU32 error_ratio)
{
    ...
    slot = ETAL_receiverGetRDSSlot((ETAL_HANDLE)source);
		...
		OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
				"DABMW_RdsMain: hReceiver %d, slot  = %d, data = 0x%x", source, slot, data);
#endif
#else
    slot = DABMW_RdsGetSlotFromSource (source);
#endif
...
tSInt DABMW_RdsDataInit (tVoid)
{
    ...
    for (cnt = 0; cnt < DABMW_RDS_SOURCE_MAX_NUM; cnt++)
    {
		...
		DABMW_rdsData[cnt].DABMW_nbConsecutivePIErrorAllowed = DABMW_RDS_DEFAULT_NB_PI_ERROR;
...
{
    tSInt res;
	tChar dataPtrTemp[64];
	memset(dataPtrTemp, 0x00, 64);
    
    // Send back the PI if it is available and verified
    if (DABMW_STORAGE_STATUS_IS_VERIFIED == DABMW_rdsData[slot].DABMW_rt.newValue)
    {
		
		OSAL_pvMemoryCopy ((tPVoid)dataPtrTemp, (tPVoid)&DABMW_rdsData[slot].DABMW_rt.rt[0], DABMW_rdsData[slot].DABMW_rt.rtLen);
        OSAL_pvMemoryCopy ((tPVoid)dataPtr, (tPVoid)dataPtrTemp, 64);
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_2)
	OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_2, TR_CLASS_APP_DABMW, 
						"DABMW_RdsFetchBlockData : slot = %d, group = %d, complete = %d, A %d, A_used %d, B %d, B_used %d, C %d, C_used %d, D %d, D_used %d,\n", 
						slot, group, DABMW_rdsData[slot].DABMW_blockInfo.blocksComplete,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_A,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_A_used,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_B,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_B_used,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_C,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_C_used,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_D,
						DABMW_rdsData[slot].DABMW_blockInfo.availBlock_D_used);

#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) ("DABMW_RdsFetchBlockData : slot = %d, group = %d, complete = %d, A %d, A_used %d, B %d, B_used %d, C %d, C_used %d, D %d, D_used %d,\n", 
...
static tVoid DABMW_RdsExtrData (tSInt slot, DABMW_RDS_mwAppTy app)
{
	...
	*/
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, PTY, TP and TA found, blocks %c - %c\n", slot, 'A', 'B');
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)    
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d PS found, blocks %c - %c - %c on group # %d\n", slot, 'A', 'B', 'D', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)    
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)
					OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
							"DABMW RDS: slot %d New label stored, %8s\n", slot, &DABMW_rdsData[slot].DABMW_ps.label[0]);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)  
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, TIME found, blocks %c - %c - %c on group # %d\n", slot, 'B', 'C', 'D', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)   
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, AF found, blocks %c - %c - %c on group # %d\n", slot, 'A', 'B', 'C', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)   
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, PS (RBDS) found, blocks %c - %c - %c - %c on group # %d\n", slot, 'A', 'B', 'C', 'D', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, RT 2A found, blocks %c - %c - %c - %c on group # %d\n", slot, 'A', 'B', 'C', 'D', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)
						OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
							"DABMW RDS: slot %d, New RT stored, %s\n", slot, &DABMW_rdsData[slot].DABMW_rt.rt[0]);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)   
			}
        ...
        (DABMW_GROUP_2B == DABMW_rdsData[slot].DABMW_realGroup) && true == DABMW_rdsData[slot].DABMW_blockInfo.availBlock_D)
    {
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_1)
        OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_1, TR_CLASS_APP_DABMW, 
            "DABMW RDS: slot %d, RT 2B found, blocks %c - %c - %c - %c on group # %d\n", slot, 'A', 'B', 'D', DABMW_rdsData[slot].DABMW_realGroup);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 
        ...
        {
            DABMW_rdsData[slot].DABMW_rt.lastSegNum = 63;
			DABMW_rdsData[slot].DABMW_rt.rtLen = 63;
        }
...
static tVoid DABMW_CleanUpData (tSInt slot)
{
    ...
    OSAL_pvMemorySet((tPVoid)DABMW_rdsData[slot].DABMW_af.af, 0x00, sizeof(DABMW_rdsData[slot].DABMW_af.af));
    DABMW_rdsData[slot].DABMW_af.afNum = 0;
    OSAL_pvMemorySet((tPVoid)DABMW_rdsData[slot].DABMW_af.afMethod, 0x00, sizeof(DABMW_rdsData[slot].DABMW_af.afMethod));
#endif // !CONFIG_ETALTML_HAVE_AMFMLANDSCAPE
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_2)
	OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_2, TR_CLASS_APP_DABMW, 
						"DABMW_RdsPi_Processing: rds_slot %d, A %d, A_used %d, B %d, B_used %d, C %d, C_used %d, D %d, D_used %d,\n", 
								slot,
								DABMW_rdsData[slot].DABMW_blockInfo.availBlock_A,
								DABMW_rdsData[slot].DABMW_blockInfo.availBlock_A_used,
								DABMW_rdsData[slot].DABMW_blockInfo.availBlock_B,
	...
	if (vl_PI_read)
		...
		*/
			...
			if (DABMW_rdsData[slot].DABMW_pi.value != DABMW_rdsData[slot].DABMW_pi.lastValid)
				...
				if (DABMW_STORAGE_STATUS_IS_EMPTY != DABMW_rdsData[slot].DABMW_pi.newValue)
					...
					OSALUTIL_s32TracePrintf (0, TR_LEVEL_SYSTEM, TR_CLASS_APP_DABMW, 
								"DABMW_RdsPi_Processing: rds_slot %d, New PI detected old 0x%04x, old_confidence_threshold %d, new PI 0x%04x, wrongPiConsecutivesErrorCount %d\n", 
								slot,
								DABMW_rdsData[slot].DABMW_pi.lastValid, 
								DABMW_rdsData[slot].DABMW_pi.confidenceThr,
								DABMW_rdsData[slot].DABMW_pi.value,
								DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 

					// specific processing to be more robust on error : 
					// 1 error may not change the PI value...
					// so if PI verified , do not consider direcly the error

					// increase error count
					DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount++;

					// decrease threshold
					if (DABMW_rdsData[slot].DABMW_pi.confidenceThr > 0)
					{
						DABMW_rdsData[slot].DABMW_pi.confidenceThr--;
					}

					// if we are above the number of allowed errors...
					// or if we are back with confidence threshold null
					//
					if (	(DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount > DABMW_rdsData[slot].DABMW_nbConsecutivePIErrorAllowed)
						|| 	(DABMW_rdsData[slot].DABMW_pi.confidenceThr == 0)
						)
						{
						// counter is null, or number of error too high... no confidence on prior PI
						// reset 
						//
						...
						{
						// still confident in prior PI
						// keep same status and confidence level
						// keep last valid as the last stored
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)
				OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
							"DABMW_RdsPi_Processing: rds_slot %d, New PI received 0x%04x, confidence_threshold = %d, status = %d, DABMW_criticalDataThr %d, wrongPiConsecutivesErrorCount %d, DABMW_nbConsecutivePIErrorAllowed %d\n", 
							slot,
							DABMW_rdsData[slot].DABMW_pi.value,
							DABMW_rdsData[slot].DABMW_pi.confidenceThr, 
							DABMW_rdsData[slot].DABMW_pi.newValue,
							DABMW_rdsData[slot].DABMW_criticalDataThr,
							DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount,
								DABMW_rdsData[slot].DABMW_nbConsecutivePIErrorAllowed);
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT)
				OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
						"DABMW_RdsPi_Processing: rds_slot %d, PI confirmed 0x%04x, confidence_threshold %d, DABMW_criticalDataThr %d\n", 
						slot,
						DABMW_rdsData[slot].DABMW_pi.value,
						DABMW_rdsData[slot].DABMW_pi.confidenceThr,
						DABMW_rdsData[slot].DABMW_criticalDataThr);
				...
				// increase confidence threshould, up to max.. ie threshold + 1
				if (DABMW_rdsData[slot].DABMW_pi.confidenceThr <= DABMW_rdsData[slot].DABMW_criticalDataThr)
					{
					DABMW_rdsData[slot].DABMW_pi.confidenceThr++;
					}

				// set back consecutive error to 0	
				DABMW_rdsData[slot].DABMW_pi.wrongPiConsecutivesErrorCount = 0;
		...
		if ((DABMW_rdsData[slot].DABMW_pi.confidenceThr >= DABMW_rdsData[slot].DABMW_criticalDataThr) 
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_SYSTEM)
				OSALUTIL_s32TracePrintf (0, TR_LEVEL_COMPONENT, TR_CLASS_APP_DABMW, 
						"DABMW_RdsPi_Processing: rds_slot %d, PI verified 0x%04x, confidence_threshold %d, DABMW_criticalDataThr %d, newValue %d\n", 
						slot,
						DABMW_rdsData[slot].DABMW_pi.value,
						DABMW_rdsData[slot].DABMW_pi.confidenceThr,
						DABMW_rdsData[slot].DABMW_criticalDataThr,
			...
			{
				if (DABMW_rdsData[slot].DABMW_pi.value != 0) 
				{
					DABMW_rdsData[slot].DABMW_pi.backupValue = DABMW_rdsData[slot].DABMW_pi.value;
				}
			}
			...
			{
			// PI error case : 
			// store the PI confirmation only if this is the right PI !!
			//
			if (DABMW_rdsData[slot].DABMW_pi.value == DABMW_rdsData[slot].DABMW_pi.backupValue)
			{
				// Ask for current frequency
				if (DABMW_INVALID_FREQUENCY != curFreq)
				{
...
#if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_USER_2)
		OSALUTIL_s32TracePrintf (0, TR_LEVEL_USER_2, TR_CLASS_APP_DABMW, 
							"DABMW_RdsPi_Processing: rds_slot %d, vl_PI_verified = %d\n", slot, vl_PI_verified);	
#endif // #if defined (CONFIG_ENABLE_CLASS_APP_DABMW) && (CONFIG_ENABLE_CLASS_APP_DABMW >= TR_LEVEL_COMPONENT) 

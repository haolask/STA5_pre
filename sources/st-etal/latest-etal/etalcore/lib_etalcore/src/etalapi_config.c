...
ETAL_STATUS ETAL_audioSelectInternal(ETAL_HANDLE hReceiver, EtalAudioSourceTy src)
{
	...
	switch (src)
	{
		...
		case ETAL_AUDIO_SOURCE_STAR_AMFM:
			 ...
			 */
#if defined(CONFIG_ETAL_SUPPORT_CMOST_STAR_T_CUT_EB) 
            starInputSrc = ETAL_STAR_BLENDING_STAR_ANALOG_AMFM_EARLY;
#else
		...
		default:
			ASSERT_ON_DEBUGGING(0);
#ifdef CONFIG_ETAL_SUPPORT_CMOST_STAR_T_CUT_EB
            starInputSrc = ETAL_STAR_BLENDING_STAR_ANALOG_AMFM_EARLY;
#else
            starInputSrc = ETAL_STAR_BLENDING_STAR_ANALOG_AMFM;
#endif
...
// Audio On Host Default action
#if (defined CONFIG_ETAL_SUPPORT_DCOP_MDR) && (defined CONFIG_ETAL_MDR_DABMW_ON_HOST)
	// Enable by default DAB notification for DAB receiver
	if (ETAL_receiverGetStandard(hReceiver) == ETAL_BCAST_STD_DAB)
	{
		ETAL_cmdSetDefaultActionForReceiver_DABMWoH(hReceiver);
	}
#endif
...
ETAL_STATUS ETAL_configDatapathInternal(ETAL_HANDLE *pDatapath, const EtalDataPathAttr *pDatapathAttr)
{
	...
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "INTERNAL->ETAL : ETAL_configDatapathInternal()");
    if((pDatapath != NULL)&&(pDatapathAttr != NULL))
    {
        ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "datPat: %x, datPatAtt: (recHan: %d, DatTyp: 0x%x)",
    ...
    ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "INTERNAL->ETAL : ETAL_configDatapathInternal(datPat: 0x%x) = %s", *pDatapath, ETAL_STATUS_toString(ret));

...
tBool ETAL_receiverGetInProgress(etalReceiverStatusTy *recvp, etalCmdSpecialTy cmd)
{
	...
	switch (cmd)
	{
		...
		case cmdSpecialExternalAlternateFrequencyRequestInProgress:
			res = recvp->isExternalAlternateFrequencyRequestInProgress;
			break;
		...
		case cmdSpecialExternalRDSSeekRequestInProgress:
			res = recvp->isExternalRDSSeekRequestInProgress;
			break;
#endif
...
tVoid ETAL_receiverSetSpecial(ETAL_HANDLE hReceiver, etalCmdSpecialTy cmd, etalCmdActionTy action)
{
	...
	switch (cmd)
	{
		...
		case cmdSpecialLearn:
			...
			break;
#endif // #ifdef CONFIG_ETAL_HAVE_ETALTML
		...
		case cmdSpecialExternalAlternateFrequencyRequestInProgress:
			recvp->isExternalAlternateFrequencyRequestInProgress = local_action;
			break;
		...
		case cmdSpecialExternalSeekRequestInProgress:
			...
			// Set or Suspend implicitly INFO TUNE event
			// During the seek because main point is end of seek ?
			// Resume because we are now on a frequency
		...
		case cmdSpecialExternalLearnRequestInProgress:
			...
			// Set or Suspend implicitly INFO TUNE event
			// Suspend During the learn because main point is end of learn ?
			// Resume because we are now on a frequency
			//
			recvp->isExternalTuneRequestInProgress = (local_action == cmdActionStart) ? cmdActionStop : cmdActionStart;
			ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_API, "Event info tune notification : %d (0:stopped, 1:started) on %d", recvp->isExternalTuneRequestInProgress, hReceiver);
		...
		case cmdSpecialExternalScanRequestInProgress:
			...
			// Set or Suspend implicitly INFO TUNE event
			// During the scan because main point is end of scan ?
			// Resume because we are now on a frequency
			recvp->isExternalTuneRequestInProgress = (local_action == cmdActionStart) ? cmdActionStop : cmdActionStart;
			ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_API, "Event info tune notification : %d (0:stopped, 1:started) on %d", recvp->isExternalTuneRequestInProgress, hReceiver);
			break;
...
#ifdef CONFIG_ETALTML_HAVE_RDS_STRATEGY
		case cmdSpecialExternalRDSSeekRequestInProgress:
			recvp->isExternalRDSSeekRequestInProgress = local_action;
			ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "Event RDS notification : %d (0:stopped, 1:started) on %d", recvp->isExternalRDSSeekRequestInProgress, hReceiver);
			// Set or Suspend implicitly INFO TUNE event
			// During the seek because main point is end of seek ?
			// Resume because we are now on a frequency
			recvp->isExternalTuneRequestInProgress = (local_action == cmdActionStart) ? cmdActionStop : cmdActionStart;
		...
		case cmdSpecialExternalSeamlessSwitchingRequestInProgress:
			...
			// Why should we suspend info tune during seamless switching ??
...
tVoid ETAL_receiverDatapathDestroy(ETAL_HANDLE hDatapath)
{
	...
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "INTERNAL->ETAL : ETAL_receiverDatapathDestroy(datPat: 0x%x)", hDatapath);

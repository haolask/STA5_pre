...
tSInt ETAL_InitCMOSTtoMDRInterface(tVoid)
{
	...
	if (!ETAL_initStatusIsTunerStatusError((tU32)tunerIndex))
	{
        ...
        if(ETAL_CmdTunerConfHDBlend_CMOST(hTuner, 22, 22, 0, 0, 0) != OSAL_OK)
        {
            return OSAL_ERROR;
        }
#endif
...
{
	// MTD may be configured with 2 STARs, so manage 1st and 2nd tuner independently of the tuner type
	...
	if (haveFirstTuner)
	{
        ...
        if(ETAL_CmdTunerConfHDBlend_CMOST(vl_first_hTuner, 22, 22, 0, 0, 0) != OSAL_OK)
        {
            return OSAL_ERROR;
        }
	}

...
#define ETAL_DABMW_GET_BOARD_TYPE(_d_)             	   ((tU32)(_d_)[10])
...
static tSInt ETAL_cmdMonitorAddFilters_MDR(ETAL_HANDLE hMonitor)
{
	...
	for (i = 0; i < total_filters; i++)
	{
		...
		if ((mon->requested.m_monitoredIndicators[i].m_MonitoredIndicator == EtalQualityIndicator_DabFicErrorRatio) ||
			(mon->requested.m_monitoredIndicators[i].m_MonitoredIndicator == EtalQualityIndicator_DabMscBer))
		{
		...
		{
			// Monitored by the CMOST or not monitored by the DCOP
			flt = ETAL_statusFilterGet(filter_index);
			if(flt != NULL)
			{
...
tSInt ETAL_extractPADDLPLUS_MDR(tU8 *data, tU32 len, etalPADDLPLUSTy *pad, tU8 *app)
{
	...
	*app = ((payload[payload_len-1] & 0xFF) != 0x10) ? DABMW_SECONDARY_AUDIO_DAB_APP : DABMW_MAIN_AUDIO_DAB_APP;
	...
	if(payload_len > 0)
	{
		/* Reset DL PLUS variable before updating */
	    memset(pad, 0x00, sizeof(etalPADDLPLUSTy));
	}
...
tSInt ETAL_cmdResetMonitorAndFilters_MDR(tVoid)
{
	...
	DABMWcmd_SetupEventNotification[1] = (tU8)((tU8)(DABMW_MAIN_AUDIO_DAB_APP) << 2);
    ...
    DABMWcmd_SetupEventNotification[1] = (tU8)((tU8)(DABMW_SECONDARY_AUDIO_DAB_APP) << 2);
	retval = ETAL_sendCommandTo_MDR(DABMWcmd_SetupEventNotification, sizeof(DABMWcmd_SetupEventNotification), &cstatus, FALSE, NULL, NULL);
	if (retval != OSAL_OK)
	{
		return OSAL_ERROR;
...
tSInt ETAL_cmdGetPADDLSEvent_MDR(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
	...
	if (retval != OSAL_OK)
	{
		// if error is duplicated command
		// it means this is already active
		// so consider this is ok
		// 
		if (DABMW_CMD_STATUS_ERR_DUPLICATED_AUTO == cstatus)
		{
			
			/* truncate to supported size */
			ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_CMD, 
							"ETAL_cmdGetPADDLSEvent_MDR Cmd duplicated, consider it is ok, rec %d, status = %d, action = %d\n",
							hReceiver, cstatus, cmd);
...
{
    tU8 DABMWcmd_GetSLI[]  = {0x94, 0x01, 0x00}; // lower command id byte set below
    tU16 cstatus = 0;
    tU32 resplen = 0;
    tU8 *resp;
    tSInt retval = OSAL_OK;

    ETAL_paramSetApplicationFromReceiver_MDR(DABMWcmd_GetSLI, hReceiver);
    if (cmd == cmdActionStop)
    {
        DABMWcmd_GetSLI[2] = (tU8)0x07;
    }
    else
    {
        DABMWcmd_GetSLI[2] = (tU8)0x06;
    }
    retval = ETAL_sendCommandTo_MDR(DABMWcmd_GetSLI, sizeof(DABMWcmd_GetSLI), &cstatus, FALSE, &resp, &resplen);
    if (retval != OSAL_OK)
    {
        ETAL_sendCommunicationErrorEvent(hReceiver, ETAL_commandStatusToEventErrStatus_MDR(cstatus), cstatus, resp, resplen);
        return OSAL_ERROR;
...
tSInt ETAL_cmdSetupEventNotification_MDR(ETAL_HANDLE hReceiver, tU16 eventBitMap)
{
    ...
    ETAL_paramSetApplicationFromReceiver_MDR(DABMWcmd_SetupEventNotification, hReceiver);
...
tSInt ETAL_cmdGetFirmwareVersion_MDR(tU32 *year, tU32 *month, tU32 *day, tU32 *major, tU32 *minor, tU32 *internal, tU32 *build, tU32 *boardType)
{
	...
	else
	{
		...
		*boardType = ETAL_DABMW_GET_BOARD_TYPE(payload);
	}
	return OSAL_OK;

								...
								else if (type == ETAL_SERVSEL_SUBF_APPEND)
								{
									...
									if (ETAL_receiverGetSubch_MDR(hReceiver, &old_subch) == OSAL_OK)
									{
										if ((old_subch != ETAL_INVALID_SUBCH_ID) && (old_subch != subch))
										{
											...
											retval_2 = ETAL_cmdServiceSelect_MDR(hDatapath, hReceiver, ETAL_SERVSEL_MODE_DAB_SUBCH, ETAL_SERVSEL_SUBF_REMOVE, (tU32)UEId, (tU32)service, (tU32)sc, (tU32)old_subch, &no_data);

											if (retval_2 != OSAL_OK)
											{
												if ((retval_2 == OSAL_ERROR) && no_data)
												{
													ret = ETAL_RET_NO_DATA;
												}
												else
												...
												// not that it is may fail here if a tune to new station was done !!
												//								
												ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_API, "ETAL_serviceSelectInternal_MDR : service removal failed retval_2 %d, no_data %d", 
															retval_2, no_data);
											}

											// consider ret ok even if removal fails
											ret = ETAL_RET_SUCCESS;
											
										}

										// remove also in channel selection
										if (add_api_ms_notify_channel_selection(recvp->MDRConfig.application, INTERFACEREF_AUD_OUT, 0, audio_mode, old_subch, 0) != OSAL_OK)
										{
											ret = ETAL_RET_ERROR;
										}
			...
			else
			{
				if (ret == ETAL_RET_SUCCESS)
				{
					...
					if (ret == ETAL_RET_SUCCESS)
					{
						...
						// Init the XPAD data
						//
						ETAL_cmdSetServiceSelectionInformation_DABMWoH(hReceiver, mode, ETAL_SERVSEL_SUBF_SET, UEId, service, sc, subch);

...
tSInt ETAL_statusInitLock(tBool power_up)
{
    ...
    {
        if(ETAL_DABSeekTaskInit(i) != OSAL_OK)
        {
            retval = OSAL_ERROR;
            goto exit;
        }
...
tSInt ETAL_statusDeinitLock(tBool power_down)
{
    ...
    for (i = 0; i < ETAL_MAX_RECEIVERS; i++)
    {
        if (ETAL_DABSeekTaskDeinit(i) != OSAL_OK)
        {
            ret = OSAL_ERROR;
        }
    }
...
static ETAL_HANDLE ETAL_statusAddMonitorHandle(tVoid)
{
	...
	for (i = 0; i < ETAL_MAX_MONITORS; i++)
	{
		if (!etalStatus.monitors[i].isValid)
		{
			...
			etalStatus.monitors[i].isMonitorActiveForTimers = FALSE;
#endif
...
tSInt ETAL_statusStartRDSIrq(ETAL_HANDLE hReceiver)
{
    ...
    if (ETAL_receiverIsRDSCapable(hReceiver) == TRUE)
    {
		...
		ETAL_control_IrqRds_StartEvent();
...
tBool ETAL_statusGetRDSIrqhReceiver(ETAL_HANDLE tunerId, etalChannelTy channel, ETAL_HANDLE *phReceiver)
{
    ...
    for (receiver_index = 0; receiver_index < ETAL_MAX_RECEIVERS; receiver_index++)
    {
		...
		// check receiver is IRQ one
		if (etalStatus.hReceiverRDSIRQ[receiver_index] == hReceiver)
		{
			// check the tuner ID is correct one
			if ((ETAL_receiverGetTunerId(hReceiver, &l_hTuner) == OSAL_OK) && (l_hTuner == tunerId))
			{
				// check the channel
				// we should also support the VPA case
	            // channel FOREGROUND INPUT is searched, but  channel cannot be FOREGROUND or BOTH 
	            if (ETAL_receiverGetChannel(hReceiver, &hchannel) == OSAL_OK)
	            {
	            	// either the channel match, or Foreground requested and channel set to both for VPA
	            	//
	            	if ((hchannel == channel) 
						|| ((ETAL_CHN_FOREGROUND == channel) && (ETAL_CHN_BOTH == hchannel)))
					...
					{
						// not matching channel : continue
						//
					}
	            }
				else
				{
					// error in the get channel
					
				}
			}
			else
			{
				// not the correct Tuner Id ==> continue
			}
		}
		else
		{
			// not an IRQ receiver, continue
		}
    }
...
tSInt ETAL_statusCreateModifyMonitor(ETAL_HANDLE* pMonitor, const EtalBcastQualityMonitorAttr* pMonitorAttr)
{
	...
	if (retval == OSAL_OK)
	{
		...
		{
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT		
			// monitor already exist
			// we need to cancel prior timers 
			pmon = ETAL_statusGetMonitor(*pMonitor);
			pmon->isMonitorActiveForTimers = FALSE;
			ETAL_control_PeriodicMonitorStopEvent();
			// wait some ms to be sure the cancel is processed
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT
		ETAL_control_PeriodicMonitorStartEvent();
#endif
...
tSInt ETAL_statusDestroyMonitor(ETAL_HANDLE hMonitor)
{
	...
	if (retval == OSAL_OK)
	{
			...
			// monitor  exist
			// we need to cancel prior timers 
			// reset all monitors attributes and send stop event
			//
			// Request a stop
			pmon->isMonitorActiveForTimers = FALSE;
			ETAL_control_PeriodicMonitorStopEvent();

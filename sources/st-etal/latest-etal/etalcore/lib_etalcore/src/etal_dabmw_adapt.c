...
#include "pad_mngr.h"
#include "dabmw_common.h"

// Operation status type
typedef enum
{
    DABMW_OP_IDLE               = 0,
    DABMW_OP_IN_PROGRESS        = 1,
    DABMW_OP_ENDED_CORRECTLY    = 2,
    DABMW_OP_ENDED_FOR_TIMEOUT  = 3,
    DABMW_OP_ENDED_IN_ERROR     = 4,
    DABMW_OP_ENDED_TUNE_ZERO    = 5
} DABMW_operationStatusEnumTy;


// from system_status.h needed interface
// #include "system_status.h"
extern tVoid DABMW_SetSendMOTToDataChannel(DABMW_mwAppTy app, tU32 bitmap);
extern tVoid DABMW_SetStopMOTToDataChannel(DABMW_mwAppTy app, tU32 bitmap);
extern tSInt DABMW_SetApplicationService (DABMW_mwAppTy app, tU32 id, tU8 subChid, tU8 subFnct,tBool dataPortEnabled, tBool audioPortEnabled, tBool is_audio_channel);
extern tSInt DABMW_ServiceSelect(DABMW_mwAppTy app, tU8 selectionType, tU8 subFnct,
                          tU32 ensembleId, tU32 id, tU8 serviceComponentIndex,
                          tBool audioPortEnabled, tBool dataPortEnabled, tBool isInternalTune);
extern tSInt DABMW_SetAppTableSourceId (DABMW_mwAppTy app,  OSAL_tIODescriptor id);

extern tSInt DABMW_SetTunerStatus (OSAL_tIODescriptor id, DABMW_operationStatusEnumTy status, tU32 frequency, tU32 timeout);
extern tVoid DABMW_ClearApplicationSyncStatusBySourceId (OSAL_tIODescriptor id);
extern tSInt DABMW_SetApplication (DABMW_mwAppTy app, tU16 ensemble, tU32 frequency, tU32 service, tPU8 additionalParametersPtr);
extern tVoid DABMW_MOT_Tune(DABMW_mwAppTy app);
extern tVoid DABMW_EpgStorageReset(DABMW_mwAppTy app);
extern tVoid DABMW_SetApplicationConfirmedFrequencyBySourceId (OSAL_tIODescriptor id, tU32 frequency);
extern tVoid DABMW_SetApplicationSyncStatusBySourceId (OSAL_tIODescriptor id, tU8 sync);
extern tSInt DABMW_ParseFic_AudiOnHost (tPU8 dataPtr, OSAL_tIODescriptor id, tU32 len);


#define DABMW_DATACHANNEL_DATAGROUP_HEADER_SIZE 5


static tVoid ETAL_PADDLS_periodic_callback(ETAL_HANDLE hGeneric);
static tVoid  ETAL_MdrHostDataCallback(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context);
static tSInt ETAL_cmdSetDabTuneInformation_DABMWoH(ETAL_HANDLE hReceiver, tU32 vl_frequency, tU32 vI_sync);
static tVoid  ETAL_MdrHostFicCallback_DABMWoH(tU8* pBuffer, tU32 dwActualBufferSize, EtalDataBlockStatusTy *status, void* pvContext);
static tVoid  ETAL_MdrTuneCallback_DABMWoH(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context);
...
static tSInt ETAL_translateParameters_MDR(etalMonitorTy *monp, tU8 index, tU16 *offp, tU8 *lenp, tU32 *thminp, tU32 *thmaxp, tU16 *skipp)
{
	...
	switch (reqp->m_MonitoredIndicator)
	{
		...
		case EtalQualityIndicator_DabAudioBerLevel:
			// Indicator not monitored by the DCOP
			*offp = 0;
			*lenp = (tU8)0;
			break;
...
static tVoid ETAL_PADDLS_periodic_callback(ETAL_HANDLE hGeneric)
{
	...
	tU8 charset;
	etalPADDLSTy pad;
	ETAL_HANDLE vl_hDatapath = ETAL_INVALID_HANDLE;
	...
	dataLen = DABMW_GetPad (DABMW_DATATPE_IS_DAB_DLS, &charset, (tVoid**)&dataPtr, false);
	...
	else
	{
		...
		// Post the PAD on the data datapath if exists
		vl_hDatapath = ETAL_receiverGetDatapathFromDataType(hGeneric, ETAL_DATA_TYPE_DAB_DLS);
		if (vl_hDatapath != ETAL_INVALID_HANDLE)
		{
			ETAL_datahandlerInvoke(ETAL_COMM_DATA_CALLBACK_HANDLER, vl_hDatapath, (tVoid *)&pad, sizeof(etalPADDLSTy), NULL);
		}
	}
 ...
 *
 **************************/
tSInt ETAL_cmdGetPADDLSevent_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
	tSInt retval = OSAL_OK;
	...
	if (cmd == cmdActionStart)
	{
		...
		if (ETAL_intCbIsRegisteredPeriodic(&ETAL_PADDLS_periodic_callback, hReceiver) == FALSE)
		{
			if (ETAL_intCbRegisterPeriodic(&ETAL_PADDLS_periodic_callback, hReceiver, ETAL_DABMWOH_PAD_PERIOD) != ETAL_RET_SUCCESS)
			{
				retval = OSAL_ERROR;
				goto exit;
			}
	...
	else if (cmd == cmdActionStop)
	{
		...
		if (ETAL_intCbDeregisterPeriodic(&ETAL_PADDLS_periodic_callback, hReceiver) != ETAL_RET_SUCCESS)
		{
			retval = OSAL_ERROR;
			goto exit;
		}
...
/***************************
 *
 * ETAL_MdrHostDataCallback
 *
 **************************/
static tVoid  ETAL_MdrHostDataCallback(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context)
{
	tU32 vl_ExtractedPayloadSize;
	DABMWDataServiceType vl_type;
	EtalDataServiceType vl_etalType = ETAL_DATASERV_TYPE_NONE;
	tU8 vl_app;
	ETAL_HANDLE vl_hDatapath = ETAL_INVALID_HANDLE;
	ETAL_HANDLE vl_hReceiver;
	DABMW_HostDataComm_EnqueueParamTy *pl_MdrHostData;
    DABMW_DataCommModeTy vl_mode;
	tVoid *pl_context;
	tU8 *pl_extractedPayload, *pl_tmp_payload;
	static tBool vl_warning_issued = FALSE, vl_warning2_issued = FALSE, vl_warning3_issued = FALSE;
	
	/*
	 * incoming data are structured
	 * field indicating the data information
	 * an header which describer the data as per described in DABMW doc
	 * ie 27.6.1	Raw MOT Object payload format
	 * or 27.6.2	Raw DataGroup Object payload format
	 * the real payload
	 */
	pl_MdrHostData = (DABMW_HostDataComm_EnqueueParamTy *)param;
	
	vl_app = pl_MdrHostData->app;
	vl_hReceiver = ETAL_receiverSearchFromApplication(vl_app);
	vl_mode = pl_MdrHostData->mode;
	...
	switch (vl_mode)
	{
		case DABMW_DATACHANNEL_MSGTYPE_DECODED:
			vl_type = pl_MdrHostData->type;
			vl_hDatapath = ETAL_receiverGetDatapathFromDABType(vl_hReceiver, vl_type, &pl_context);
			// convert to correct ETAL TYPE
			vl_etalType = ETAL_receiverConvertDABMWDataServiceTypeToInternal(vl_type);
			...
			switch (vl_type)
			{
				...
				case DABMW_DATACHANNEL_DECODED_TYPE_SLS_XPAD:
					// in that case, the data are directly available 
					// we do not care of the optionnal header information but send the data raw
					// 
					pl_extractedPayload = pl_MdrHostData->data;
					vl_ExtractedPayloadSize = pl_MdrHostData->dataSize;

					ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback: Mot Extraction , DATASERV type (0x%x), dataSize = %d", 
						vl_etalType, vl_ExtractedPayloadSize);

					break;

				case DABMW_DATACHANNEL_DECODED_TYPE_TPEG_RAW:
					// in that case, the data are directly available 
					// for now, we do not care of the optionnal header information but send the data raw
					// However, should not we care of the data header ???
					pl_extractedPayload = pl_MdrHostData->data;
					vl_ExtractedPayloadSize = pl_MdrHostData->dataSize;
					
					ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback: Tpeg Raw Extraction , DATASERV type (0x%x), dataSize = %d", 
										vl_etalType, vl_ExtractedPayloadSize);
				...
				case DABMW_DATACHANNEL_DECODED_TYPE_JML_OBJ:
					...
					*/
					...
					ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : DATASERV type (0x%x) len %d", vl_type, vl_ExtractedPayloadSize);
					break;

				default:
					// TODO implement other decoders
					if (!vl_warning_issued)
					{
						ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : Unsupported DECODED DATA TYPE (0x%x)", vl_type);
						vl_warning_issued = TRUE;
					}
			...
			pl_tmp_payload = (tU8 *)OSAL_pvMemoryAllocate(vl_ExtractedPayloadSize+4);
			if (pl_tmp_payload == NULL)
			{
				ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : allocation failed)", vl_type);
				goto exit;
			}
			OSAL_pvMemoryCopy((tVoid *)pl_tmp_payload, (tPCVoid)&vl_etalType, 4);
			OSAL_pvMemoryCopy((tVoid *)(pl_tmp_payload+4), (tPCVoid)pl_extractedPayload, vl_ExtractedPayloadSize);
			pl_extractedPayload = pl_tmp_payload;
			vl_ExtractedPayloadSize += 4;
		...
		case DABMW_DATACHANNEL_MSGTYPE_FIC:
			...
			vl_ExtractedPayloadSize = pl_MdrHostData->dataSize;
			pl_extractedPayload = pl_MdrHostData->data;
			vl_hDatapath = ETAL_receiverGetDatapathFromDataType(vl_hReceiver, ETAL_DATA_TYPE_DAB_FIC);
			break;

		case DABMW_DATACHANNEL_MSGTYPE_RAW:
			vl_ExtractedPayloadSize = pl_MdrHostData->dataSize; // TODO: WARNING assumes only one Network Packet per buffer, not true if sending the whole subch
			pl_extractedPayload = pl_MdrHostData->data;
			vl_hDatapath = ETAL_receiverGetDatapathFromDataType(vl_hReceiver, ETAL_DATA_TYPE_DAB_DATA_RAW);
			break;

		case DABMW_DATACHANNEL_MSGTYPE_AUDIO:
			vl_ExtractedPayloadSize = pl_MdrHostData->dataSize;
			pl_extractedPayload = pl_MdrHostData->data;
			vl_hDatapath = ETAL_receiverGetDatapathFromDataType(vl_hReceiver, ETAL_DATA_TYPE_DAB_AUDIO_RAW);
			break;

		default:
			if (!vl_warning2_issued)
			{
				ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : Unrecognized binary payload format vl_mode %d", vl_mode);
				vl_warning2_issued = TRUE;
			}
			break;
	}
	...
	if (vl_hDatapath != ETAL_INVALID_HANDLE)
	{
		ETAL_datahandlerInvoke(ETAL_COMM_DATA_CALLBACK_HANDLER, vl_hDatapath, (tVoid *)pl_extractedPayload, vl_ExtractedPayloadSize, NULL);
	}
	else if (!vl_warning3_issued)
	{
		ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : Undefined callback for DATA LUN, data type = %d", vl_mode);
		if (DABMW_DATACHANNEL_MSGTYPE_DECODED == vl_type)
		{
			ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : Undefined callback for DATA LUN, data type = %d, type %d", vl_mode, vl_type);
		}
		else
		{
			ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostDataCallback : Undefined callback for DATA LUN, data type = %d", vl_type);
		}
		vl_warning3_issued = TRUE;
	}
...
exit:
	...
	if ((DABMW_DATACHANNEL_MSGTYPE_DECODED == vl_type)
		&& (pl_tmp_payload != NULL))
	{
		OSAL_vMemoryFree((tVoid *)pl_tmp_payload);
	}
	
	return;
}

// Proc to enable the data services
/***************************
 *
 * ETAL_StartStopMOT_DABMWoH
 *
 **************************/
tSInt ETAL_cmdStartStopMOT_DABMWoH(ETAL_HANDLE hReceiver, etalMOTserviceTy service, etalCmdActionTy action)
{
	tSInt retval = OSAL_OK;
	ETAL_STATUS vl_etalStatus = ETAL_RET_SUCCESS;
	etalReceiverStatusTy *recvp = NULL;
	DABMW_mwAppTy vl_app;

	recvp = ETAL_receiverGet(hReceiver);

	// start by callback registering if not already done
	if (ETAL_intCbIsRegistered(callAtMDRMotReception, ETAL_MdrHostDataCallback, ETAL_INVALID_HANDLE) == FALSE)
    {
	   	vl_etalStatus = ETAL_intCbRegister(callAtMDRMotReception, ETAL_MdrHostDataCallback, ETAL_INVALID_HANDLE, ETAL_INTCB_CONTEXT_UNUSED);

		if (ETAL_RET_SUCCESS != vl_etalStatus)
		{
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETAL_cmdStartStopMOT_DABMWoH: call back registration error %d", vl_etalStatus);
			retval = OSAL_ERROR;			
			goto exit;
		}
	}

	// we assume all is valid here
	recvp = ETAL_receiverGet(hReceiver);

	if (NULL != recvp)
	{
		// get the app
		vl_app = recvp->MDRConfig.application;

		if (action == cmdActionStart)
		{
			// Control the DAB MW to activate the MOT
			DABMW_SetSendMOTToDataChannel(vl_app, service);
		}
		else
		{
			// Control the DAB MW to deactivate the MOT
			DABMW_SetStopMOTToDataChannel(vl_app, service);
		}
	}
	else
	{
		ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETAL_cmdStartStopMOT_DABMWoH: invalid receiver %d", hReceiver);
		retval = OSAL_ERROR;			
		goto exit;
	}
...
/***************************
 *
 * ETAL_cmdGetTPEGRAW_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetTPEGRAW_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
...
/***************************
 *
 * ETAL_cmdGetEPGBIN_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetEPGBIN_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetEPGSRV_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetEPGSRV_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd, tU8 ecc, tU16 eid)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetTPEGSNI_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetTPEGSNI_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetSLI_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetSLI_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
    tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetEPGPRG_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetEPGPRG_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd, tU8 ecc, tU16 eid, tU32 sid)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetEPGLogo_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetEPGLogo_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd, tU8 ecc, tU16 eid, tU32 sid, tU8 logoType)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetJMLOBJ_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetJMLOBJ_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd, tU16 objectId)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdGetPADDLPlusevent_DABMWoH
 *
 **************************/
tSInt ETAL_cmdGetPADDLPlusEvent_DABMWoH(ETAL_HANDLE hReceiver, etalCmdActionTy cmd)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdSetPADDLPlusEvent_DABMWoH
 *
 **************************/
tSInt ETAL_cmdSetPADDLPlusEvent_DABMWoH(ETAL_HANDLE hReceiver)
{
	tSInt retval = OSAL_OK;
...
/***************************
 *
 * ETAL_cmdSetServiceSelectionInformation_DABMWoH
 *
 **************************/
tSInt ETAL_cmdSetServiceSelectionInformation_DABMWoH(ETAL_HANDLE hReceiver, EtalServiceSelectMode mode,  EtalServiceSelectSubFunction type, tU32 UEId, tU32 service, tSInt sc, tSInt subch)
{
	tSInt retval = OSAL_OK;
	etalReceiverStatusTy *recvp;
	DABMW_mwAppTy app;
	...
	if(recvp != NULL)
	{
		...
		retval = DABMW_ServiceSelect(app,										   // Use parameter passed
										 mode,				 // Mode is 0
										 type,				// Sub-function is 5
										 UEId,		 // Ensemble  
										 service,		// Service ID 		  
										 DABMW_INVALID_DATA_BYTE,						 // Not used for this kind of selection
										 true,											 // Audio port is enabled
										 false, 										 // Data port is disabled
...
/***************************
 *
 * ETAL_MdrHostFicCallback_DABMWoH
 *
 **************************/
static tVoid  ETAL_MdrHostFicCallback_DABMWoH(tU8* pBuffer, tU32 dwActualBufferSize, EtalDataBlockStatusTy *status, void* pvContext)

{
	...
	//ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_COMM, "ETAL_MdrHostFicCallback_DABMWoH: app = %d, Size = %d", app, dwActualBufferSize);


	DABMW_ParseFic_AudiOnHost(pBuffer, app, dwActualBufferSize);
...
static tVoid  ETAL_MdrTuneCallback_DABMWoH(ETAL_HANDLE hGeneric, void *param, tU32 param_len, tU32 context)
{
	...
	{
		if (pl_TuneStatus->m_syncInternal == ETAL_TUNESTATUS_SYNCMASK_DAB_FOUND)
		{
			// this means sync reached
			vl_syncMask = ETAL_TUNESTATUS_SYNCMASK_DAB_FOUND|ETAL_TUNESTATUS_SYNCMASK_DAB_SYNC|ETAL_TUNESTATUS_SYNCMASK_DAB_MCI;
		}
		
		ETAL_cmdSetDabTuneInformation_DABMWoH(hGeneric, pl_TuneStatus->m_Frequency, vl_syncMask);
	}
	else
	{
...
/***************************
 *
 * ETAL_cmdSetDefaultActionForReceiver_DABMWoH
 *
 **************************/
tSInt ETAL_cmdSetDefaultActionForReceiver_DABMWoH(ETAL_HANDLE hReceiver)
{
	tSInt retval = OSAL_OK;
	...
	// enable FIC and create a datapath for FIC
	etal_get_fic(hReceiver);
	...
	datapathAttr.m_sink.m_CbProcessBlock = ETAL_MdrHostFicCallback_DABMWoH;
	...
	// start by callback registering if not already done
	if (ETAL_intCbIsRegistered(callAtTuneFrequency, ETAL_MdrTuneCallback_DABMWoH, ETAL_INVALID_HANDLE) == FALSE)
    {
	   	vl_etalStatus = ETAL_intCbRegister(callAtTuneFrequency, ETAL_MdrTuneCallback_DABMWoH, ETAL_INVALID_HANDLE, ETAL_INTCB_CONTEXT_UNUSED);

		if (ETAL_RET_SUCCESS != vl_etalStatus)
		{
			ETAL_tracePrintError(TR_CLASS_APP_ETAL_COMM, "ETAL_cmdSetDefaultActionForReceiver_DABMWoH: call back registration error %d", vl_etalStatus);
			retval = OSAL_ERROR;			
			goto exit;
		}
	}	

	// set the Osal Id => simulate = app
	//
	DABMW_SetAppTableSourceId (app,  (OSAL_tIODescriptor) app);
...
/***************************
 *
 * ETAL_cmdSetDabTuneInformation_DABMWoH
 *
 **************************/
static tSInt ETAL_cmdSetDabTuneInformation_DABMWoH(ETAL_HANDLE hReceiver, tU32 vl_frequency, tU32 vI_sync)
{
	...
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL, "ETAL_cmdSetDabTuneInformation_DABMWoH: hReceiver %d, vl_frequency %d, vI_sync %d", hReceiver,vl_frequency, vI_sync);
	...
	// Set status
	DABMW_SetTunerStatus (app, DABMW_OP_IN_PROGRESS, vl_frequency, 0);

	// Clear the sync indication because we are tuning to a new ensemble
	DABMW_ClearApplicationSyncStatusBySourceId (app);

	// Set the data regarding the application with the new frequency and setting
	// to invalid the EID, it will be set to the proper value after the tune
	// process will be completed
	DABMW_SetApplication (app, DABMW_INVALID_EID, vl_frequency, 0xFFFFFFFF, NULL);

#if defined (CONFIG_DABMW_MOT_DECODER_ENABLE)
	 DABMW_MOT_Tune (app);
#endif // #if defined (CONFIG_DABMW_MOT_DECODER_ENABLE)
	     

#if (defined CONFIG_DABMW_EPG_ENABLED)
	 DABMW_EpgStorageReset (app);
#endif // #if (defined CONFIG_DABMW_EPG_ENABLED)

	// TODO : should be done only once tune but for not let's do it now
	// 
    // Store the information regarding the new frequency for this dab device
    DABMW_SetApplicationConfirmedFrequencyBySourceId (app, vl_frequency);
	// Set the applicatino table with the new sync status
	DABMW_SetApplicationSyncStatusBySourceId (app, vI_sync);
 ...
 *
 **************************/
tSInt ETAL_cmdGetFirmwareVersion_MDR(tU32 *year, tU32 *month, tU32 *day, tU32 *major, tU32 *minor, tU32 *internal, tU32 *build, tU32 *boardType)
{

...
// In IRQ case we accept a tuner in TKERNEL CASE
	...
	{
		// this is possible in cross case with CPU_IMPROVEMENTS
		// a stop_rds but still timer not stopped.
	...
	if (len >= 6)
	{
		...
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "ETAL_RDSRawPeriodicFunc: hReceiver = %d, data_rnr 0x%x, len = %d", hReceiver, data_rnr, len);
...
#ifdef CONFIG_ETAL_SUPPORT_CMOST_STAR
	ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "APP->ETAL: etal_start_RDS(rec: %d, forFasPi: %d, numPi: %d, mode: %d, errthresh: %u)", hReceiver, forceFastPi, numPi, mode, errthresh);
	...
	else
	{
		...
		else
		{
			...
			if (ret == ETAL_RET_SUCCESS)
			{
				...
				RDSAttr.rdsRbdsMode = (EtalRDSRBDSModeTy)(mode & ~ETAL_RBS_GROUP_OUTPUT_MODE);
				RDSAttr.groupouten = (tBool)(((tU32)mode & (tU32)ETAL_RBS_GROUP_OUTPUT_MODE) == (tU32)ETAL_RBS_GROUP_OUTPUT_MODE);
				RDSAttr.errthresh = errthresh;

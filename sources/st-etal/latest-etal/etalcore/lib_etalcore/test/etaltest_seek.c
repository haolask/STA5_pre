...
#define ETAL_TEST_DAB_SEEK_BAND_TIME         (40 * ETAL_TEST_ONE_SECOND)
// #define ETAL_TEST_DAB_SEEK_BAND_TIME         (25 * ETAL_TEST_ONE_SECOND)
...
static tSInt etalTestSeekDAB(ETAL_HANDLE hReceiver)
{
	...
	if(hReceiver == handledab)
	{
		...
		for(i=0; i<2; i++)
		{
			...
			// add a step to verify seek continue is rejected while seek is running
			etalTestPrintNormal("Wait some time (%d sec), handle %d", ETAL_TEST_ONE_SECOND, hReceiver);
			OSAL_s32ThreadWait(ETAL_TEST_ONE_SECOND);

			etalTestPrintNormal("* Check that continue autoseek is rejected during seek, handle %d", hReceiver);
			...
			// In that configuration (Unmuted) the seek continue will be forbidden 
			// either because the seek process is on-going, either because the seek is finished
			//
			if ((ret = etal_autoseek_continue(hReceiver)) != ETAL_RET_ERROR)
			{
				etalTestPrintError("etal_autoseek_continue DAB (%s, %d), handle %d", ETAL_STATUS_toString(ret), ret, hReceiver);
		...
		for(i=0; i<2; i++)
		{
			...
			// In that configuration (Muted) the seek continue is allowed to continue seek 
			// BUT ONLY if the SEEK is finished for sure : freq found or full cycle reached
			//
			...
			// in theory here the fill band seek is reached !! 
			// we could check : 
			// 
			if ((etalTestSeekEventCountDAB.FREQ_FOUND == 0) && (etalTestSeekEventCountDAB.FULL_CYCLE_REACHED == 0))
			{
				// frequency not found, and no full cyle reached
				// seek not ended !!
				etalTestPrintError("Seek not ended within delay !! handle %d", hReceiver);
				return OSAL_ERROR;
			}	
				
			etalTestPrintNormal("* Continue autoseek number 1, handle %d", hReceiver);

			// reset information
			etalTestSeekEventCountDAB.FREQ_FOUND = 0;
			etalTestSeekEventCountDAB.FULL_CYCLE_REACHED = 0;
			...
			etalTestPrintNormal("* Continue autoseek number 2, handle %d", hReceiver);
	...
	else
	{
		...
		for(i=0; i<2; i++)
		{
			...
			if ((ret = etal_autoseek_continue(hReceiver)) != ETAL_RET_ERROR)
			{
				etalTestPrintError("FAILED etal_autoseek_continue return ok (%s, %d), handle %d", ETAL_STATUS_toString(ret), ret, hReceiver);
				return OSAL_ERROR;
			}
			...
			if ((ret = etal_autoseek_continue(hReceiver)) != ETAL_RET_ERROR)
			{
				etalTestPrintError("FAILED etal_autoseek_continue (%s, %d), handle %d", ETAL_STATUS_toString(ret), ret, hReceiver);
				return OSAL_ERROR;
			}
		...
		for(i=0; i<2; i++)
		{
			...
			// In that configuration (Muted) the seek continue will be allowed 
			// but only if the seek is ended...
			//
			// in theory here the fill band seek is reached !! 
			// we could check : 
			// 
			if ((etalTestSeekEventCountDAB1_5.FREQ_FOUND == 0) && (etalTestSeekEventCountDAB1_5.FULL_CYCLE_REACHED == 0))
			{
				// frequency not found, and no full cyle reached
				// seek not ended !!
				etalTestPrintError("etalTestSeekEventCountDAB1_5 Seek not ended within delay !! handle %d", hReceiver);
				return OSAL_ERROR;
			}
				

			etalTestPrintNormal("* Continue autoseek num 1, handle %d", hReceiver);

			// reset info
			etalTestSeekEventCountDAB1_5.FREQ_FOUND = 0;
			etalTestSeekEventCountDAB1_5.FULL_CYCLE_REACHED = 0;
			...
			etalTestPrintNormal("* Continue autoseek num 2, handle %d", hReceiver);
...
static tSInt etalTestSeekAM(ETAL_HANDLE hReceiver)
{
    ...
    while (count < ETAL_TEST_SEEK_COUNT)
	{
		...
		// Reset the Freq Found information 
		// but not others : the counter are required for the test
		// because in serie
		//
		etalTestSeekEventCountAM.FREQ_FOUND = FALSE;
		etalTestSeekEventCountAM.FULL_CYCLE_REACHED = FALSE;
        ...
        if(mute == cmdAudioMuted)
        {
            ...
            etalTestPrintNormal("***********************************************");
 			...
 			// note that in present case info is received by a RESULT and no FINISH
 			//
        ...
        else
        {
            ...
            etalTestPrintNormal("***********************************************");
 			...
 			// wait until freq found or full cycle reached
 			// note that in present case info is received by a RESULT and a FINISH
 			//

			while (etalTestSeekEventCountAM.FINISHED == 0)
			{			
				OSAL_s32ThreadWait(ETAL_TEST_ONE_SECOND);
			}

...
tVoid etalTestPrintReportTestStart(tCString coszFormat, tU32 test_index)
{
	...
	etalTestPrintReport0("%02d: %s Test start", test_index, coszFormat);
}
...
tVoid etalTestPrintReportTestEnd(tCString coszFormat, tU32 test_index, tSInt retosal)
{
	...
	etalTestPrintReport0("%s%*s", etalTestReportString, pad, etalTestResult2String(res));
	etalTestReportString[0] = '\0';
}
...
tVoid etalTestPrintReportPassEnd(etalTestResultTy res)
{
	...
	etalTestPrintReport0(etalTestReportString);
	etalTestReportString[0] = '\0';
...
tVoid etalTestPrint(tU16 level, tCString coszFormat, ...)
{
	...
	// if this is a report 0 level => put an header info
	//
	if (level == ETAL_TEST_REPORT_LEVEL)
	{

		OSAL_s32NPrintFormat(szBuffer, sizeof(szBuffer), "TEST_REPORT_VERDICT:");
		header_len = OSAL_u32StringLength(szBuffer);
	}
	else if (level == ETAL_TEST_REPORT0_LEVEL)
	{

		OSAL_s32NPrintFormat(szBuffer, sizeof(szBuffer), "TEST_REPORT_INFO:");
		header_len = OSAL_u32StringLength(szBuffer);
	}
	/*
	...
	OSAL_VarArgStart(argList, coszFormat);
	OSAL_s32VarNPrintFormat(szBuffer+header_len, sizeof(szBuffer), coszFormat, argList);
	OSAL_VarArgEnd(argList);

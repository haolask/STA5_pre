...
// Why 2 ??
// We should be happy on live net especially with 1 Radio Text data callback at least
// especially here the test is doing a strict comparison so when we write 2 it means 3 received 
// 
#define GET_RADIOTEXT_FM_MIN               1
#define GET_RADIOTEXT_HD_MIN               1
...
static void etalTestRadiotextCallback(tU8* pBuffer, tU32 dwActualBufferSize, EtalDataBlockStatusTy *status, void* pvContext)
{
	...
	if (RadiotextCbBcastStd == ETAL_BCAST_STD_FM) 
	{
		...
		{
			etalTestPrintNormal("RadioTextCallback compared failed ");
		}

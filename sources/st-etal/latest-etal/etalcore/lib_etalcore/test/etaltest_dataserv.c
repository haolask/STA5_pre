...
#if  defined (CONFIG_APP_TEST_HDRADIO_FM) ||defined (CONFIG_APP_TEST_HDRADIO_AM) || defined (CONFIG_APP_TEST_FM) || defined (CONFIG_APP_TEST_AM)
typedef enum {
	DATA_SERV_HDPASS1_READ_PSD = 1,
	DATA_SERV_HDPASS1_VALIDATE_PSD,
	DATA_SERV_HDPASS2_PARAMETER_CHECK,
	DATA_SERV_HDPASS3_SETUP_PSD_LENGTH,
	DATA_SERV_NON_HD_PARAMETER_CHECK
} DataServTestTy;
#endif // CONFIG_APP_TEST_HDRADIO_FM ||CONFIG_APP_TEST_HDRADIO_AM || CONFIG_APP_TEST_FM || CONFIG_APP_TEST_AM
...
#if defined(CONFIG_APP_TEST_DATASERVICES_SLI) || defined(CONFIG_APP_TEST_DATASERVICES_SLS) || defined(CONFIG_APP_TEST_DATASERVICES_EPG) || defined(CONFIG_APP_TEST_DATASERVICES_JOURNALINE) || defined(CONFIG_APP_TEST_DATASERVICES_TPEG) ||	defined(CONFIG_APP_TEST_DATASERVICES_DLS) || defined(CONFIG_APP_TEST_DATASERVICES_DLPLUS)
...
#if defined(CONFIG_APP_TEST_DATASERVICES_SLI)
tSInt etalTestDataServices_SLI(void);
#endif //#if defined(CONFIG_APP_TEST_DATASERVICES_SLI)
...
#define ETAL_TEST_DATASERV_CMDLEN  64
...
#if defined(CONFIG_APP_TEST_DATASERVICES_SLI)
    static tU32 count_SLI = 0;
	tU32 SLILength, SLIFig6, SLIFig21, SLIFig24, SLIRDS, SLILAInfo, j;
    tU8 *SLIData;
#endif
	...
	switch((*DataServiceRawBuffer).m_dataType)
	{
        ...
        case ETAL_DATASERV_TYPE_SLI:
            /*
             * Writes the SLI content to files on the filesystem
             */
            etalTestPrintNormal("Data Service Linking Info callback received packet size %d", dwActualBufferSize);
            SLIData = &(DataServiceRawBuffer->m_data);
            SLILength = ETAL_utilityGetU24(SLIData, 0);
            etalTestPrintNormal("SLI length %d", SLILength);
            SLIFig6 = ETAL_utilityGetU16(SLIData, 3);
            etalTestPrintNormal("SLI FIG06 length %d", SLIFig6);
            if(SLIFig6 != 0)
            {
                for(j=0;j<SLIFig6;j++)
                {
                    etalTestPrintNormal("SLI FIG06[%d] = 0x%x", j, ETAL_utilityGetU8(SLIData, 3 + 3 + j));
                }
            }
            SLIFig21 = ETAL_utilityGetU16(SLIData, 5 + SLIFig6);
            etalTestPrintNormal("SLI FIG021 length %d", SLIFig21);
            if(SLIFig21 != 0)
            {
                for(j=0;j<SLIFig21;j++)
                {
                    etalTestPrintNormal("SLI FIG21[%d] = 0x%x", j, ETAL_utilityGetU8(SLIData, 5 + SLIFig6 + 2 + j));
                }
            }
            SLIFig24 = ETAL_utilityGetU16(SLIData, 7 + SLIFig6 + SLIFig21);
            etalTestPrintNormal("SLI FIG024 length %d", SLIFig24);
            if(SLIFig24 != 0)
            {
                for(j=0;j<SLIFig24;j++)
                {
                    etalTestPrintNormal("SLI FIG24[%d] = 0x%x", j, ETAL_utilityGetU8(SLIData, 7 + SLIFig6 + SLIFig21 + 2 + j));
                }
            }
            SLIRDS = ETAL_utilityGetU16(SLIData, 9 + SLIFig6 + SLIFig21 + SLIFig24);
            etalTestPrintNormal("SLI RDS length %d", SLIRDS);
            if(SLIRDS != 0)
            {
                for(j=0;j<SLIRDS;j++)
                {
                    etalTestPrintNormal("SLI RDS[%d] = 0x%x", j, ETAL_utilityGetU8(SLIData, 9 + SLIFig6 + SLIFig21 + SLIFig24 + 2 + j));
                }
            }
            SLILAInfo = ETAL_utilityGetU16(SLIData, 11 + SLIFig6 + SLIFig21 + SLIFig24 + SLIRDS);
            etalTestPrintNormal("SLI LA Info length %d", SLILAInfo);
            if(SLILAInfo != 0)
            {
                for(j=0;j<SLILAInfo;j++)
                {
                    etalTestPrintNormal("SLI LAI[%d] = 0x%x", j, ETAL_utilityGetU8(SLIData, 11 + SLIFig6 + SLIFig21 + SLIFig24 + SLIRDS + 2 + j));
                }
            }

            OSAL_s32NPrintFormat(name, (size_t)ETAL_TEST_DATASERV_NAMELEN, "SLI-%d.txt", count_SLI++);
            fp = fopen(name, "w");
            if (fp == NULL)
            {
                etalTestPrintError("opening file %s", name);
...
tSInt etalTestDataServices_EPG(void)
{
	 ...
	 */
	OSAL_s32ThreadWait(4 * ETAL_TEST_ONE_SECOND);
...
tSInt etalTestDataServices_TPEG(void)
{
    ...
    /*
     * disable TPEG SNI data service
     */
    if ((ret = etal_disable_data_service(handledab, ETAL_DATASERV_TYPE_TPEG_SNI)) != ETAL_RET_SUCCESS)
    {
        etalTestPrintError("etal_disable_data_service (%s, %d)", ETAL_STATUS_toString(ret), ret);
        etalTestPrintError("TPEG_SNI test FAILED");
        return OSAL_ERROR;
...
tSInt etalTestDataServices_SLI(void)
{
    ETAL_STATUS ret;
     ...
     etalTestPrintNormal("<---SLI Test start");
     /*
      * Assumes the FraunhoferIIS-ServiceFollowing-2010-02-25.eti
      */
     ...
     if (etalTestDoServiceSelectData(handledab, &hDatapath, ETAL_DAB_ETI9_UEID, ETAL_DAB_ETI9_SERVD1_SID, 1, ETAL_INVALID, ETAL_INVALID) != OSAL_OK)
     {
         etalTestPrintError("This test requires FraunhoferIIS-ServiceFollowing-2010-02-25.eti file");
         etalTestPrintError("Service Linking Info test FAILED");
         return OSAL_ERROR;
     }

     /*
      * Enables the SLI data services.
      */

     etalTestPrintNormal("<---Service Linking Info");

     enabledServices = 0;
     etalTestDataServ = 0;
     OSAL_pvMemorySet(&param, 0, sizeof(param));

     if ((ret = etal_enable_data_service(handledab, ETAL_DATASERV_TYPE_SLI, &enabledServices, param)) != ETAL_RET_SUCCESS)
     {
         etalTestPrintError("etal_enable_data_service (%s, %d)", ETAL_STATUS_toString(ret), ret);
         etalTestPrintError("Service Linking Info test FAILED");
         return OSAL_ERROR;
     ...
     OSAL_s32ThreadWait(50 * ETAL_TEST_ONE_SECOND);

     /*
      * disable TPEG RAW data service
      */
     if ((ret = etal_disable_data_service(handledab, ETAL_DATASERV_TYPE_SLI)) != ETAL_RET_SUCCESS)
     {
         etalTestPrintError("etal_disable_data_service (%s, %d)", ETAL_STATUS_toString(ret), ret);
         etalTestPrintError("Service Linking Info test FAILED");
         return OSAL_ERROR;
      ...
      * cleanup
      */
     if (etalTestUndoServiceSelectData(hDatapath, ETAL_DAB_ETI9_UEID, ETAL_DAB_ETI9_SERVD1_SID) != OSAL_OK)
     {
         etalTestPrintError("Service Linking Info test FAILED");
         return OSAL_ERROR;
     }
...
#define ETAL_TEST_DATASERV_NAMELEN 32
	...
	etalTestPrintReportPassStart(DATA_SERV_HDPASS1_READ_PSD, test_mode, "Read PSD");
	...
	else
	{
		...
		etalTestPrintReportPassStart(DATA_SERV_HDPASS1_VALIDATE_PSD, test_mode, "Validate PSD");
		/*
...
static tSInt etalTestDataServicesHDPass2(etalTestBroadcastTy test_mode, ETAL_HANDLE hReceiver, tBool *pass)
{
	...
	etalTestPrintReportPassStart(DATA_SERV_HDPASS2_PARAMETER_CHECK, test_mode, "Parameter checks");
...
static tSInt etalTestDataServicesHDPass3(etalTestBroadcastTy test_mode, ETAL_HANDLE hReceiver, tBool *pass)
{
	...
	etalTestPrintReportPassStart(DATA_SERV_HDPASS3_SETUP_PSD_LENGTH, test_mode, "Setup PSD Length");
...
static tSInt etalTestDataServicesNonHD(etalTestBroadcastTy test_mode, ETAL_HANDLE hReceiver, tBool *pass)
{
	...
	etalTestPrintReportPassStart(DATA_SERV_NON_HD_PARAMETER_CHECK, test_mode, "Check parameter");
...
static tSInt etalTestDataServices_internal(etalTestBroadcastTy test_mode, EtalDataServiceType serviceType, tBool *pass)
{
	...
	switch (test_mode)
	{
		case ETAL_TEST_MODE_DAB:
			...
			tune_mode = ETAL_TUNE_DAB2; // not used
	...
	if (test_mode == ETAL_TEST_MODE_DAB)
	{
        ...
        if(serviceType == ETAL_DATASERV_TYPE_SLI)
        {
            if (OSAL_OK != etalTestDataServices_SLI())
            {
                *pass = FALSE;
            }
        }
...
#ifdef CONFIG_APP_TEST_DATASERVICES_SLI
    if (OSAL_OK != etalTestDataServices_internal(ETAL_TEST_MODE_DAB, ETAL_DATASERV_TYPE_SLI, &pass))
    {
        return OSAL_ERROR;
    }
#endif

...
static tSInt etalTestDoRDSConfig(ETAL_HANDLE *hReceiver, tBool reconfigure, etalTestBroadcastTy mode, ETAL_HANDLE hFrontend)
{
	...
	if (hReceiver != NULL)
	{
		...
		if ((ret = etal_config_receiver(hReceiver, &attr)) != ETAL_RET_SUCCESS)
		{
			...
			// Audio path should already been correctly configured
#if 0
...
static tSInt etalTestGetRDSDatapath2(ETAL_HANDLE *hReceiver, etalTestBroadcastTy mode, ETAL_HANDLE *hReceiver2, tU8 mode2, tBool *pass)
{
	...
	if ((ret = etal_start_RDS(*hReceiver2, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_SUCCESS)
	{
...
static tSInt etalTestRDSInvalidParam(tBool *pass)
{
	...
	*/
	...
	if (pass1 == TRUE)
	{
		...
		if ((ret = etal_start_RDS(ETAL_INVALID_HANDLE, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_INVALID_HANDLE)
		{
		...
		if ((ret = etal_start_RDS(handlefm, TRUE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_PARAMETER_ERR)
		{
		...
		for (i = 16; i < 256; i++)
		{
			if ((ret = etal_start_RDS(handlefm, FALSE, (tU8)i, ETAL_RDS_MODE, 0)) != ETAL_RET_PARAMETER_ERR)
			{
		...
		if ((ret = etal_start_RDS(handlefm, FALSE, 0, (ETAL_RBDS_MODE + 3), 0)) != ETAL_RET_PARAMETER_ERR)
		{
	...
	*/
	...
	if (pass2 == TRUE)
	{
		 ...
		 */
		etalTestPrintNormal("* Start RDS with invalid forceFastPi");
		if ((ret = etal_start_RDS(handlehd, TRUE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_PARAMETER_ERR)
		{
		...
		for (i = 16; i < 256; i++)
		{
			if ((ret = etal_start_RDS(handlehd, FALSE, (tU8)i, ETAL_RDS_MODE, 0)) != ETAL_RET_PARAMETER_ERR)
			{
		...
		etalTestPrintNormal("* Start RDS with invalid mode");
		if ((ret = etal_start_RDS(handlehd, FALSE, 0, (ETAL_RBDS_MODE + 1), 0)) != ETAL_RET_PARAMETER_ERR)
		{
	...
	if (pass3 == TRUE)
	{
		...
		if ((ret = etal_start_RDS(handleam, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_INVALID_RECEIVER)
		{
	...
	if (pass4 == TRUE)
	{
		...
		if ((ret = etal_start_RDS(handledab, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_INVALID_RECEIVER)
		{
	...
	if (pass5 == TRUE)
	{
		...
		if ((ret = etal_start_RDS(handlehd, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_INVALID_RECEIVER)
		{
...
static tSInt etalTestGetRDSRawDatapath(ETAL_HANDLE *hReceiver, etalTestBroadcastTy mode, tBool *pass_out)
{
	...
	if ((ret = etal_start_RDS(*hReceiver, FALSE, 0, ETAL_RBDS_MODE, 0)) != ETAL_RET_SUCCESS)
	{
		pass = FALSE;
		etalTestPrintReportPassEnd(testFailed);
		etalTestPrintNormal("etal_start_RDS for %s (%s, %d)", standard, ETAL_STATUS_toString(ret), ret);
...
static tSInt etalTestGetRDSRawFastPIDatapath(ETAL_HANDLE *hReceiver, etalTestBroadcastTy mode, tBool *pass_out)
{
	...
	if ((ret = etal_start_RDS(*hReceiver, FALSE, 15, ETAL_RDS_MODE, 0)) != ETAL_RET_SUCCESS)
	{
		pass = FALSE;
		etalTestPrintReportPassEnd(testFailed);
		etalTestPrintNormal("etal_start_RDS for %s (%s, %d)", standard, ETAL_STATUS_toString(ret), ret);
	...
	if ((ret = etal_start_RDS(*hReceiver, TRUE, 1, ETAL_RDS_MODE, 0)) != ETAL_RET_SUCCESS)
	{
		pass = FALSE;
		etalTestPrintNormal("etal_start_RDS for %s (%s, %d)", standard, ETAL_STATUS_toString(ret), ret);
	}
...
static tSInt etalTestForegroundBackgroundFM(tBool *pass)
{
    ...
    if ((ret = etal_start_RDS(handlefm2, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_SUCCESS)
    {
...
tSInt etalTestGetRDS(void)
{
	...
	if ((ret = etal_start_RDS(handlehd, FALSE, 0, ETAL_RDS_MODE, 0)) != ETAL_RET_SUCCESS)
	{

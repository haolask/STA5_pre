...
#define ETAL_DAB_SEEK_STACK_SIZE           (16*4096)
...
typedef enum
{
    ...
    /*! Alternate frequency operation that requires sending feedback to the ETAL API user (e.g. events) */
    cmdSpecialExternalAlternateFrequencyRequestInProgress,
	...
	cmdSpecialExternalRDSSeekRequestInProgress,
 ...
 * 			2. identify the point in ETAL where the callback should be invoked (e.g. #ETAL_Report_Seek_Info)
...
typedef enum
{
	...
	/*! Use RBDS */
	//ETAL_RBDS_MODE = 1,
	/*! Use RBS block output mode */
	ETAL_RBS_BLOCK_OUTPUT_MODE = 0,
	/*! Use RBS group output mode */
	ETAL_RBS_GROUP_OUTPUT_MODE = 2
} etalRDSRBDSModeExtTy;
...
typedef struct
{
	...
	tBool               groupouten;             /* rds group output mode */
	/* RDS block error threshold: 6 (‘110’), the filter is ignored and all the blocks are stored in the buffer regardless
	   of the number of errors. Practical values for the threshold are in the interval 0..3, with 0 being the
	   most restrictive condition (toughest quality threshold), and 3 being the most permissive condition
	   (less demanding quality threshold)*/
	tU8                 errthresh;
} etalRDSAttr;
...
typedef enum
{
    ...
    ETAL_STAR_BLENDING_STAR_ANALOG_AMFM         = 0x01, /* Select STAR analog input (AM/FM) */
    ...
    ETAL_STAR_BLENDING_HD_ALIGN                 = 0x03, /* HD alignment mode (left = analog, right = digital) */
    ...
    ETAL_STAR_BLENDING_STAR_ANALOG_AMFM_EARLY   = 0x06  /* Select STAR analog input (early AM/FM) */
#endif
#endif
...
typedef struct
{
	...
	tBool			  isExternalAlternateFrequencyRequestInProgress;
...
#ifdef CONFIG_ETALTML_HAVE_RDS_STRATEGY
	tBool			  isExternalRDSSeekRequestInProgress;
...
typedef struct
{
	...
	struct
	{
		OSAL_tTimerHandle		TimerId[ETAL_MAX_QUALITY_PER_MONITOR]; /*! The timer ID */
		tBool					TimerIsEnabled[ETAL_MAX_QUALITY_PER_MONITOR]; /*! Timer Status : enabled/disabled	*/
	} timer_infos;
	tBool isMonitorActiveForTimers;
#endif
...
{
	tU8 v_monitorIndex;
	tU8 v_qualityMonitorAttrIndex;
} etalMonitorTimerMgtTy;
...
// definition for stream type
// Components types
typedef enum
{
    ETAL_COMPONENTTYPE_IS_NO_TYPE              = 0,
    ETAL_COMPONENTTYPE_IS_MSC_STREAM_AUDIO     = 1,
    ETAL_COMPONENTTYPE_IS_MSC_STREAM_DATA      = 2,
    ETAL_COMPONENTTYPE_IS_FIDC                 = 3,
    ETAL_COMPONENTTYPE_IS_MSC_PACKET_DATA      = 4
} ETAL_componentTypeTy;
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT	
	/*! The timers info for monitor callback */
etalMonitorTimerMgtTy monitorsTimerInfo[ETAL_MAX_MONITORS*ETAL_MAX_QUALITY_PER_MONITOR];
#endif // #ifdef CONFIG_ETAL_CPU_IMPROVEMENT	
 ...
 */
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT
tVoid ETAL_intCb_PeriodicCallbackEnable(tVoid);
tVoid ETAL_intCb_PeriodicCallbackDisable(tVoid);
...
EtalDataServiceType ETAL_receiverConvertDABMWDataServiceTypeToInternal(tU8 dabmw_dstype);
...
//tVoid ETAL_TaskWakeUpOnEventFlag(OSAL_tEventHandle vI_eventHandler, tU32 event);
#endif

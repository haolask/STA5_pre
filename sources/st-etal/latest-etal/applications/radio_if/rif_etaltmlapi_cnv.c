...
tSInt rif_etaltmlapi_cnv_etaltml_get_decoded_RDS(tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen);
tSInt rif_etaltmlapi_cnv_etaltml_start_decoded_RDS(tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen);
tSInt rif_etaltmlapi_cnv_etaltml_stop_decoded_RDS(tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen);
...
rif_etaltmlapi_cnv_etal_cmd_list_Ty rif_etaltmlapi_cnv_etal_cmd_list[RIF_ETALTML_API_CNV_ETAL_CMDNUM_MAX] =
{
	...
	{rif_etaltmlapi_cnv_etaltml_get_textinfo}, {rif_etaltmlapi_cnv_etaltml_get_decoded_RDS}, {rif_etaltmlapi_cnv_etaltml_start_decoded_RDS}, 
	{rif_etaltmlapi_cnv_etaltml_stop_decoded_RDS}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, {NULL}, 
	{NULL}, {NULL}, {NULL}, {NULL}
};
...
/***********************************
 *
 * rif_etaltmlapi_cnv_resp_add_EtalRDSData
 *
 **********************************/
/*!
 * \brief		add EtalRDSData to a payload
 * \details		add a EtalRDSData to a response payload. Increase response length and response payload length.
 ...
 * \param[in]	  ptr   - EtalRDSData pointer to value to add.
 * \return		OSAL_OK
 * \return		OSAL_ERROR - invalid response payload length
 * \callgraph
 * \callergraph
 */
tSInt rif_etaltmlapi_cnv_resp_add_EtalRDSData(tU8 *resp, tU32 *rlen, EtalRDSData *ptr)
{
	tSInt ret = OSAL_OK, i;
	...
	if (ptr != NULL)
	{
		if (ret == OSAL_OK)
		{
			/* add m_validityBitmap */
			ret = rif_etalapi_cnv_resp_add_tU32(resp, rlen, ptr->m_validityBitmap);
		}
		if (ret == OSAL_OK)
		{
			/* add m_storedInfoBitmap */
			ret = rif_etalapi_cnv_resp_add_tU32(resp, rlen, ptr->m_storedInfoBitmap);
		}
		if (ret == OSAL_OK)
		{
			/* add m_PS[ETAL_DEF_MAX_PS_LEN] */
			ret = rif_etalapi_cnv_resp_add_pointer_type(resp, rlen, (tVoid *)ptr->m_PS);
			if (ptr != NULL)
			{	
				for(i = 0; i < ETAL_DEF_MAX_PS_LEN; i++)
				{
					ret = rif_etalapi_cnv_resp_add_tChar(resp, rlen, ptr->m_PS[i]);
				}
			}
		}
		if (ret == OSAL_OK)
		{
			/* add m_DI */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_DI);
		}
		if (ret == OSAL_OK)
		{
			/* add m_PI */
			ret = rif_etalapi_cnv_resp_add_tU16(resp, rlen, ptr->m_PI);
		}
		if (ret == OSAL_OK)
		{
			/* add m_PTY */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_PTY);
		}
		if (ret == OSAL_OK)
		{
			/* add m_TP */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_TP);
		}
		if (ret == OSAL_OK)
		{
			/* add m_TA */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_TA);
		}
		if (ret == OSAL_OK)
		{
			/* add m_MS */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_MS);
		}
		if (ret == OSAL_OK)
		{
			/* add m_timeHour */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_timeHour);
		}
		if (ret == OSAL_OK)
		{
			/* add m_timeMinutes */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_timeMinutes);
		}
		if (ret == OSAL_OK)
		{
			/* add m_offset */
			ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_offset);
		}
		if (ret == OSAL_OK)
		{
			/* add m_MJD */
			ret = rif_etalapi_cnv_resp_add_tU32(resp, rlen, ptr->m_MJD);
		}
		if (ret == OSAL_OK)
		{
			/* add m_RT[ETAL_DEF_MAX_RT_LEN] */
			ret = rif_etalapi_cnv_resp_add_pointer_type(resp, rlen, (tVoid *)ptr->m_RT);
			if (ptr != NULL)
			{	
				for(i = 0; i < ETAL_DEF_MAX_RT_LEN; i++)
				{
					ret = rif_etalapi_cnv_resp_add_tChar(resp, rlen, ptr->m_RT[i]);
				}
			}
		}
		if (ret == OSAL_OK)
		{
			/* add m_AFListLen */
			ret = rif_etalapi_cnv_resp_add_tU32(resp, rlen, ptr->m_AFListLen);
		}
		if (ret == OSAL_OK)
		{
			/* add m_AFListPI */
			ret = rif_etalapi_cnv_resp_add_tU32(resp, rlen, ptr->m_AFListPI);
		}
		if (ret == OSAL_OK)
		{
			/* add m_AFList[ETAL_DEF_MAX_AFLIST] */
			ret = rif_etalapi_cnv_resp_add_pointer_type(resp, rlen, (tVoid *)ptr->m_AFList);
			if (ptr != NULL)
			{	
				for(i = 0; i < ETAL_DEF_MAX_AFLIST; i++)
				{
					ret = rif_etalapi_cnv_resp_add_tU8(resp, rlen, ptr->m_AFList[i]);
				}
			}
		}
	}
...
tSInt rif_etaltmlapi_cnv_resp_add_EtalTextInfo(tU8 *resp, tU32 *rlen, EtalTextInfo *ptr)
{
	...
	if (ptr != NULL)
	{
		...
		if (ret == OSAL_OK)
		{
			/* add m_serviceNameIsStored */
			ret = rif_etalapi_cnv_resp_add_tBool(resp, rlen, ptr->m_serviceNameIsStored);
		}
		...
		if (ret == OSAL_OK)
		{
			/* add m_currentInfoIsStored */
			ret = rif_etalapi_cnv_resp_add_tBool(resp, rlen, ptr->m_currentInfoIsStored);
		}
...
/***********************************
 *
 * rif_etaltmlapi_cnv_resp_add_pEtalRDSData
 *
 **********************************/
/*!
 * \brief		add EtalRDSData pointer to a payload
 * \details		add a EtalRDSData pointer to a response payload. Increase response length and response payload length.
 * \param[in/out] resp  - pointer to buffer containing the response payload. the function
 * 				          does not make any assumption on the content of the buffer
 * \param[in/out] rlen  - size in bytes of the *resp* buffer
 * \param[in]	  ptr   - EtalRDSData pointer value to add.
 * \return		OSAL_OK
 * \return		OSAL_ERROR - invalid response payload length
 * \callgraph
 * \callergraph
 */
tSInt rif_etaltmlapi_cnv_resp_add_pEtalRDSData(tU8 *resp, tU32 *rlen, EtalRDSData *ptr)
{
	tSInt ret = OSAL_OK;

	/* add RDSData * to response payload */
	...
	if (ptr != NULL)
	{
		...
		{
			/* add RDSData */
			ret = rif_etaltmlapi_cnv_resp_add_EtalRDSData(resp, rlen, ptr);
		}
	}
...
/***********************************
 *
 * rif_etaltmlapi_cnv_etaltml_get_decoded_RDS
 *
 **********************************/
/*!
 * \brief		handle an etaltml_get_decoded_RDS rimw command
 * \details		call etaltml_get_decoded_RDS api from a rimw message and create rimw response.
 * \param[in]	  cmd   - pointer to buffer of tU8 containing the command. the function
 * 				          does not make any assumption on the content of the buffer
 * \param[in]	  clen  - size in bytes of the *cmd* buffer
 * \param[in/out] resp  - pointer to buffer containing the response. the function
...
tSInt rif_etaltmlapi_cnv_etaltml_get_decoded_RDS (tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen)
{
	...
	tSInt ret = OSAL_OK;
	ETAL_HANDLE hReceiver;
	EtalRDSData rdsData, *pRDSdata = &rdsData;
	ETAL_STATUS retval = ETAL_RET_SUCCESS;
	...
	if (ret == OSAL_OK)
	{
		OSAL_pvMemorySet(&rdsData, 0, sizeof(EtalRDSData));
		ret = rif_etalapi_cnv_type_get_p(&cmd_payload, &cmd_len, (tPVoid *)&pRDSdata);
	}
	if (ret == OSAL_OK)
	{
		/* call etaltml_get_decoded_RDS */
		retval = etaltml_get_decoded_RDS(hReceiver, pRDSdata);
		/* create response */
		ret = rif_etalapi_cnv_create_resp(cmd, clen, resp, rlen, rif_rimw_OSAL_ERROR_to_rimw_error(ret));
	} 
	else
	...
	if (ret == OSAL_OK)
	{
		/* add EtalRDSData * to response payload */
		ret = rif_etaltmlapi_cnv_resp_add_pEtalRDSData(resp, rlen, pRDSdata);
	}
...
/***********************************
 *
 * rif_etaltmlapi_cnv_etaltml_start_decoded_RDS
 *
 **********************************/
/*!
 * \brief		handle an etaltml_start_decoded_RDS rimw command
 * \details		call etaltml_start_decoded_RDS api from a rimw message and create rimw response.
 * \param[in]	  cmd   - pointer to buffer of tU8 containing the command. the function
 * 				          does not make any assumption on the content of the buffer
 * \param[in]	  clen  - size in bytes of the *cmd* buffer
 * \param[in/out] resp  - pointer to buffer containing the response. the function
...
tSInt rif_etaltmlapi_cnv_etaltml_start_decoded_RDS (tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen)
{
	...
	if (ret == OSAL_OK)
	{
		ret = rif_etalapi_cnv_type_get_tU32(&cmd_payload, &cmd_len, &rdsServiceList);
	} 	
	if (ret == OSAL_OK)
	{
		/* call etaltml_start_decoded_RDS */
		retval = etaltml_start_decoded_RDS(hReceiver, rdsServiceList);
		/* create response */
		ret = rif_etalapi_cnv_create_resp(cmd, clen, resp, rlen, rif_rimw_OSAL_ERROR_to_rimw_error(ret));
	} 
	else
...
/***********************************
 *
 * rif_etaltmlapi_cnv_etaltml_stop_decoded_RDS
 *
 **********************************/
/*!
 * \brief		handle an etaltml_stop_decoded_RDS rimw command
 * \details		call etaltml_stop_decoded_RDS api from a rimw message and create rimw response.
 * \param[in]	  cmd   - pointer to buffer of tU8 containing the command. the function
 * 				          does not make any assumption on the content of the buffer
 * \param[in]	  clen  - size in bytes of the *cmd* buffer
 * \param[in/out] resp  - pointer to buffer containing the response. the function
...
tSInt rif_etaltmlapi_cnv_etaltml_stop_decoded_RDS (tU8 *cmd, tU32 clen, tU8 *resp, tU32 *rlen)
{
	...
	if (ret == OSAL_OK)
	{
		/* call etaltml_stop_decoded_RDS */
		retval = etaltml_stop_decoded_RDS(hReceiver, rdsServiceList);
		/* create response */
		ret = rif_etalapi_cnv_create_resp(cmd, clen, resp, rlen, rif_rimw_OSAL_ERROR_to_rimw_error(ret));
	} 
	else

...
#testbuild result file
RESULT_FILE=testbuild/testbuild_result.txt
...
process_target ()
{
	...
	echo "--------------------" >> $RESULT_FILE
	echo "Building set " $2 >> $RESULT_FILE
	echo "-----------" >> $RESULT_FILE
	...
	echo "log ... " $LOG
	make clean 1>&1 | tee $LOG
	make $1 2>&1 | tee -a $LOG

	# Here we count the number of lines in the gcc output created with tee, containing
	# the string 'warning'
    if [ "$1" == "all" ]; then
        let "warning_current += `perl -ne 'BEGIN{$cnt=0;}if(/: warning:\s*Clock skew detected.\s*Your build may be incomplete./i){$_=\"\";}if(/has modification time .* in the future/i){$_=\"\";}if(/: warning:/i){$cnt++;}END{print\"$cnt\";}' $LOG`"
    fi
    if [ "$1" == "splint" ]; then
        let "warning_current += `perl -ne 'BEGIN{$cnt=0;}if(/: warning:\s*Clock skew detected.\s*Your build may be incomplete./i){$_=\"\";}if(/has modification time .* in the future/i){$_=\"\";}if(/(\d+) code warnings/i){$cnt+=$1;}END{print\"$cnt\";}' $LOG`"
    fi
	...
	if [ $? -eq 0 ] ; then
		...
		echo >> $RESULT_FILE
		echo "****************************" >> $RESULT_FILE
		echo BUILD FAILED for $2 >> $RESULT_FILE
		echo "****************************" >> $RESULT_FILE
		...
		echo -e "\n" \
		        "****************************\n" \
		        "BUILD WITH WARNING for $2\n" \
		        "****************************"

		echo >> $RESULT_FILE
		echo "****************************" >> $RESULT_FILE
		echo  BUILD WITH WARNING for $2 >> $RESULT_FILE
		echo "****************************" >> $RESULT_FILE	    
	    else
		echo -e "\n" \
		        "****************************\n" \
		...
		echo >> $RESULT_FILE	    
		echo "****************************" >> $RESULT_FILE	    
		echo BUILD OK for $2 >> $RESULT_FILE	    
		echo "****************************" >> $RESULT_FILE	    	    
	    fi
	fi
	let "total += 1"
   ...
   echo "failed_current " $failed_current >> $RESULT_FILE	    
   echo "warning_current " $warning_current >> $RESULT_FILE	    
   ...
   let warning+=$warning_current
    ...
    if [ "$warning_current" == "0" ] && [ "$failed_current" == "0" ]; then
         rm -f $LOG
          ...
          cp $LOG /tmp/tmp.$2
          rm -f $LOG	  
	  echo "LOG SAVED in : " /tmp/tmp.$2 >> $RESULT_FILE	    
    fi
...
script_warning ()
{
	echo -e "\n" \
	        "*******************************************************************\n" \
	        "* WARNING:\n" \
	        "* This script temporarily overwrites your target_config.h,mak files\n" \
	        "* If you interrupt the script before its natural end you will find a\n" \
	        "* copy of the original files in target_config.*.testbuild\n" \
	        "*******************************************************************"
}
...
script_warning2 ()
{
	echo -e "\n" \
	        "*******************************************************************\n" \
	        "* WARNING:\n" \
	        "* This script overwrites your target_config.h,mak files\n" \
	        "*******************************************************************"
}
...
if [ -n "$2" ] ; then
	...
	echo -e "\n" \
	        "ERROR: The file \'$2\' does not exist, exiting\n"
	script_usage
	exit 1
fi
...
/bin/rm ../../etalcore/exports/etal.a.testbuild > /dev/null 2>&1
cp -f ../../etalcore/exports/etal.a ../../etalcore/exports/etal.a.testbuild > /dev/null 2>&1
/bin/rm -f $RESULT_FILE  > /dev/null 2>&1
...
echo -e "\n" \
        ...
        "****************************\n" \
        "**** TEST BUILD RESULTS : \n" \
        "**** " $failed "builds FAILED / " $total "total\n" \
        "**** total " $warning " warnings\n" \
        "*******************************************************************"

echo "*******************************************************************" >> $RESULT_FILE	    
echo "****************************" >> $RESULT_FILE	    
echo "**** TEST BUILD RESULTS : " >> $RESULT_FILE	    
echo "**** " $failed "builds FAILED / " $total "total" >> $RESULT_FILE	    
echo "**** total " $warning " warnings" >> $RESULT_FILE	    
echo "*******************************************************************" >> $RESULT_FILE	    

...
typedef struct EventElement_t
{
    ...
    EventInfo_t EventInfo[LINUX_OSAL_MAX_THREAD_PER_EVENT_HANDLER];
#ifndef	OSAL_EVENT_SKIP_NAMES
                                               ...
                                               OSAL_tThreadID thread_id)
{
    ...
    for (j = 0; j < LINUX_OSAL_MAX_THREAD_PER_EVENT_HANDLER; j++)
    {
    ...
    tBool waitAllEvents)
{
    ...
    for (j = 0; j < LINUX_OSAL_MAX_THREAD_PER_EVENT_HANDLER; j++)
    {
        ...
        if (thread_entry->isThreadInfoUsed == FALSE)
        {
			...
			//printf("linux_EventInfoAddThread : new thread added j = %d, OSAL_ThreadWhoAmI %d, requestedEvent %d\n", j, OSAL_ThreadWhoAmI(), requestedEvent );
            return thread_entry;
        }
    }
    return NULL;
...
/**
* @brief     OSAL_s32EventWaitForDemo
*
* @details   This function is equivalent to OSAL_s32Event
*		But specific for demos, which may share OSAL
*		Specificy is that thread validity is checked differently 
*		An DEMO Thread is set default
...
*
*/
tS32 OSAL_s32EventWaitForDemo(OSAL_tEventHandle      hEvent,
                       OSAL_tEventMask        mask,
                       OSAL_tenEventMaskFlag  enFlags,
                       OSAL_tMSecond          msec,
                       OSAL_tEventMask       *pResultMask)
{
#define OSAL_THREAD_ID_FOR_DEMOS	0x0FFFFFFF

    EventElement_t *entry;
    EventInfo_t *event_thread_info;
    tU32 current_event;
    OSAL_tThreadID myThread;
	...
	myThread = OSAL_THREAD_ID_FOR_DEMOS;
	
	if (-1 == myThread)
	{
		// this means the thread is no more active
		// we may be in deletion cross case
		//
		errno = ESRCH;
	}
        ...
        )
    {
        ...
        event_thread_info = linux_EventInfoSearchThread(entry, myThread);
        ...
        if (event_thread_info == NULL)
        {
			...
			// printf("OSAL_s32EventWait : new thread added : hEvent %d,  OSAL_ThreadWhoAmI %d, \n", hEvent, OSAL_ThreadWhoAmI());
            if (event_thread_info == NULL)
            {
                errno = ENOMEM;
            }
            ...
            (mask & current_event) != 0))
        {
			...
			// info print no wake
			// printf("++++++++++> OSAL_s32EventWait wake up with event in queue, hEvent %d, mask %d, current_event %d\n", hEvent, mask, current_event);

			// We should mark that the thread is no more waiting and has been served
			//
			pthread_mutex_lock(&event_table_lock);
			event_thread_info->requestedEvent = 0;
			pthread_mutex_unlock(&event_table_lock);
                       ...
                       OSAL_tenEventMaskFlag enFlags)
{
    ...
    if (isValid(hEvent))
    {
		...
		//printf("++++++++++>cond broadcast (0x%x)\n", entry->currentEvent);
					...
					{
						// info print no wake
						// but if it is not a clear but event remain ==> post an print
						if (entry->currentEvent != 0)
						{
							//printf("++++++++++> OSAL_s32EventPost no wake up hEvent %d, mask %d, entry->currentEvent %d, thread_entry->waitAllEvents %d, thread_entry->requestedEvent %d, j = %d\n", 
							//	hEvent, mask, entry->currentEvent, thread_entry->waitAllEvents, thread_entry->requestedEvent, j);
						}
					}
                }
            }

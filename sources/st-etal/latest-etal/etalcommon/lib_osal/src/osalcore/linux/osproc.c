...
#define LINUX_C_THREAD_MAX_NAMELEN 16
	char al_ThreadName[LINUX_C_THREAD_MAX_NAMELEN];
       ...
       (pcorAttr->pfEntry != NULL))
    {
        if(strlen((char*)pcorAttr->szName)<= LINUX_C_THREAD_MAX_NAMELENGTH)
        {
			...
			// set thread name
			memset(al_ThreadName, 0, LINUX_C_THREAD_MAX_NAMELEN);
			strncpy(al_ThreadName, pcorAttr->szName, (LINUX_C_THREAD_MAX_NAMELEN-1));
			
			pthread_setname_np(thread, al_ThreadName);

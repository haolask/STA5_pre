        ...
        tU32 u32Class, tCString coszFormat, ...)	/*lint !e960 - MISRA 16.1 - function has variable number of arguments */
{
    ...
    if( OSALUTIL_s32TraceCheckFilter(u32Level,u32Class) == TRUE )
    {
            ...
            (void)OSAL_s32SemaphoreWait(ostracesem_sem, OSAL_C_TIMEOUT_FOREVER);
        {
            ...
            OSAL_VarArgStart(argList, coszFormat); //lint !e718 !e746 - MISRA 8.1 - __va_start part of posix includes
            /*lint -restore */            
            
            s32Size = OSAL_s32VarNPrintFormat(szBuffer, sizeof(szBuffer), coszFormat, argList);
            OSAL_VarArgEnd(argList); //lint !e718 !e746 - MISRA 8.1 - __va_start part of posix includes
            if (s32Size < 0)
            {
                if (ostracesem_sem != (OSAL_tSemHandle)NULL)
                {
            ...
            else
            {
                ...
                if(OSALUTIL_PrintfEnabled == TRUE)
                {
                     ...
                     */
                    (void)OSALUTIL_s32TracePrintHeader(u32Level, u32Class, headerBuffer, headerBufferLen);
                ...
                if( osutil_trace_output_driver != OSAL_NULL )
                {
                    osutil_trace_output_driver(u32Level, u32Class, OSUTIL_TRACE_MSG_TYPE_ASCII, n, (tU8*)szBuffer);
                }
            }
        }
        if (ostracesem_sem != (OSAL_tSemHandle)NULL)

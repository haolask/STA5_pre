...
static tS32 BSP_LinuxSignalling_fd = -1;
#endif //BSP_INCLUDE_SIGNALLING_GPIO
#endif //CONFIG_ETAL_SUPPORT_CMOST

#ifdef CONFIG_ETAL_SUPPORT_DCOP_HDRADIO
static tS32 HDRADIO_I2C_LinuxDevice_fd = -1;
static tS32 HDRADIO_SPI_LinuxDevice_fd = -1;
static tS32 HDRADIO_LinuxSPI_CS_fd = -1;
#endif //CONFIG_ETAL_SUPPORT_DCOP_HDRADIO
...
static tVoid BSP_SteciCallback_MDR(tVoid);
#endif
...
tVoid BSP_PrintBufNoSpace(tS32 level, tBool useI2C, tUChar *buf, tS32 len, BSPPrintOperationTy op)
{
	...
	if (len > 0)
	{
		for (i = 0; (i < (tU32) len) && (vl_count < (BSP_PRINTBUF_HEX_NS - 2)); i++)
		{
...
static tSInt BSP_BusConfig_MDR(tU32 deviceID)
{
    ...
    {
    	// in case the driver already init : deactivate and reopen
		if (MDR_LinuxDevice_fd != -1)
		{
			BSP_Linux_CloseDevice(MDR_LinuxDevice_fd);
		}
		
    	// in case the driver already init : deactivate and reopen
		if (MDR_LinuxSPI_CS_fd != -1)
		{
			BSP_Linux_CloseDevice(MDR_LinuxSPI_CS_fd);
		}

    	// in case the driver already init : deactivate and reopen
		if (MDR_LinuxSPI_REQ_fd != -1)
		{
			BSP_Linux_CloseDevice(MDR_LinuxSPI_REQ_fd);
		}
		
#ifdef CONFIG_COMM_DCOP_MDR_ENABLE_SPI_BOOT_GPIO
    	// in case the driver already init : deactivate and reopen
		if (MDR_LinuxSPI_BOOT_fd != -1)
		{
			BSP_Linux_CloseDevice(MDR_LinuxSPI_BOOT_fd);
		}
#endif
...
#ifdef CONFIG_ETAL_CPU_IMPROVEMENT_STECI
		MDR_LinuxSPI_REQ_fd = BSP_Linux_OpenGPIO(strGPIO_REQ, GPIO_OPEN_READ, GPIO_IRQ_BOTH, BSP_SteciCallback_MDR);
#else
 ...
 *
 *************************************/
tVoid BSP_WaitForREQInterrupt_MDR(tVoid)
{
	BSP_Linux_WaitForInterrupt(MDR_LinuxSPI_REQ_fd);
}
...
static tSInt BSP_RDSInterruptInit_CMOST(tU32 deviceID)
{
	...
	str_GPIO_IRQ[(sizeof(str_GPIO_IRQ) - (size_t)1)] = '\0';
	(void) OSAL_s32NPrintFormat(str_GPIO_IRQ, (sizeof(str_GPIO_IRQ) - (size_t)1), "%u", TUNERDRIVER_GetGPIOIrq(deviceID));

...
tVoid BSP_Linux_WaitForInterrupt(tS32 fd)
{
	...
	do 
	{
		BSP_tracePrintComponent(TR_CLASS_BSP, "BSP_Linux_WaitForInterrupt poll GPIO %d, ", fd);
		
		ret = poll(param, 1, -1);

		if (ret <= 0)
		{
			BSP_tracePrintError(TR_CLASS_BSP, "Waiting for GPIO IRQ, ret = %d", ret);
		}
		
		// BSP_tracePrintSysmin(TR_CLASS_BSP, "BSP_Linux_WaitForInterrupt Got GPIO %d, ret = %d, param[0].revents 0x%x", fd, ret, param[0].revents);

		// read the value
		value = BSP_Linux_ReadGPIO(fd);
		
		BSP_tracePrintComponent(TR_CLASS_BSP, "BSP_Linux_WaitForInterrupt Got GPIO %d, ret = %d, param[0].revents 0x%x, read = %d", fd, ret, param[0].revents, value);
	} while (POLLPRI != (POLLPRI & param[0].revents)); // if we get an other event we do not care	

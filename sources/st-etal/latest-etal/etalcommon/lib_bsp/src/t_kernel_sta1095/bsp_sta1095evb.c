...
#if defined (CONFIG_COMM_DRIVER_EMBEDDED) && (defined (CONFIG_BOARD_ACCORDO2) || defined (CONFIG_BOARD_ACCORDO5)) && defined (CONFIG_HOST_OS_TKERNEL)
...
static ER _i2c_sample_set_config(ID id)
{   ER err;
    ST_I2C_CONFIG_PARAM buf = { ST_I2C_OPE_MAS, 	// ope_mode
						        ST_I2C_ADDR_NORMAL, // addr_mode
						        ST_I2C_SPEED_FAST, //400Kbps  //ST_I2C_SPEED_STD,   // 100Kbps  
						        0,					// High Speed Master Code 
						        ST_I2C_SGC_TRANSP,	//  Slave General Call Mode 
						        FALSE,				// DMA Tx Enable
						        FALSE,				// DMA Rx Enable
						        FALSE,				// DMA Synchronization Logic Enable 
						        ST_I2C_NORMAL,		// Loopback Mode 
						        ST_I2C_FILTER_NO,	//  Filtering On
						        &callback_master,	// Callback Function pointer when interrup
						        0x00 };
 ...
 ER TNR_SPI_write_SSP( ID dd, W start, void* buf, W size )
{
	...
	if (err != E_OK)
	{
		...
		COMMON_tracePrint(TR_LEVEL_ERRORS, TR_CLASS_BSP, "TNR_SPI_write_SSP : dd = %d, size = %d, err = %x\n", dd, size, err);
	}
...
ST_SSP_CONFIG_PARAM vl_configParam = {
	...
	*/
	...
	if (err != E_OK) 
	{
		COMMON_tracePrint(TR_LEVEL_ERRORS, TR_CLASS_BSP, "SSP/SPI sample : write config error CODE=%08x\n", err);
	}
...
static ER _i2c_data_read(ID id, INT i, UB* read_data,  UINT   slv_add)
{
    ...
    ST_I2C_DATA_PARAM buf = {
        ...
        FALSE,			// /* Burst Mode */
        0,				//  /* Destination Burst Size */
        NULL,			//  /* Pointer to Read/Write Data */
        0 				// sizeof(read_data)
    };

	// add-on, memset the read buffer
	//
	memset(read_data, 0x00,i);
...
static int tnr_write(tS32 fd, tS32 device_address, tU8 *w_buf, tS32 w_len )
{
	...
	err = _i2c_data_write(fd, (UB *)w_buf, w_len, (tU32)(device_address) >> 1);

	if (err != E_OK) 
	{
		COMMON_tracePrint(TR_LEVEL_ERRORS, TR_CLASS_BSP, "tnr_write : fd = %d, len = %d, err = %x\n", fd, w_len, err);
		ret = -1;
	}
...
static int tnr_read(tS32 fd, tS32 device_address, tU8 *r_buf, tS32 r_len )
{
    ...
    err = _i2c_data_read(fd, r_len, (UB *)r_buf, (UINT) device_address >> 1);

	if (err != E_OK) 
	{
		COMMON_tracePrint(TR_LEVEL_ERRORS, TR_CLASS_BSP, "tnr_read : fd = %d, len = %d, err = %x\n", fd, r_len, err);
		ret = -1;
}
...
tVoid BSP_PrintBufNoSpace(tS32 level, tBool useI2C, tUChar *buf, tS32 len, BSPPrintOperationTy op)
{
	...
	if (useI2C)
	{
		sprintf(sbuf + vl_count, "I2C ");
	}
	else
	{
		sprintf(sbuf + vl_count, "SPI ");
	}

	vl_count += 4;

	// print len
	sprintf(sbuf + vl_count, "l=%.2d ", len);
	vl_count += 5;

	
	if (len > 0)
	{
		for (i = 0; (i < len) && (vl_count < (BSP_PRINTBUF_HEX_NS-2)); i++)
		{
			//sprintf(sbuf + (i * 2) + 6, "%.2x", buf[i]);
			sprintf(sbuf + vl_count, "%.2x", buf[i]);
			vl_count += 2;
		}
...
tS32 BSP_Write_CMOST(tU32 deviceID, tU8* buf, tU32 len)
{
...
#if (defined CONFIG_TRACE_CLASS_BSP) && (CONFIG_TRACE_CLASS_BSP >= TR_LEVEL_COMPONENT)
	{
		...
		{
			BSP_PrintBufNoSpace(TR_LEVEL_COMPONENT, TRUE, buf, len, BSP_WRITE_OPERATION);
			}
		else
		{
			BSP_PrintBufNoSpace(TR_LEVEL_COMPONENT, FALSE, buf, len, BSP_WRITE_OPERATION);
			}
		}
#endif

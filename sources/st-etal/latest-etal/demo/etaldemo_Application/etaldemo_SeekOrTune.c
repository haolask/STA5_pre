...
// TMP
ETAL_HANDLE handleDab_RT = ETAL_INVALID_HANDLE;
...
#ifdef HAVE_DAB
static char *streamType(EtalServiceInfo svinfo)
{
	static char *p_streamType;
	
	if (svinfo.m_streamType == ETALDEMO_COMPONENTTYPE_IS_MSC_STREAM_AUDIO)
	{
		if (svinfo.m_componentType == ETALDEMO_ASCTY_DAB_PLUS)
		{
	...
	else if (svinfo.m_streamType == ETALDEMO_COMPONENTTYPE_IS_MSC_STREAM_DATA)
	{
		if (svinfo.m_componentType == ETALDEMO_DSCTY_MPEG2TS)
		{
...
static ETAL_HANDLE etalDemo_DABRadioSeek(tU32 vI_StartFreq)
{
	...
	if ((ETAL_RET_SUCCESS == ret) || (ret == ETAL_RET_NO_DATA))
	{
		...
		ret = etalDemo_DABServiceListAndSelect(handleDab, &Service, &serv_list);
	...
	do
	{
		...
		if (vl_continue)
		{
			...
			if (vl_mode == ETALDEMO_MODE_SEEK)
			{
				...
				if (tunedEnsemble)
				{
					etalDemoPrintf(TR_LEVEL_COMPONENT, "******** Change Service current %d/%d, direction %d \n", (Service+1), serv_list.m_serviceCount, vl_direction);
				    if (vl_direction)
				    {
					    //down
					    if (Service == 0)
					...
					etalDemoPrintf(TR_LEVEL_COMPONENT, "******** Change Service new %d / %d\n", (Service+1), serv_list.m_serviceCount);
				}
				else
				{
				...
				else
				{
					...
					if (ueid != ETAL_INVALID_UEID)
					{
						...
						for (i = 0; i < serv_list.m_serviceCount; i++)
						{
							...
							etalDemoPrintf(TR_LEVEL_COMPONENT, "\tSid %d: 0x%x (%s) charset: %d, bitrate: %d kbps, streamType: %s\n", 
							 		(i+1), serv_list.m_service[i], svinfo.m_serviceLabel, svinfo.m_serviceLabelCharset,
									svinfo.m_serviceBitrate,streamType( svinfo));
						...
						if ((!IsDemonstrationMode) && ((vl_mode == ETALDEMO_MODE_SERVICE_SELECT) || (tunedEnsemble == false)))
						{
							...
							etalDemoPrintf(TR_LEVEL_COMPONENT, "Selecting a service from the ensemble, service number = %d, service ID = 0x%x, Service Name = %s, Charset: %d Bitrate: %d\n",
												Service+1,
												serv_list.m_service[Service], svinfo.m_serviceLabel, svinfo.m_serviceLabelCharset, svinfo.m_serviceBitrate);
			...
			else if (vl_mode == ETALDEMO_MODE_MANUAL_SEEK)
			{
				...
				if ((ret = etal_seek_start_manual(handleDab, vl_direction, 0, &seekFreq)) != ETAL_RET_NO_DATA)
				{
				    ...
				    if ((ret = etalDemo_DABServiceListAndSelect(handleDab, &Service, &serv_list)) != ETAL_RET_SUCCESS)
				    {
					    etalDemoPrintf(TR_LEVEL_COMPONENT, "etalDemo_DABServiceListAndSelect error" );
...
static ETAL_STATUS etalDemo_DABServiceListAndSelect(ETAL_HANDLE handleDab, int *pO_Service, EtalServiceList *pO_servList)
{
	EtalServiceList serv_list;
	EtalServiceInfo svinfo;
	static EtalServiceComponentList scinfo; // static due to size
    ...
    else
	{
		...
		if (uEId != ETAL_INVALID_UEID)
		{
			...
			etalDemoPrintf(TR_LEVEL_COMPONENT, "Selecting a service from the ensemble, service number = %d, service ID = 0x%x, Service Name = %s, bitrate = %d kbps, StreamType %s\n",
							Service+1, serv_list.m_service[Service], svinfo.m_serviceLabel, svinfo.m_serviceBitrate, streamType(svinfo));
			if ((ret = etal_service_select_audio(handleDab, ETAL_SERVSEL_MODE_SERVICE, uEId, serv_list.m_service[Service], ETAL_INVALID, ETAL_INVALID)) != ETAL_RET_SUCCESS)
			{
				etalDemoPrintf(TR_LEVEL_COMPONENT, "ERROR: etal_service_select (%d)", ret);
				return ret;
	...
	if (NULL != pO_servList)
	{
		memcpy(pO_servList, &serv_list, sizeof(EtalServiceList));
	}
...
static void etalDemo_DABQualityMonitorCallback(EtalBcastQualityContainer* pQuality, void* vpContext)
{
	...
	// radio Text notification will happen only if something new
	// so get current
	etaltml_get_textinfo(handleDab_RT, &vl_Radiotext);
	etalDemo_printRadioText("  etalDemo_currentRadioTextInfo : ", &vl_Radiotext);
}

...
my ($fresR, $forceFastPi, $numPi, $rbdsmode);
my ($frestat);
...
$fresR->Button(-text => 'etal_start_RDS', -command => sub { gui_print("estaR $cur_recv_idx $forceFastPi $numPi $rbdsmode\n"); })->pack(-side => "left");
$forceFastPi = 0;
...
$fresR->Button(-text => 'etaltml_start_textinfo', -command => sub { gui_print("estat $cur_recv_idx\n"); })->pack(-side => "left");
$fresR->Button(-text => 'etaltml_get_textinfo', -command => sub { gui_print("egt $cur_recv_idx\n"); })->pack(-side => "left");
...
$frestat->Button(-text => 'etaltml_stop_decoded_RDS', -command => sub { gui_print("estodR $cur_recv_idx $RDSServiceList\n"); })->pack(-side => "left");

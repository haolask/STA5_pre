...
tS32 DAB_Init(tyDABDeviceConfiguration *deviceConfiguration)
{
    ...
    if(deviceConfiguration != NULL)
    {
        ...
        {
           STECI_tracePrintSystem(TR_CLASS_STECI, "DAB_Init : IsBootMode = %d, Bus name = %s, device address = 0x%x, GPIO_reset = %d, GPIO_REQ = %d, GPIO_BOOT = %d",
           ...
           ret = OSAL_ERROR;
           STECI_tracePrintError(TR_CLASS_STECI, "DAB_Init : Return = %d, I2C not supported for DCOP", ret);
        }
        else
        {
            STECI_tracePrintSystem(TR_CLASS_STECI, "DAB_Init : IsBootMode = %d, Bus name = %s, SPI mode = %d, SPI speed = %d, GPIO_CS = %d, GPIO_reset = %d, GPIO_REQ = %d, GPIO_BOOT = %d",
                    deviceConfiguration->isBootMode,
    	...
    	STECI_tracePrintSystem(TR_CLASS_STECI, "DAB_Init : Start TCP/IP thread for EXTERNAL driver operation with DCOP with IP : %s",
		...
		{
			ret = OSAL_ERROR;
			STECI_tracePrintError(TR_CLASS_STECI, "DAB_Init : Return = %d, ERROR DCOP TCP/IP thread or semaphore creation", ret);
		}
		else
		{
			/* Send a RESET command to the DCOP */
			DAB_sendResetWithExternalDriver();
			STECI_tracePrintSystem(TR_CLASS_STECI, "DAB_Init : Return = %d, SUCCESS", ret);
		}
        ...
        if (BSP_DeviceInit_MDR(deviceInfo->DABDeviceConfigID) == OSAL_OK)
        {
        	...
        	BSP_DeviceReset_MDR(deviceInfo->DABDeviceConfigID);
        	if (ETAL_InitSteciProtocol(&STECIParam) != OSAL_OK)
        	{
        		ret = OSAL_ERROR;
        		STECI_tracePrintError(TR_CLASS_STECI, "DAB_Init : Return = %d, ERROR in ETAL_InitSteciProtocol()", ret);
        	}
        	else
        	{
        		STECI_tracePrintSystem(TR_CLASS_STECI, "DAB_Init : Return = %d, SUCCESS", ret);
        ...
        {
        	ret = OSAL_ERROR;
        	STECI_tracePrintError(TR_CLASS_STECI, "DAB_Init : Return = %d, ERROR in BSP_DeviceInit_MDR()", ret);
        }
#endif //CONFIG_COMM_DRIVER_EXTERNAL
    }
    else
    {
        ret = OSAL_ERROR;
        STECI_tracePrintError(TR_CLASS_STECI, "DAB_Init : Return = %d, ERROR pointer is NULL (deviceConfiguration = 0x%x)", ret, deviceConfiguration);
    }
...
tS32 DAB_Reset(tVoid)
{
	...
	if (TcpIpProtocolIsConnected(ProtocolLayerIndexDCOP))
	{
		/* Send a RESET command to the DCOP */
		DAB_sendResetWithExternalDriver();
	    STECI_tracePrintError(TR_CLASS_STECI, "DAB_Reset : Return = %d, SUCCESS", ret);
	}
	else
	{
        ret = OSAL_ERROR;
...
tS32 DAB_StartupReset(tyDABDeviceConfiguration *deviceConfiguration)
{
    ...
    {
#ifdef CONFIG_COMM_DRIVER_EXTERNAL
        STECI_tracePrintError(TR_CLASS_STECI, "DAB_Reset : Return = %d, SUCCESS (Warning : this function should has no effect)", ret);
#else //CONFIG_COMM_DRIVER_EXTERNAL
    	if(deviceConfiguration->communicationBusType == BusI2C)
        {
...
#ifdef CONFIG_COMM_DRIVER_EXTERNAL
    ret = OSAL_ERROR;
    STECI_tracePrintError(TR_CLASS_STECI, "DAB_Send_Message : Return = %d, ERROR This function should not be called", ret);
#else //CONFIG_COMM_DRIVER_EXTERNAL
...
#ifdef CONFIG_COMM_DRIVER_EXTERNAL
    ret = OSAL_ERROR;
    STECI_tracePrintError(TR_CLASS_STECI, "DAB_Get_Response : Return = %d, ERROR This function should not be called", ret, *buf, max_buf);
#else //CONFIG_COMM_DRIVER_EXTERNAL

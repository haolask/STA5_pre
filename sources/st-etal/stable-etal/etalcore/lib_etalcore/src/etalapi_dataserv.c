...
static tSInt ETAL_enableDataService(ETAL_HANDLE hReceiver, EtalDataServiceType service, etalCmdActionTy action, EtalDataServiceParam param)
{
	...
	switch (service)
	{
		case ETAL_DATASERV_TYPE_EPG_RAW:
			ret = ETAL_cmdStartStopMOT(hReceiver, etalMOTServiceEPGraw, action);
			break;

		case ETAL_DATASERV_TYPE_SLS:
			ret = ETAL_cmdStartStopMOT(hReceiver, etalMOTServiceSLS, action);
			break;

		case ETAL_DATASERV_TYPE_SLS_XPAD:
			ret = ETAL_cmdStartStopMOT(hReceiver, etalMOTServiceSLSoverXPAD, action);
			break;
		...
		case ETAL_DATASERV_TYPE_SLI:
			ETAL_tracePrintSysmin(TR_CLASS_APP_ETAL_CMD, "Requeste for a Data Service not supported yet");
			break;
...
#else
			ret = ETAL_cmdGetPADevent_DABMWoH(hReceiver, action);
#endif
			break;

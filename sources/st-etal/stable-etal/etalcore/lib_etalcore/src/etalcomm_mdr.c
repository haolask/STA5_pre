...
#define DABMW_DATACHANNEL_MSGTYPE_FIC            0x00
#define DABMW_DATACHANNEL_MSGTYPE_RAW            0x02
#define DABMW_DATACHANNEL_MSGTYPE_DECODED        0x03
#define DABMW_DATACHANNEL_MSGTYPE_AUDIO          0x06
...
#endif // #ifdef CONFIG_COMM_DRIVER_DIRECT
#if defined (CONFIG_COMM_DRIVER_EXTERNAL)
static tBool                ETAL_commMdr_wait_reset_response = FALSE;
...
tSInt ETAL_initCommunication_MDR(EtalDcopBootType isBootMode, tBool vI_manageReset)
{
	...
	{
		// Dab_init function is initializing the DAB driver and Setting the DCOP in reset
	...
	else
	{
		...
		if (DAB_Driver_Init(&DABDeviceConfig) != OSAL_OK)
		{
#ifdef CONFIG_COMM_DRIVER_EXTERNAL
			ETAL_initStatusSetDCOPStatus(deviceCommunication);
#endif
	...
	ETAL_tracePrintComponent(TR_CLASS_APP_ETAL, "ETAL_initCommunication_MDR DCOP DAB manage reset : %d\n", vI_manageReset);
	...
	{
		// instead of waiting, we can do a ping.
		// when ping is ok, it means the DCOP is ready to work
#if 0
...
tSInt ETAL_Reset_MDR(tVoid)
{
	...
	// make sure the reset is completed here ?
	//OSAL_s32ThreadWait(DAB_FW_LOADING_TIME);
		
#elif defined (CONFIG_COMM_DRIVER_EXTERNAL)
	if (DAB_Reset() == OSAL_OK)
	{
		ETAL_commMdr_wait_reset_response = TRUE;
	}
	else
	{
		ret = OSAL_ERROR_DEVICE_NOT_OPEN;
...
static tVoid ETAL_CommunicationLayer_Send_MDR(etalToMDRCmdTy *cmd, etalToMDRCmdStatusTy *status)
{
	...
	if (ETAL_toMDRCmdBufferHaveNew)
	{
		...
		ForwardPacketToCtrlAppPort(ProtocolLayerIndexDCOP, cmd->cmd, cmd->cmdLen, (tU8)0x1D, ETAL_CONTROL_LUN, cmd->specific0, cmd->specific1, cmd->specific2, cmd->specific3);
#else
			...
			if (ETAL_extractTune_MDR(data, len, &tune) == OSAL_OK)
			{
				...
				vl_intEvent = callAtTuneFrequency;
	 			...
	 			if(tune.m_stopFrequency != 0)
	 			{
					...
					vl_tuneStatusInternal.m_externalRequestInfo = ETAL_receiverIsSpecialInProgress(tune.m_receiverHandle, cmdSpecialExternalTuneRequestInProgress);
...
static tVoid ETAL_CommunicationLayer_Receive_Direct_MDR(tU8 *data, tU32 len, etalToMDRCmdTy *cmd, etalToMDRCmdStatusTy *status, tU8 lun)
{
	...
	if (rif_pr_msg_queue_p != NULL)
	{
		...
		else
		{
			...
			msg_p->num_bytes = len;
			msg_p->msg_buf_ptr = (tU8 *)((tPVoid)msg_p + sizeof(rif_pr_msg_header_t));
 ...
 * byte 8     = new flag, region flag, sub channel Id,
 * byte 9     = region Id
 */
static tVoid ETAL_CommunicationLayer_DABAnnouncementRawAutonotification_MDR(ETAL_HANDLE hReceiver, tU8 *data, tU32 len)
{
	...
	if((recvp != NULL) && (autoNotifp != NULL))
	{
		...
		autoNotifp->status.DABAnnouncementRaw.clusterId = ETAL_utilityGetU8(data, 5);
		autoNotifp->status.DABAnnouncementRaw.announcementType = ETAL_utilityGetU16(data, 6); // the first 4 bytes are the common auto notification header
		autoNotifp->status.DABAnnouncementRaw.subChId = ETAL_utilityGetU8(data, 8) & 0x1F;
		autoNotifp->status.DABAnnouncementRaw.newFlag = ETAL_utilityGetU8(data, 8) & 0x20;
		autoNotifp->status.DABAnnouncementRaw.regionFlag = ETAL_utilityGetU8(data, 8) & 0x40;
		autoNotifp->status.DABAnnouncementRaw.regionId = ETAL_utilityGetU8(data, 9);

		ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "DAB Announcement Switching RAW auto-notification(previous), type=%s (0x%x), clusterId=0x%x, subChId=0x%x, regionFlag=0x%x, regionId=0x%x, newFlag=%d",
				ETAL_DABAnnouncementAutonotification2String_MDR(previousNotif.status.DABAnnouncementRaw.announcementType),
				previousNotif.status.DABAnnouncementRaw.announcementType,
				previousNotif.status.DABAnnouncementRaw.clusterId,
				previousNotif.status.DABAnnouncementRaw.subChId,
				previousNotif.status.DABAnnouncementRaw.regionFlag,
				previousNotif.status.DABAnnouncementRaw.regionId,
				previousNotif.status.DABAnnouncementRaw.newFlag);

		ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "DAB Announcement Switching RAW auto-notification(new), type=%s (0x%x), clusterId=0x%x, subChId=0x%x, regionFlag=0x%x, regionId=0x%x, newFlag=%d",
				ETAL_DABAnnouncementAutonotification2String_MDR(autoNotifp->status.DABAnnouncementRaw.announcementType),
				autoNotifp->status.DABAnnouncementRaw.announcementType,
				autoNotifp->status.DABAnnouncementRaw.clusterId,
				autoNotifp->status.DABAnnouncementRaw.subChId,
				autoNotifp->status.DABAnnouncementRaw.regionFlag,
				autoNotifp->status.DABAnnouncementRaw.regionId,
				autoNotifp->status.DABAnnouncementRaw.newFlag);
...
static tVoid ETAL_CommunicationLayer_ReceiveAutonotification_MDR(tU8 *data, tU32 len)
{
	...
	switch (data[DABMW_AUTONOTIFICATION_TYPE_OFFSET])
	{
		...
		case DABMW_AUTONOTIFICATION_ANNOUNCEMENT_RAW:
			if (DABMW_AUTONOTIFICATION_GET_SIZE(data) == (tU8)DABMW_AUTONOTIFICATION_ANNOUNCEMENT_RAW_SIZE)
			{
...
#endif
{
	...
	/*
	 * This buffer is used as the equally-named buffer in the CONFIG_COMM_DRIVER_EMBEDDED
	 * code below. It uses a different macro name for the size
	 * but the macro maps to the same value
	 */
	static tU8 data[PROTOCOL_LAYER_INTERNAL_BUFFER_LEN];
	...
	while (TRUE)
	{
		...
		do
		{
			got_data = FALSE;
#ifdef CONFIG_COMM_DRIVER_EXTERNAL
			if (ProtocolLayer_FifoPop(ProtocolLayerIndexDCOP, data, PROTOCOL_LAYER_INTERNAL_BUFFER_LEN, &len, &head) == OSAL_OK)
			{
				if ((ETAL_commMdr_wait_reset_response == TRUE) && (head.CtrlAppLunId == ETAL_BROADCAST_LUN) && 
					(len == 1) && (data[0] == ETAL_EXTERNAL_DRIVER_TYPE_RESET))
				{
					ETAL_commMdr_wait_reset_response = FALSE;
					/* is Reset response from DAB_sendResetWithExternalDriver => drop response */
					/* to avoid crossing case with first MDR ping command */
					ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "Received message ID 0x%.3x from DCOP on LUN 0x%.2x, %d bytes, reset response not sent to upper layer", ETAL_MDR_COMMAND_ID(data), head.CtrlAppLunId, len);
				}
				else if (ETAL_commMdr_wait_reset_response == TRUE)
				{
					/* drop all response until reset mdr response received */
					ETAL_tracePrintComponent(TR_CLASS_APP_ETAL_COMM, "Received message ID 0x%.3x from DCOP on LUN 0x%.2x, %d bytes, not sent to upper layer, waiting reset response", ETAL_MDR_COMMAND_ID(data), head.CtrlAppLunId, len);
				}
				else
				{
					got_data = TRUE;
...
{
	OSAL_tMSecond start_time, end_time, current_time, retry_time = 0;

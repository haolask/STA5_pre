...
#if 0
			// Problem in the DCOP no response on this command.
			// DAB monitoring is still performed in the DCOP.
			// But RF field strength is monitored by polling the CMOST
			if (ETAL_cmdGetQuality_MDR(hReceiver, pBcastQuality) != OSAL_OK)
			{
				ret = ETAL_RET_ERROR;
			}
#endif

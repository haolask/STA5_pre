 ...
 */
#define ETAL_CMOST_CSR_RDS_IREN               0x04
/*!
 ...
 */
#define ETAL_CMOST_CSR_ERRTHRESH_0            0x00
#define ETAL_CMOST_CSR_ERRTHRESH_1            0x10
#define ETAL_CMOST_CSR_ERRTHRESH_2            0x20
#define ETAL_CMOST_CSR_ERRTHRESH_3            0x30
#define ETAL_CMOST_CSR_ERRTHRESH_6            0x60
 ...
 */
#define ETAL_CMOST_CSR_RESET                  0x02
...
{
#define ETAL_RDS_DEFAULT_NB_BLOCKS_FAST_PI	1
#define ETAL_RDS_DEFAULT_NB_BLOCKS_NORMAL	16
	...
	if (RDSAttr != NULL)
	{
		...
		if (RDSAttr->rdsRbdsMode == ETAL_RBDS_MODE)
		{
	...
	else
	{
		...
		// disable RDS
		//
		CMOSTcmd_RDSBufferSet[8] = 0;
		//
		// set some valid default values to nb blocks awake
		// modify NBR (PAR3)
		CMOSTcmd_RDSBufferSet[10] |= (ETAL_RDS_DEFAULT_NB_BLOCKS_FAST_PI & 0x0F);
...
tSInt ETAL_cmdSeekGetStatus_CMOST(ETAL_HANDLE hReceiver, tBool *pO_seekStoppedOnGoodFrequency, tBool *pO_fullCycleReached, tBool *pO_bandBorderCrossed, tU32 *freq, EtalBcastQualityContainer *quality)
{
	...
	*/
	...
	if (quality != NULL)
	{
		...
		ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_CMD, "GetSeekStatus: RFFS %ld, BBFS %ld, Det %lu, ADJ %ld, MPX %lu, MP %lu, SNR %lu, coch %lu, Good freq %d, Cycle reached %d, Band crossed %d, freq %d",
...
tSInt ETAL_cmdGetRDS_CMOST(ETAL_HANDLE hReceiver, tU8 *resp, tU32 *buflen)
{
    ...
    tU8 CMOSTcmd_RDSBufferRead[] = {0x00, 0x1B, 0x00};
#else
	tU8 CMOSTcmd_RDSBufferRead[] = {0x00, 0x1B, 0x01, 0x00, 0x00, 0x01}; // PAR1 changed below
#endif
	tU16 cstatus;
	tSInt retval;
	tU32 rlen;

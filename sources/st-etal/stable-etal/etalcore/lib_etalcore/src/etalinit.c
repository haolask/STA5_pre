...
tSInt ETAL_init(const EtalHardwareAttr *hardware_attr, tBool power_up)
{
	...
	(LINT_IGNORE_RET)ipforwardPowerUp();
#endif
...
//	if (1)
	...
	if (FALSE == tunerIsAlreadyStarted)
	{
		 ...
		 */
// SPI1 ACCORDO 5 : reset done by M3
...
//CLE Virer tout le code CONFIG_COMM_DRIVER_EXTERNAL (utiliser uniquement pour une liason PC)

...
tCString ETAL_EventToString(ETAL_EVENTS ev)
{
	...
	switch (ev)
	{
		...
		case ETAL_INFO_DAB_AUTONOTIFICATION:
			return "Info DAB autonotification";
	...
	if (cbTypeEvent == msg_type)
	{
		...
		switch (event)
		{
			...
			case ETAL_INFO_DAB_AUTONOTIFICATION:
			{
				...
				{
					ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "%s (recHan: %d, eveTyp=%d, annTyp=0x%x, clusterId=0x%x, subchId=0x%x, regionFlag=0x%x, regionId=0x%x, newFlag=%d)",
						str_evt,
						autonotification_status->receiverHandle,
						autonotification_status->type,
						autonotification_status->status.DABAnnouncementRaw.announcementType,
						autonotification_status->status.DABAnnouncementRaw.clusterId,
						autonotification_status->status.DABAnnouncementRaw.subChId,
						autonotification_status->status.DABAnnouncementRaw.regionFlag,
						autonotification_status->status.DABAnnouncementRaw.regionId,
						autonotification_status->status.DABAnnouncementRaw.newFlag);
				}
			...
			case ETAL_INFO_RDS_STRATEGY:
			{
				...
				ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "%s (SwitchedFreq: %d, Bitmap : %d)",
					str_evt,
...
static tVoid ETAL_traceRDSData(EtalRDSData *rds, tPChar prefix)
{
	...
	ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sRDS decoded data 0x%03x", prefix, rds->m_validityBitmap);

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_PI) == ETAL_DECODED_RDS_VALID_PI)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sPI          : 0x%x", PREFIX0, rds->m_PI);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_DI) == ETAL_DECODED_RDS_VALID_DI)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sDI          : 0x%x", PREFIX0, rds->m_DI);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_PS) == ETAL_DECODED_RDS_VALID_PS)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sPS          : %.*s", PREFIX0, ETAL_DEF_MAX_PS_LEN, rds->m_PS);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_RT) == ETAL_DECODED_RDS_VALID_RT)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sRT          : %.*s", PREFIX0, ETAL_DEF_MAX_RT_LEN, rds->m_RT);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_TOM) == ETAL_DECODED_RDS_VALID_TOM)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTime(hour)  : %d", PREFIX0, rds->m_timeHour);
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTime(min)   : %d", PREFIX0, rds->m_timeMinutes);
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTime(offs)  : %d", PREFIX0, rds->m_offset);
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTime(MJD)   : %d", PREFIX0, rds->m_MJD);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_AF) == ETAL_DECODED_RDS_VALID_AF)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sAFListPI    : 0x%x", PREFIX0, rds->m_AFListPI);
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sAFListLen   : %d", PREFIX0, rds->m_AFListLen);
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sAFList      : ", PREFIX0);
		for (i = 0; i < rds->m_AFListLen; i++)
		{
			ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%s%6d MHz", PREFIX1, 87500 + 100 * (tU32)rds->m_AFList[i]);
		}
	}
	
	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_PTY) == ETAL_DECODED_RDS_VALID_PTY)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sPTY         : %d", PREFIX0, rds->m_PTY);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_TP) == ETAL_DECODED_RDS_VALID_TP)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTP          : %d", PREFIX0, rds->m_TP);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_TA) == ETAL_DECODED_RDS_VALID_TA)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sTA          : %d", PREFIX0, rds->m_TA);
	}

	if ((rds->m_validityBitmap & ETAL_DECODED_RDS_VALID_MS) == ETAL_DECODED_RDS_VALID_MS)
	{
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sMS          : %d", PREFIX0, rds->m_MS);
	}
#endif // defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_1)
}
...
static tVoid ETAL_traceTextInfo(EtalTextInfo *radio_text)
{
	...
	ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sRadio text:\n", PREFIX0);
	ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sBroadcast Standard: %s\n", PREFIX1, ETAL_Standard2Ascii(radio_text->m_broadcastStandard));
	ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sService name %s: %.*s\n", PREFIX1, (radio_text->m_serviceNameIsNew == TRUE)?"is new":"      ", 
						ETAL_DEF_MAX_SERVICENAME, radio_text->m_serviceName);
	ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "%sCurrent Info %s: %s\n", PREFIX1, (radio_text->m_currentInfoIsNew == TRUE)?"is new":"      ", 
	...
	if (ETAL_handleIsDatapath(hDatapath))
	{
		...
		switch (datapathDataType)
		{
			...
			case ETAL_DATA_TYPE_FM_RDS:
				if (param != NULL)
				{
					ETAL_traceRDSData((EtalRDSData *)param, "");
				}
				else
				{
					ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "param = NULL");

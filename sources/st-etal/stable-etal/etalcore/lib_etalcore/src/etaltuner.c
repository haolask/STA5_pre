...
#ifndef CONFIG_HOST_OS_FREERTOS
...
#endif // #ifdef CONFIG_COMM_CMOST_I2C
#endif // ACCORDO_5
...
#ifdef CONFIG_COMM_CMOST_I2C
#if defined (CONFIG_BOARD_ACCORDO5) && !defined(CONFIG_HOST_OS_FREERTOS)
...
#endif
			/* make lint happy in case of CONFIG_COMM_CMOST_SPI not defined */
		}
	}
...
/***************************
 *
 * ETAL_getDeviceConfigExternal_CMOST
 *
 **************************/
/*!
 * \brief		Returns the device configuration of a Tuner
...
tSInt ETAL_getDeviceConfigExternal_CMOST(ETAL_HANDLE hTuner, tyCMOSTDeviceConfiguration *deviceConfiguration)
{
	...
	if(tuner_index == ETAL_CMOST_TUNER_1_ID)
	{
		deviceConfiguration->GPIO_RESET = 0 /*CMOST_T1_ACCORDO2_RESET_GPIO*/;
		deviceConfiguration->GPIO_IRQ = 0 /*CMOST_T1_ACCORDO2_IRQ_GPIO*/;
		deviceConfiguration->IRQCallbackFunction = NULL;
		...
		{
#ifdef CONFIG_COMM_CMOST_SPI
			OSAL_pvMemorySet(deviceConfiguration->communicationBus.spi.busName, 0, MAX_SIZE_BUS_NAME);
			deviceConfiguration->communicationBus.spi.GPIO_CS = 0 /*CMOST_T1_ACCORDO2_SPI_CS_GPIO*/;
			deviceConfiguration->communicationBus.spi.mode = CMOST_ACCORDO2_SPI_MODE;
			deviceConfiguration->communicationBus.spi.speed = 0 /*CMOST_ACCORDO2_SPI_SPEED*/;
#endif
		}
	}
	else if(tuner_index == ETAL_CMOST_TUNER_2_ID)
...
#ifdef CONFIG_COMM_CMOST_SPI
			OSAL_pvMemorySet(deviceConfiguration->communicationBus.spi.busName, 0, MAX_SIZE_BUS_NAME);
			deviceConfiguration->communicationBus.spi.GPIO_CS = 0 /*CMOST_T2_ACCORDO2_SPI_CS_GPIO */;
			deviceConfiguration->communicationBus.spi.mode = CMOST_ACCORDO2_SPI_MODE;
			deviceConfiguration->communicationBus.spi.speed = 0 /*CMOST_ACCORDO2_SPI_SPEED*/;
#endif
		}
...
/***************************
 *
 * ETAL_tunerGetBusIsI2C
 *
 **************************/
/*!
 * \brief		Checks if a Tuner is connected in the I2C bus
 * \details		CMOST Tuners support communication to the Host controller via I2C or
 * 				SPI buses. Whether one or the other is used depends on the hardware
 * 				configuration and is specified at ETAL build-time.
 * \param[in]	hTuner  - Tuner handle
 * \return		TRUE if the device is connected to a I2C bus, FALSE if it is an SPI bus.
 * \see			The etalconfig_*.c files for the Tuner description.
 * \callgraph
 * \callergraph
 */
tBool ETAL_tunerGetBusIsI2C(ETAL_HANDLE hTuner)
{
	ETAL_HINDEX tuner_index;
	...
	tuner_index = ETAL_handleTunerGetIndex(hTuner);
	return etalTuner[tuner_index].deviceDescr.busType == BusI2C;
}
#endif

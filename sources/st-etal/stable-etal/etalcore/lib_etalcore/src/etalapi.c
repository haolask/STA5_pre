...
static tSInt ETAL_versionSet_CMOST(EtalComponentVersion vers[])
{
	...
	for (i = 0; i < ETAL_CAPA_MAX_TUNER; i++)
	{
			...
			ETAL_statusHardwareAttrIsTunerActive(hTuner))
		{
			...
			(void)OSAL_s32NPrintFormat(vers[i].m_name, ETAL_VERSION_NAME_MAX, "CMOST[%u] %s-%s %d.%d.%d build 0", i+1, cmost_flavour, cmost_channels, major, middle, minor);
...
{
	tSInt retval;
	tU32 year, month, day, major, minor, internal, build;

	OSAL_pvMemorySet((tVoid *)vers, 0x00, sizeof(EtalComponentVersion));
	if (ETAL_statusHardwareAttrIsDCOPActive())
	{
		retval = ETAL_cmdGetFirmwareVersion_MDR(&year, &month, &day, &major, &minor, &internal, &build);
		if (retval != OSAL_OK)
		{
			return OSAL_ERROR;
		}
		...
		vers->m_minor = minor;
		vers->m_build = build;
		OSAL_s32NPrintFormat(vers->m_name, ETAL_VERSION_NAME_MAX, "MDR %u.0.%u build %u %u-%u-%u", major, minor, build, year, month, day);
		vers->m_isValid = TRUE;
	}
	else
	{
...
static tVoid ETAL_tracePrintInitParameters(const EtalHardwareAttr *init_params)
{
	...
	if (init_params != NULL)
	{
		...
		ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "init_params (m_DCOPAttr: (m_isDisabled: %d, m_doFlashDump: %d, m_doFlashProgram: %d, m_doDownload: %d, m_cbGetImage: 0x%x, m_pvGetImageContext: %p, m_cbPutImage: 0x%x, m_pvPutImageContext: %p))",
			...
			init_params->m_DCOPAttr.m_cbPutImage, init_params->m_DCOPAttr.m_pvPutImageContext);

		for (i = 0; i < ETAL_CAPA_MAX_TUNER; i++)
		{
			ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "init_params (TunerAttr: (TunerId = %d, m_isDisable %d, m_useDownloadImageuse %d, m_DownloadImageSize %d, m_useCustomParam %d, m_CustomParamSize %d, XTALAli: %d, XTALAli: %d))",
				i,

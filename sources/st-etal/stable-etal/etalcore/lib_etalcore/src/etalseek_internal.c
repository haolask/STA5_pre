...
static OSAL_tEventHandle DABSeek_TaskEventHandler;
static OSAL_tThreadID ETAL_DABSeek_ThreadId;

static tVoid ETAL_DABSeekStartTask(tVoid);
static tVoid ETAL_DABSeekThreadEntry(tPVoid dummy);
...
tSInt ETAL_DABSeekTaskInit (tVoid)
{
    ...
    if (OSAL_s32EventCreate((tCString)"DABSeek_EventHandler", &DABSeek_TaskEventHandler) == OSAL_OK)
    {
        /* Initialize the thread */
        attr0.szName = (tChar *)ETAL_DAB_SEEK_THREAD_NAME;
        ...
        if ((ETAL_DABSeek_ThreadId = OSAL_ThreadSpawn(&attr0)) == OSAL_ERROR)
        {
...
tSInt ETAL_DABSeekTaskDeinit (tVoid)
{
	...
	/* Post the kill event */
	(LINT_IGNORE_RET) OSAL_s32EventPost (DABSeek_TaskEventHandler,
                    ETAL_DAB_SEEK_EVENT_KILL_FLAG,
                    OSAL_EN_EVENTMASK_OR);
	OSAL_s32ThreadWait(10);

	/* Delete the thread */
    if (OSAL_s32ThreadDelete(ETAL_DABSeek_ThreadId) != OSAL_OK)
    {
        ret = OSAL_ERROR;
    }
	else
	{
		ETAL_DABSeek_ThreadId = (OSAL_tThreadID)0;
	}
    return ret;
}
 ...
 *
 **************************/
static tVoid ETAL_DABSeek_TaskClearEventFlag(tU32 eventFlag)
{
...
#endif //CONFIG_HOST_OS_TKERNEL
{
    ...
    while (TRUE)
    {
        ...
        ETALTML_PRINTF(TR_LEVEL_USER_4, "ETAL_DABSeekWaitingTask : waiting for DAB seek activation\n");
#endif //#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)


        vl_res = OSAL_s32EventWait (DABSeek_TaskEventHandler,
                                     ETAL_DAB_SEEK_EVENT_WAIT_WAKEUP_MASK,
                                     OSAL_EN_EVENTMASK_OR,
                                     OSAL_C_TIMEOUT_FOREVER,
                                     &vl_resEvent);
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)
        ETALTML_PRINTF(TR_LEVEL_USER_4, "ETAL_DABSeekWaitingTask : DAB seek awaked, event = 0x%x, vl_res = %d\n",
                vl_resEvent, vl_res);
#endif //#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)

        if ((vl_resEvent == ETAL_DAB_SEEK_NO_EVENT_FLAG) || (OSAL_ERROR_TIMEOUT_EXPIRED == vl_res))
        {
        ...
        * normal processing or event return flag contains EVENT_CMD.. so normal processing */
        else if (ETAL_DAB_SEEK_EVENT_KILL_FLAG == (ETAL_DAB_SEEK_EVENT_KILL_FLAG & vl_resEvent))
        {
        	/* Kill the DAB seek */
        	break;
        }
        ...
        else   if (ETAL_DAB_SEEK_EVENT_WAKEUP_FLAG == (ETAL_DAB_SEEK_EVENT_WAKEUP_FLAG & vl_resEvent))
        {
        	 ...
        	 */
        	ETAL_DABSeek_TaskClearEventFlag(ETAL_DAB_SEEK_EVENT_ALL_FLAG);
    ...
    if (OSAL_s32EventClose(DABSeek_TaskEventHandler) != OSAL_OK)
    {
        vl_res = OSAL_ERROR;
    }
    ...
    if (OSAL_s32EventDelete((tCString)"DABSeek_EventHandler") != OSAL_OK)
    {
        vl_res = OSAL_ERROR;
    }
#else
    if (OSAL_s32EventFree(DABSeek_TaskEventHandler) != OSAL_OK)
    {
        vl_res = OSAL_ERROR;
    }
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)
    ETALTML_PRINTF(TR_LEVEL_USER_4, "ETAL_DABSeekThreadEntry :  DAB seek awaked\n");
#endif //#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)
...
#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)
        ETALTML_PRINTF(TR_LEVEL_USER_4, "ETAL_DABSeekThreadEntry :  awaked, event = 0x%x, vl_res = %d\n",
            vl_resEvent, vl_res);
#endif //#if defined(CONFIG_TRACE_CLASS_ETAL) && (CONFIG_TRACE_CLASS_ETAL >= TR_LEVEL_USER_4)

        /* If this is for sleeping, leave this */
        if (ETAL_DAB_SEEK_EVENT_SLEEP_FLAG == (vl_resEvent & ETAL_DAB_SEEK_EVENT_SLEEP_FLAG))
        {
        	ETAL_DABSeek_TaskClearEventFlag(ETAL_DAB_SEEK_EVENT_SLEEP_FLAG);
            break;
        }
        else if ((vl_resEvent == ETAL_DAB_SEEK_NO_EVENT_FLAG) ||
                 (vl_res == OSAL_ERROR_TIMEOUT_EXPIRED))
        {
            ...
            for (i = 0; i < ETAL_MAX_RECEIVERS; i++)
            {
                ...
                if (ETAL_receiverIsValidHandle(hReceiver))
                {
                    ...
                    OSAL_s32SemaphoreWait(etalDABSeekSem, OSAL_C_TIMEOUT_FOREVER);
                    ETAL_DABSeekMainLoop(hReceiver);
                    OSAL_s32SemaphorePost(etalDABSeekSem);
...
static tVoid ETAL_DABSeekMainLoop(ETAL_HANDLE hReceiver)
{
    ...
    {
#ifdef DEBUG_SEEK
		ETAL_tracePrintUser1(TR_CLASS_APP_ETAL_API, "state: %d\n",seekCfgp->state);
#endif
		switch(seekCfgp->state)
		{
            	...
            	seekCfgp->noiseFloor = NOISEFLOOR_INIT;
             	...
             	/* tune to frequency 0 */
          		...
          		if ((retval != OSAL_OK) && (retval != OSAL_ERROR_TIMEOUT_EXPIRED))
          		{
	                ...
	                ETALTML_stop_and_send_event(hReceiver, seekCfgp, ETAL_SEEK_ERROR_ON_START);
          		...
          		{
#ifdef DEBUG_SEEK
          			ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "DAB TUNED freq: 0!!!\n");
#endif
             ...
             case ETAL_DABSEEK_FST:
            	 ...
            	 {
            		 /* Stop seek and send event */
            		 ETALTML_stop_and_send_event(hReceiver, seekCfgp, ETAL_SEEK_ERROR);
            	 ...
            	 else
            	 {
            		 ...
            		 {
            			 /* Build seek event */
            			 ETALTML_InitAutoSeekStatus(ETAL_receiverGetStandard(hReceiver), seekStatusp);
                         ...
                         else
                         {
                        	 ...
                        	 else
                        	 {
                        		 if(seekStatusp->m_quality.EtalQualityEntries.dab.m_RFFieldStrength < seekCfgp->noiseFloor)
                        		 {
                        			 ...
                        			 ETALTML_Report_Seek_Info(hReceiver, seekStatusp);
                        		 }
                        		 else
                        		 {
                        			 if(seekStatusp->m_quality.EtalQualityEntries.dab.m_RFFieldStrength > seekCfgp->seekThr)
                        			 {
                        				 if (ETAL_intCbIsRegistered(callAtDABAutonotification, ETAL_SeekIntCbFuncDabSync, hReceiver) == FALSE)
                        				 {
                        					 if(ETAL_intCbRegister(callAtDABAutonotification, ETAL_SeekIntCbFuncDabSync, hReceiver, ETAL_INTCB_CONTEXT_UNUSED) == ETAL_RET_SUCCESS)
                        					 {
                            					 ...
                            					 {
#ifdef DEBUG_SEEK
                            						 ETAL_tracePrintSystem(TR_CLASS_APP_ETAL_API, "DAB TUNED freq: %d!!!\n", freq);
#endif
                            						 seekCfgp->time = OSAL_ClockGetElapsedTime();
             ...
             case ETAL_DABSEEK_STOP:
            	 ...
            	 /* Stop seek and send event */
            	 ETALTML_stop_and_send_event(hReceiver, seekCfgp, seekStatusp->m_status);
             ...
             default:
            	 ETAL_tracePrintError(TR_CLASS_APP_ETAL_API, "ETAL_DABSeekMainLoop wrong state: %d", seekCfgp->state);
                 ASSERT_ON_DEBUGGING(0);
            	 break;
         }
    }
...
/***************************
 *
 * ETALTML_SeekStatusPeriodicFuncInternal
 *
 **************************/
	                ...
	                NULL, &(seekStatusp->m_frequency), &(seekStatusp->m_quality)) == OSAL_OK)
	        {
	            ...
	            if((seekStatusp->m_frequencyFound == true) || (seekStatusp->m_fullCycleReached == true))
	            {
	                ...
	                if(seekCfgp->exitSeekAction == cmdAudioUnmuted)
	                {
	                    ...
	                    ETALTML_stop_and_send_event(hReceiver, seekCfgp, ETAL_SEEK_FINISHED);
	                }
	                else
	                {
	                    /* Send event and notify */
...
ETAL_STATUS ETALTML_seek_start_internal(ETAL_HANDLE hReceiver, etalSeekDirectionTy direction, tU32 step, etalSeekAudioTy exitSeekAction, etalSeekHdModeTy seekHDSPS, tBool updateStopFrequency)
{
	...
	{
 		if (ETALTML_checkSeekStartParameters(hReceiver, direction, step, exitSeekAction, seekHDSPS) != ETAL_RET_SUCCESS)
		{
		...
		else
		{
			...
			if (need_tune)
			{
				...
				if (ETAL_CMDROUTE_TO_STAR(device_list))
				{
					...
					{
		                if (ETAL_intCbIsRegisteredPeriodic(ETALTML_SeekStatusPeriodicFuncInternal, hReceiver) == FALSE)
		                {
		                	if((ret = ETAL_intCbRegisterPeriodic(ETALTML_SeekStatusPeriodicFuncInternal, hReceiver, ETAL_SEEK_INTCB_DELAY)) != ETAL_RET_SUCCESS)
		                	{
		                		ret = ETAL_RET_ERROR;
		                	}
		                }
...
#endif // #if defined (CONFIG_ETAL_SUPPORT_CMOST_STAR) || defined (CONFIG_ETAL_SUPPORT_DCOP_MDR)
...
/***************************
 *
 * ETALTML_seek_continue_internal
 *
 **************************/
ETAL_STATUS ETALTML_seek_continue_internal(ETAL_HANDLE hReceiver)
{
	...
	else if (false == ETAL_receiverIsSpecialInProgress(hReceiver, cmdSpecialSeek))
	{
		ETAL_tracePrintError(TR_CLASS_APP_ETAL_CMD, "ETALTML_seek_continue_internal seek not in progress");
		ret = ETAL_RET_ERROR;
	}
	// if a callback is registered it means the seek on the cmost side is ongoing
	// so continue is not valid
	else if (ETAL_intCbIsRegisteredPeriodic(ETALTML_SeekStatusPeriodicFuncInternal, hReceiver) == TRUE)
	{
		ETAL_tracePrintError(TR_CLASS_APP_ETAL_CMD, "ETALTML_seek_continue_internal seek on-going already");
		ret = ETAL_RET_ERROR;
	}
	else if (!ETAL_receiverSupportsQuality(hReceiver))
	{
	...
	{
        if (ETALTML_checkSeekContinueParameters(hReceiver) != ETAL_RET_SUCCESS)
        {
            ret = ETAL_RET_PARAMETER_ERR;
        }
        else
        {
            ...
            if (ETAL_CMDROUTE_TO_STAR(device_list))
            {
                ...
                if (ETAL_cmdSeekStart_CMOST(hReceiver, /*unused*/seekCfgp->direction, /*unused*/seekCfgp->step, cmdContinue, /*unused*/seekCfgp->exitSeekAction, /*unused*/seekCfgp->updateStopFrequency, /*unused*/FALSE) == OSAL_OK)
                {
	                    if((ret = ETAL_intCbRegisterPeriodic(ETALTML_SeekStatusPeriodicFuncInternal, hReceiver, ETAL_SEEK_INTCB_DELAY)) != ETAL_RET_SUCCESS)
	                    {
	                    	ETAL_tracePrintError(TR_CLASS_APP_ETAL_CMD, "ETALTML_seek_continue_internal could not register call back");
	                        ret = ETAL_RET_ERROR;
	                    }
                }
                else
...
ETAL_STATUS ETALTML_seek_stop_internal(ETAL_HANDLE hReceiver, EtalSeekTerminationModeTy terminationMode)
{
    ...
    {
        if (ETALTML_checkSeekStopParameters(hReceiver, terminationMode) != ETAL_RET_SUCCESS)
        {
            ret = ETAL_RET_PARAMETER_ERR;
        }
        else

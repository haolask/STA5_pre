...
#define SDI_FIFO_SIZE (SDI_FIFO_BURST_SIZE * sizeof(u32))
...
static int read_bytes(struct mmc *dev, u32 *dest, u32 blkcount, u32 blksize)
{
	...
	while ((!status_err) && (xfercount >= sizeof(u32))) {
		if ((status & SDI_STA_RXFIFOBR) && xfercount >= SDI_FIFO_SIZE) {
			tempbuff = mmc_fast_read(tempbuff, &host->base->fifo);
			xfercount -= SDI_FIFO_SIZE;
		}
		else if (status & SDI_STA_RXDAVL) {
...
static int write_bytes(struct mmc *dev, u32 *src, u32 blkcount, u32 blksize)
{
	...
	while (!status_err && xfercount) {
		if (status & SDI_STA_TXFIFOBW) {
			if (xfercount >= SDI_FIFO_SIZE) {
				tempbuff = mmc_fast_write(tempbuff,
						&host->base->fifo);
				xfercount -= SDI_FIFO_SIZE;
			} else {
			...
			struct mmc_data *data)
{
	...
	if (host->hsem)
		while (!hsem_trylock(host->hsem, 0))
			;
	if (data)
...
static int mmc_host_reset(struct mmc *dev)
{
	...
	debug("%s: Init start, base: %X\n", __func__, (uint32_t)host->base);
	if (host->hsem)
		while (!hsem_trylock(host->hsem, 0))
			; /* Take HSEM will be released at init end (set_ios) */
...
static int  host_set_ios(struct mmc *dev)
{
	...
	/* End of init ? */
	if (dev->clock > 5000000 && dev->bus_width > 1) {
		if (host->hsem)
			hsem_unlock(host->hsem);
		debug("%s: Init end, base: %X\n", __func__, (uint32_t)host->base);
	}

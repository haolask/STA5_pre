				 ...
				 uint8_t *buf, int oob_required, int page)
{
	...
	for (i = 0, s = 0; s < eccsteps; s++, i += eccbytes, p += eccsize) {
		...
		if ((uint32_t)p & 3 || eccsize != 512) {
			chip->read_buf(mtd, p, eccsize);
		} else {
			nand_read_fast_512(
				mtd_to_nand(mtd)->IO_ADDR_R,
				(uint32_t *)p);
		}
		...
		/* Jump back to data */
		if (s != (eccsteps - 1))
			chip->cmdfunc(mtd, NAND_CMD_RNDOUT, (s+1)*eccsize, -1);
...
static int fsmc_nand_ready(struct mtd_info *mtd)
{
...
static void fsmc_select_chip(struct mtd_info *mtd, int chip)
{
	/* Do nothing: avoid deselecting FSMC for M3 concurrent access */
}
...
#error Please define CONFIG_SYS_FSMC_NAND_16BIT or CONFIG_SYS_FSMC_NAND_8BIT
	...
	writel(readl(&fsmc_regs_p->pc) | FSMC_TCLR_1 | FSMC_TAR_1,
			&fsmc_regs_p->pc);
	writel(FSMC_THIZ | FSMC_THOLD | FSMC_TWAIT | FSMC_TSET,
	       &fsmc_regs_p->comm);
	writel(FSMC_THIZ | FSMC_THOLD | FSMC_TWAIT | FSMC_TSET,
	       &fsmc_regs_p->attrib);
	...
	nand->badblockbits = 7;
	nand->dev_ready = fsmc_nand_ready;

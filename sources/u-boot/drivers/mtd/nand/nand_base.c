...
#define iowrite8_rep	__raw_writesb
#define iowrite16_rep	__raw_writesw
#define ioread8_rep	__raw_readsb
#define ioread16_rep	__raw_readsw
...
void nand_write_buf(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	...
	if ((uint32_t)buf & 3 || len != 512) {
		iowrite8_rep(chip->IO_ADDR_W, buf, len);
	} else {
		nand_write_fast_512(chip->IO_ADDR_W, (uint32_t *)buf);
	}
}
			    ...
			    int column, int page_addr)
{
	...
	/* Manage READ command with or without cache */
	if (command == NAND_CMD_READ0) {
		if (!chip->start_cache_reading && chip->has_readcache) {
			command = NAND_CMD_READCACHE;
		} else {
			command = NAND_CMD_READSTART;
		}
	}
	...
	/* Command latch cycle */
	chip->cmd_ctrl(mtd, (command == NAND_CMD_READCACHE
				|| command == NAND_CMD_READSTART)
					? NAND_CMD_READ0 : command,
			NAND_NCE | NAND_CLE | NAND_CTRL_CHANGE);

	pr_info("CMD: %02X", command);

	if ( (command != NAND_CMD_READCACHE) &&
			(column != -1 || page_addr != -1)) {
		int ctrl = NAND_CTRL_CHANGE | NAND_NCE | NAND_ALE;

		/* Serially input address */
		if (column != -1) {
			pr_info(", Col: %d", column);
			/* Adjust columns for 16 bit buswidth */
			if (chip->options & NAND_BUSWIDTH_16 &&
					!nand_opcode_8bits(command))
				column >>= 1;
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	if (chip->has_readcache)
		chip->start_cache_reading = true;
...
read_retry:
			/* Cross block boundary => restart page cache */
			if (chip->has_readcache && !(page & blockmask))
				chip->start_cache_reading = 1;

			if (chip->start_cache_reading) {
				chip->cmdfunc(mtd, NAND_CMD_READ0, 0x00, page);
				chip->start_cache_reading = 0;
			}
	...
	if (chip->has_readcache)
		chip->cmdfunc(mtd, NAND_CMD_READCACHE_END, -1, -1);
					...
					int *busw)
{
	...
	if (le16_to_cpu(p->opt_cmd) & ONFI_OPT_CMD_READ_CACHE_CMDS) {
		chip->has_readcache = true;
		pr_info("READ CACHE commands supported\n");
	}
...
ident_done:
#ifdef CONFIG_STA
	/* STA Accordo5 Cut3.0 hack for NAND Large Page support */
	if ((chip->options & NAND_BROKEN_LP) && mtd->writesize > 2048) {
		/* Hack page size to 2048 */
		mtd->writesize = 2048;
		printf("Nand: Apply STA A5 Cut3 workaround\n");
	}
#endif /* CONFIG_STA */

...
static char *nc_stdio_getbuf(struct stdio_dev *dev, char *s, int sz)
{
	char *start = s;
	int l;
	...
	while (sz) {
		...
		if (input_offset + l <= sizeof(input_buffer)) {
			memcpy(s, &input_buffer[input_offset], l);
			input_offset += l;
			if (input_offset >= sizeof(input_buffer))
				input_offset -= sizeof(input_buffer);
		} else {
			int le = sizeof(input_buffer) - input_offset;
			memcpy(s, &input_buffer[input_offset], le);
			memcpy(s + le, &input_buffer[0], l - le);
			input_offset = l - le;
		}

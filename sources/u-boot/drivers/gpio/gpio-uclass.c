...
int dm_gpio_set_af(struct gpio_desc *desc, int alf_func)
{
	int ret;

	ret = check_reserved(desc, "set_af");
	if (ret)
		return ret;

	gpio_get_ops(desc->dev)->set_af(desc->dev, desc->offset, alf_func);
	return 0;
}
...
int gpio_set_af(unsigned gpio, int alf_func)
{
	...
	if (ret)
		return ret;
	return dm_gpio_set_af(&desc, alf_func);
}

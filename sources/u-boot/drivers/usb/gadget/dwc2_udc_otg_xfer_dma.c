...
static void complete_rx(struct dwc2_udc *dev, u8 ep_num)
{
	...
	/* in complete_rx fix condition to start again a new dma rx transfer */
	if (req->req.actual < req->req.length  && (xfer_size == ep->len)) {
		setdma_rx(ep, req);
	} else {
		debug_cond(DEBUG_OUT_EP != 0,
...
static void process_ep_out_intr(struct dwc2_udc *dev)
{
	...
	while (ep_intr) {
		if (ep_intr & 0x1) {
			...
			if (ep_num == 0) {
				if (ep_intr_status & STATUS_PHASE_RECEIVED) {
					dev->ep0state = WAIT_FOR_SETUP;
					dwc2_udc_pre_setup();
					continue;
				}

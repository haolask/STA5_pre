...
static char *_serial_getbuf(struct udevice *dev, char *str, int sz)
{
	struct dm_serial_ops *ops = serial_get_ops(dev);
	int err = 0;
	char *p = str;
...
char *serial_getbuf(char *s, int sz)
{
	if (!gd->cur_serial_dev)
		return 0;

	return _serial_getbuf(gd->cur_serial_dev, s, sz);
}
...
char *serial_stub_getbuf(struct stdio_dev *sdev, char *str, int sz)
{
	return _serial_getbuf(sdev->priv, str, sz);
}
...
static int serial_post_probe(struct udevice *dev)
{
	...
	sdev.getbuf = serial_stub_getbuf;

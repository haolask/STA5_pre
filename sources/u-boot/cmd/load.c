...
static ulong load_serial_raw(ulong offset);
			      ...
			      char * const argv[])
{
	...
	} else if (strcmp(argv[0], "loadr") == 0) {
		printf("## Ready for binary (raw) download to 0x%08lX...\n",
		       offset);

		addr = load_serial_raw(offset);
...
static ulong load_serial_raw(ulong offset)
{
	...
	for (;;) {
		...
		if (getc() == 'L') {
			len = (getc() << 8) + (uchar) getc();
			getbuf(p, len);
			putc('O'); /* Ack */
			size += len;
			p += len;
		} else {
			break; /* Protocol error or End transfert */
		}
	}
...
U_BOOT_CMD(
	loadr, 3, 0,	do_load_serial_bin,
	"load binary file over usb line (raw mode)",
	"[ off ]\n"
	"    - load binary file over usb line"
	" with offset 'off'"
);

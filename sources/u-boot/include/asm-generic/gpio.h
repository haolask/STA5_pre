...
enum gpio_func_t {
	...
	GPIOF_GPIO,		/* used as GPIO Input or Output */
	GPIOF_FUNC_A,		/* Not used as a GPIO but ALT_A function */
	GPIOF_FUNC_B,		/* Not used as a GPIO but ALT_B function */
	GPIOF_FUNC_C,		/* Not used as a GPIO but ALT_B function */

	GPIOF_COUNT,
};

/**
 * gpio_set_af() - [COMPAT] Configure ST alternate function on GPIO pin
 * gpio:	GPIO number
 * value:	ST Alternate function GPIOF_GPIO, GPIOF_FUNC_[ABC] to set
 *
 * This function implements the API that's compatible with current
 * GPIO API used in U-Boot. The request is forwarded to particular
 * GPIO driver. Returns 0 on success, negative value on error.
 */
int gpio_set_af(unsigned gpio, int alf_func);
...
struct dm_gpio_ops {
	...
	/**
	 * set_af() Set the GPIO ST specific alternate function
	 *
	 * @dev:     Device to check
	 * @offset:  GPIO offset within that device
	 * @function:  GPIO function to set - GPIOF_GPIO, GPIOF_FUNC_*
	 * @return status
	 */
	int (*set_af)(struct udevice *dev, unsigned offset, int function);

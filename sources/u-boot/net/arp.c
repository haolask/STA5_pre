...
int arp_timeout_check(void)
{
	...
	if ((t - arp_wait_timer_start) > ARP_TIMEOUT) {
		...
		if (getenv("arp_timeout_count"))
			arp_timeout_count = simple_strtol(
				getenv("arp_timeout_count"),
				NULL, 10) * CONFIG_SYS_HZ;

		if ((arp_timeout_count > 0) &&
		    (arp_wait_try >= arp_timeout_count)) {

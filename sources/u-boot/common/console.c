...
static char *console_getbuf(int file, char *s, const int sz)
{
	if (stdio_devices[file]->getbuf != NULL) {
		return stdio_devices[file]->getbuf(stdio_devices[file], s, sz);
	} else {
		char *p = s;
		char *end = s + sz;
		int (*getc)(struct stdio_dev *dev);

		getc = stdio_devices[file]->getc;

		while (p < end)
			*p++ = (char)getc(stdio_devices[file]);
		return s;
	}
}
...
char *getbuf(char *s, int sz)
{
	...
	if (gd->flags & GD_FLG_DEVINIT) {
		/* Get from the standard input */
		return console_getbuf(stdin, s, sz);
	}

	/* Send directly to the handler */
	return serial_getbuf(s, sz);
}

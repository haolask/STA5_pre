...
/**
 * @brief        Change exposed API service set
 * @details      can be used to remove a service, add or replace a service
 *
 * @param[in]
 *
 * @return       Error code
 * @retval       CSE_NO_ERR     When setup was done
 * @retval       CSE_GENERAL_ERR
 *
 */
extern uint32_t cse_ext_update_service_set(uint32_t update_descriptor_address,
					   uint32_t update_descriptor_size);

 ...
 */
extern uint32_t cse_trng_get_rand(uint8_t *random);
 ...
 *
 * @param[in]     *entropy_buffer	pointer to input entropy buffer to use
 *					for PRNG re-seeding (128bits)
 *
 * @return        Error code
 ...
 *
 */
extern uint32_t cse_extend_prng_seed(uint8_t *entropy_buffer);
 ...
 *
 */
extern uint32_t cse_init_prng(void);
 ...
 *
 */
extern uint32_t cse_prng_get_rand(uint8_t *random);
 ...
 *
 */
extern uint32_t cse_trng_online_test(uint32_t *passed);

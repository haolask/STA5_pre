 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_cust_cert_vers(t_uint32  *pver);

/*!
 * @brief	otp_get_secr_vers:
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_secr_vers(t_uint32  *pver);
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_boot_param(volatile struct t_safemem_boot_cfg *pinfo);

/*!
 * @brief	otp_get_cust_certif_hash_info
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_cust_certif_hash_info(uint32_t *proot_hash_info);

/*!
 * @brief	otp_get_cust_certif_hash
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_cust_certif_hash(uint8_t *phash);
...
/*!
 * @brief	otp_incr_cust_certif_vers
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_incr_cust_certif_vers(void);
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_incr_security_vers(void);

/*!
 * @brief	otp_set_boot_param
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_set_boot_param(volatile struct t_safemem_boot_cfg info);

/*!
 * @brief	otp_set_cust_certif_hash_info
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_set_cust_certif_hash_info(uint32_t root_hash_info);

/*!
 * @brief	otp_set_cust_certif_hash
 * @details	This routine first check if the customer root certificate hash
 *		has been written and enabled
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_set_cust_certif_hash(uint8_t *phash);

/*!
 * @brief	otp_write_enc_boot_key
 * @details	This routine writes the secure boot key and configuration bits
 ...
 * @return	: OTP error condition
 * @retval	: OTP_FAILURE - if secure boot key was not written already,
 *		or errors while accessing OTP
 * @retval	: OTP_SUCCESS
 */
extern int otp_write_enc_boot_key(uint32_t bit_size, uint8_t *pSecBootKey);

/*!
 * @brief	otp_set_ecc_public_word
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_set_ecc_public_word(uint32_t word_addr);

/*!
 * @brief	otp_get_ecc_public_word
 ...
 * @retval	: OTP_SUCCESS
 */
extern int otp_get_ecc_public_word(uint32_t word_addr, uint32_t *p_ecc_set);

/*!
 * @brief	otp_set_monotonic_counter_type:
 ...
 * @retval	: OTP_SUCCESS
 */
extern int  otp_set_monotonic_counter_type(t_uint32 type);

/*!
 * @brief	otp_get_monotonic_counter_type:
 ...
 * @retval	:  OTP_SUCCESS
 */
extern int otp_get_monotonic_counter_type(t_uint32 *ptype);
...
/*!
 * @brief	otp_generate_CSK
 * @details	This routine let the HSM generate and store Chip Secret Key
 *		- if not already done
 * @return	: OTP error condition
 * @retval	: OTP_FAILURE - if error while generating of writing the key
 * @retva	: OTP_SUCCESS
 */
extern int otp_generate_CSK(void);
/**
 * @}
 * @}
 */

...
extern uint32_t cse_read_fw_version(uint32_t *fw_version);
 ...
 */
extern uint32_t cse_get_id(uint32_t *challenge_ad, uint32_t *UID,
 ...
 *
 * @param[out]     *fw_id	pointer to the buffer where UID will be copied
 *
 * @return         Error code
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_get_fw_id(uint32_t *fw_id);
 ...
 *		   not enabled.
 *
 * @param[out]     *fw_version	pointer to the buffer where FW version value
 *				will be copied (32bits)
 *
 * @return         Error code
 ...
 */
extern uint32_t cse_init_cse(uint32_t *fw_version);
#endif
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_secure_boot(uint32_t size, uint32_t *start,
				uint32_t *fw_version);
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_boot_failure(void);
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_boot_OK(void);
#endif
 ...
 * @details	   This step is required before calling the cse_debug_auth
 *		   command
 *
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_debug_challenge(uint32_t *challenge);
 ...
 *
 * @param[in]      *authorization	pointer to the authorization message
 *					computed from the challenge provided by
 *					cse_debug_challenge command
 *
 * @return         Error code
 * @retval 0	   When command is successful
 * @retval 1..21   In case of error - the error code values are the CSE returned
 ...
 * @note           The function is blocking and waits for operation completion
 */
extern uint32_t cse_debug_auth(uint32_t *authorization);

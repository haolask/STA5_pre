 ...
 *		   --> need to retry
 */
uint32_t cse_trng_get_rand(uint8_t *random)
{
 ...
 *
 */
uint32_t cse_extend_prng_seed(uint8_t *entropy_buffer)
{
 ...
 *
 */
uint32_t cse_prng_get_rand(uint8_t *random)
{
	/* ask the CSE to write a random value to the specified location */
	uint32_t ret = CSE_NO_ERR;
...
uint32_t cse_trng_online_test(uint32_t *passed)
{
	uint32_t ret = CSE_NO_ERR;

 ...
 */
uint32_t cse_rsa_load_ramkey(struct rsa_simple_struct *pmodulus,
				  ...
				  struct rsa_pub_key_stt *ppub_key)
{
	...
	ret = cse_hal_send_cmd2p(CSE_RSA_EXPORT_PUBLIC_KEY,
				 (uint32_t)rsa_key_idx, (uint32_t)ppub_key);
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_rsa_load_key(uint8_t *M1, uint8_t *M2, uint8_t *M3,
			  uint8_t *M4, uint8_t *M5)
{
	/*
 ...
 * @note	The function is blocking and waits for operation completion
 */
uint32_t cse_rsa_pkcs1_15_sign(vuint32_t rsa_key_idx, vuint8_t *psrc,
			       ...
			       vuint32_t hash_type)
{
	...
	ret = cse_hal_send_cmd5p(CSE_RSA_PKCS1_15_SIGN, (uint32_t)rsa_key_idx,
				 (uint32_t)psrc, (uint32_t)byte_size,
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_rsa_pkcs1_15_verify(vuint32_t rsa_key_idx, vuint8_t *psrc,
				 vuint32_t byte_size, vuint8_t *psign,
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_rsa_pkcs1_15_encrypt(vuint32_t rsa_key_idx, vuint8_t *psrc,
				  ...
				  vuint32_t *pdst_size)
{
	...
	ret = cse_hal_send_cmd5p(CSE_RSA_PKCS1_15_ENCRYPT,
				 (uint32_t)rsa_key_idx, (uint32_t)psrc,
				 (uint32_t)src_size, (uint32_t)pdst,
				 (uint32_t)(*pdst_size));
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_rsa_pkcs1_15_decrypt(vuint32_t rsa_key_idx, vuint8_t *psrc,
				  vuint32_t src_size, vuint8_t *pdst,
				  vuint32_t *pdst_size)
{
	/*

 ...
 */
uint32_t cse_load_ramkey(uint8_t *key_pt)
{
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_export_ramkey(uint8_t *M1, uint8_t *M2, uint8_t *M3, uint8_t *M4,
			   uint8_t *M5)
{
	/*
	...
	ret = cse_hal_send_cmd5p(CSE_EXPORT_RAM_KEY, (uint32_t)M1, (uint32_t)M2,
 ...
 * @note           The function is blocking and waits for operation completion
 */
uint32_t cse_load_key(uint8_t *M1, uint8_t *M2, uint8_t *M3, uint8_t *M4,

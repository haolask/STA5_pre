				  ...
				  const unsigned char *payload)
{
	...
	if (drvdata->use_processor_id)
		channel += smp_processor_id();
...
static void stm_init_generic_data(struct stm_drvdata *drvdata)
{
	...
	drvdata->use_processor_id = false;

	if (of_property_read_bool(drvdata->dev->of_node, "st,stm"))
		drvdata->use_processor_id = true;
}

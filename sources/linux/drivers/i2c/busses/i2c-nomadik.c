...
static const struct of_device_id i2c_nomadik_of_match[] = {
...
static struct amba_driver nmk_i2c_driver = {
	.drv = {
		...
		.name = DRIVER_NAME,
		.pm = &nmk_i2c_pm,
		.of_match_table = i2c_nomadik_of_match,
	},
...
static int of_i2c_nmk_notify(struct notifier_block *nb, unsigned long action,
			     void *arg)
{
	struct of_reconfig_data *rd = arg;
	struct device_node *node = rd->dn;
	struct i2c_adapter *adap;
	struct nmk_i2c_dev *dev;

	switch (of_reconfig_get_state_change(action, rd)) {
	case OF_RECONFIG_CHANGE_ADD:
		if (!of_device_is_compatible(node, "st,nomadik-i2c"))
			return NOTIFY_OK;

		pr_info("of_i2c_nmk_notify: OF_RECONFIG_CHANGE_ADD: %s\n",
			node->name);

		if (of_platform_bus_probe(node, i2c_nomadik_of_match, NULL))
			pr_err("i2c-nomadik: failed bus probe reconfig\n");
	...
	case OF_RECONFIG_CHANGE_REMOVE:
		...
		pr_info("of_i2c_nmk_notify: OF_RECONFIG_CHANGE_REMOVE: %s\n",
			node->name);

		adap = of_find_i2c_adapter_by_node(node);
		if (!adap) {
			pr_err(" i2c-nomadik: adapt is NULL - %s - %s\n",
		...
		if (!dev) {
			pr_err(" i2c-nomadik: nmk_i2c_dev is NULL\n");
			break;
		}

		put_device(&adap->dev);
		amba_device_unregister(dev->adev);
	...
	default:
		...
		pr_info("of_i2c_nmk_notify: OTHER: %s\n", node->name);
		break;
	}
...
static struct notifier_block i2c_nmk_of_notifier = {
	.notifier_call = of_i2c_nmk_notify,
	.priority = 1,
};
#endif
...
{
	int ret = amba_driver_register(&nmk_i2c_driver);

	if (IS_ENABLED(CONFIG_OF_DYNAMIC))
		WARN_ON(of_reconfig_notifier_register(&i2c_nmk_of_notifier));

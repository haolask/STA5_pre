...
u32 VC1RunAsic(decContainer_t *pDecCont, strmData_t *pStrm, u32 strmBusAddr)
{
    ...
    if (!pDecCont->asicRunning)
    {
        ...
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 0x4, 0);

        for (i = 2; i < DEC_X170_REGISTERS; i++)
        {
            DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4*i,
                        pDecCont->vc1Regs[i]);
    ...
    else /* buffer was empty and now we restart with new stream values */
    {
        ...
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4 * 5,
                    pDecCont->vc1Regs[5]);
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4 * 6,
                    pDecCont->vc1Regs[6]);
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4 * 12,
                    pDecCont->vc1Regs[12]);
    ...
    {
        pDecCont->vc1Regs[i] = DWLReadReg(pDecCont->dwl, pDecCont->coreID, 4*i);
    }
    ...
    else
    {
        ...
        SetDecRegister(pDecCont->vc1Regs, HWIF_DEC_E, 0);
...
void SetPp(decContainer_t *pDecCont)
{
    ...
    /* Connect PP output buffer to decoder output buffer */
    {
        ...
        pDecCont->PPBufferData(pDecCont->ppInstance, 
            pDecCont->ppControl.bufferIndex, luma, chroma, botLuma, botChroma);
    }
     ...
     * and not 0x07.
     */
    pDecCont->ppControl.croppedW = ((pPic[ppIdx].codedWidth+15) & ~15);
    pDecCont->ppControl.croppedH = ((pPic[ppIdx].codedHeight+15) & ~15);
         ...
         (pDecCont->ppControl.picStruct == DECPP_PIC_BOT_FIELD_FRAME) )
    {
        ...
        pDecCont->ppControl.croppedH=((pDecCont->ppControl.croppedH + 15) & ~15);
    }
    pDecCont->ppControl.vc1AdvEnable =
            (pDecCont->storage.profile == VC1_ADVANCED) ? 1 : 0;
...
void SetReferenceBaseAddress(decContainer_t *pDecCont)
{
    ...
    if (pDecCont->storage.maxBframes)
    {
            ...
            if ( (pDecCont->storage.picLayer.fcm == FIELD_INTERLACE) )
            {
                ...
                else /* NUMREF == 1 */
                {
                    ...
                    if( pDecCont->storage.work0 == INVALID_ANCHOR_PICTURE )
                        tmp2 = pDecCont->storage.workOutPrev;
                    else
                        tmp2 = pDecCont->storage.work0;
        ...
        SetDecRegister(pDecCont->vc1Regs, HWIF_REFER2_BASE,
            (u32)(pDecCont->storage.pPicBuf[tmp2].
                  data.busAddress));

                     ...
                     void * userCtx)
{
    ...
    *pDecInst = NULL;
    ...
    pDecCont->StrmStorage.unsupportedFeaturesPresent = 0;

    for(i = 0; i < DEC_X170_REGISTERS; i++)
        pDecCont->avsRegs[i] = 0;

    SetCommonConfigRegs(pDecCont->avsRegs);
    ...
    AVSDEC_DEBUG(("AVS Plus supported: %s\n", config.avsPlusSupport? "YES" : "NO"));
    pDecCont->avsPlusSupport = config.avsPlusSupport;
    ...
    if(referenceFrameFormat == DEC_REF_FRM_TILED_DEFAULT)
    {
        ...
        {
            AVSDEC_DEBUG(("AVSDecInit# ERROR: ERROR: Format not supported\n"));
            DWLfree(pDecCont);
            (void)DWLRelease(dwl);
            return AVSDEC_FORMAT_NOT_SUPPORTED;
        }
    ...
    pDecCont->StrmStorage.intraFreeze = errorHandling == DEC_EC_VIDEO_FREEZE;

    if (errorHandling == DEC_EC_PARTIAL_FREEZE)
        pDecCont->StrmStorage.partialFreeze = 1;
    else if (errorHandling == DEC_EC_PARTIAL_IGNORE)
        pDecCont->StrmStorage.partialFreeze = 2;
 ...
 */
    ...
    do
    {
        ...
        if(API_STOR.DecStat == HW_PIC_STARTED)
        {
            ...
            if (!pDecCont->asicRunning && pDecCont->StrmStorage.partialFreeze)
            {
            ...
            else if(asicStatus & DEC_8190_IRQ_ABORT)
            {
                ...
                errorConcealment = 1;
                     ...
                     (asicStatus & AVS_DEC_X170_IRQ_TIMEOUT) )
            {
                if (!pDecCont->StrmStorage.partialFreeze ||
                    !ProcessPartialFreeze(
                        (u8*)pDecCont->StrmStorage.pPicBuf[
                             pDecCont->StrmStorage.workOut].data.virtualAddress,
                        pDecCont->StrmStorage.work0 != INVALID_ANCHOR_PICTURE ?
                            (u8*)pDecCont->StrmStorage.pPicBuf[
                              pDecCont->StrmStorage.work0].data.virtualAddress :
                             NULL,
                        pDecCont->StrmStorage.frameWidth,
                        pDecCont->StrmStorage.frameHeight,
                        pDecCont->StrmStorage.partialFreeze == 1))
            ...
            /* HW finished decoding a picture */
            if(asicStatus & AVS_DEC_X170_IRQ_DEC_RDY)
            {
...
void AvsDecRelease(AvsDecInst decInst)
{
    ...
    {
        DWLDisableHW(pDecCont->dwl, pDecCont->coreID, 4 * 1, 0);
...
void AvsRefreshRegs(DecContainer * pDecCont)
{
    ...
    for(i = 0; i < DEC_X170_REGISTERS; i++)
    {
        ppRegs[i] = DWLReadReg(pDecCont->dwl, pDecCont->coreID, 4 * i);
    }
}
...
void AvsFlushRegs(DecContainer * pDecCont)
{
    ...
    {
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4 * i, ppRegs[i]);
        ppRegs[i] = 0;
    }
}
...
u32 RunDecoderAsic(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    if(!pDecContainer->asicRunning)
    {
        ...
        if (!pDecContainer->keepHwReserved)
            (void) DWLReserveHw(pDecContainer->dwl, &pDecContainer->coreID);
        ...
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 0x4, 0);

        AvsFlushRegs(pDecContainer);

        /* Enable HW */
        SetDecRegister(pDecContainer->avsRegs, HWIF_DEC_E, 1);
        DWLEnableHW(pDecContainer->dwl, pDecContainer->coreID,
                    4 * 1, pDecContainer->avsRegs[1]);
    }
    else    /* in the middle of decoding, continue decoding */
    {
        ...
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 5,
                    pDecContainer->avsRegs[5]);
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 6,
                    pDecContainer->avsRegs[6]);
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 12,
                    pDecContainer->avsRegs[12]);
    ...
    /* Wait for HW ready */
    ret = DWLWaitHwReady(pDecContainer->dwl, pDecContainer->coreID,
       ...
       (asicStatus == ID8170_DEC_SYSTEM_ERROR))
    {
        ...
        SetDecRegister(pDecContainer->avsRegs, HWIF_DEC_E, 0);

        DWLDisableHW(pDecContainer->dwl, pDecContainer->coreID, 4 * 1,
                     pDecContainer->avsRegs[1]);
        ...
        if (!pDecContainer->keepHwReserved)
            (void) DWLReleaseHw(pDecContainer->dwl, pDecContainer->coreID);
    }
        ...
        (AVS_DEC_X170_IRQ_BUFFER_EMPTY | AVS_DEC_X170_IRQ_DEC_RDY)))
    {
        ...
        if(((tmp - strmBusAddress) <= DEC_X170_MAX_STREAM) &&
           ((tmp - strmBusAddress) <= pDecContainer->StrmDesc.strmBuffSize))
        {
                               ...
                               DecContainer * pDecCont, u32 picIndex)
{
    ...
    pPicture->picCodingType = pPic[picIndex].picCodeType;
...
static u32 AvsSetRegs(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    /* AVS Plus stuff */
    SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_E,
        pDecContainer->Hdrs.weightingQuantFlag);
    SetDecRegister(pDecContainer->avsRegs, HWIF_AVS_AEC_E,
        pDecContainer->Hdrs.aecEnable);
    SetDecRegister(pDecContainer->avsRegs, HWIF_NO_FWD_REF_E,
        pDecContainer->Hdrs.noForwardReferenceFlag);
    SetDecRegister(pDecContainer->avsRegs, HWIF_PB_FIELD_ENHANCED_E,
        pDecContainer->Hdrs.pbFieldEnhancedFlag);

    if (pDecContainer->Hdrs.profileId == 0x48)
    {
        SetDecRegister(pDecContainer->avsRegs, HWIF_DEC_AVSP_ENA, 1);
    }
    else
    {
        SetDecRegister(pDecContainer->avsRegs, HWIF_DEC_AVSP_ENA, 0);
    }

    if(pDecContainer->Hdrs.weightingQuantFlag == 1 &&
        pDecContainer->Hdrs.chromaQuantParamDisable == 0x0)
    {
        SetDecRegister(pDecContainer->avsRegs, HWIF_QP_DELTA_CB,
            pDecContainer->Hdrs.chromaQuantParamDeltaCb);
        SetDecRegister(pDecContainer->avsRegs, HWIF_QP_DELTA_CR,
            pDecContainer->Hdrs.chromaQuantParamDeltaCr);
    }
    else
    {
        SetDecRegister(pDecContainer->avsRegs, HWIF_QP_DELTA_CB, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_QP_DELTA_CR, 0);
    }

    if(pDecContainer->Hdrs.weightingQuantFlag == 1)
    {
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_MODEL,
            pDecContainer->Hdrs.weightingQuantModel);

        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_0,
            pDecContainer->Hdrs.weightingQuantParam[0]);
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_1,
            pDecContainer->Hdrs.weightingQuantParam[1]);
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_2,
            pDecContainer->Hdrs.weightingQuantParam[2]);
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_3,
            pDecContainer->Hdrs.weightingQuantParam[3]);
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_4,
            pDecContainer->Hdrs.weightingQuantParam[4]);
        SetDecRegister(pDecContainer->avsRegs, HWIF_WEIGHT_QP_5,
            pDecContainer->Hdrs.weightingQuantParam[5]);
    }
    /* AVS Plus end */
    ...
    else
    {
        ...
        if(pDecContainer->dpbMode == DEC_DPB_FRAME )
        {
            SetDecRegister(pDecContainer->avsRegs, HWIF_DEC_OUT_BASE,
                           (pDecContainer->StrmStorage.pPicBuf[pDecContainer->
                                                               StrmStorage.workOut].
    ...
    if(pDecContainer->Hdrs.pictureStructure == FRAMEPICTURE)
    {
        if(pDecContainer->Hdrs.picCodingType == BFRAME)
        {
            ...
            tmp = pDecContainer->StrmStorage.future2prevPastDist;
            tmp = 16384/tmp;
        ...
        else
        {
            ...
            pDecContainer->StrmStorage.future2prevPastDist = tmp;
        ...
        /* AVS Plus stuff */
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_0, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_1, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_2, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_3, 0);

        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_0, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_1, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_2, 0);
        SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_3, 0);
        /* AVS Plus end */
    }
    else /* field interlaced */
    {
        if(pDecContainer->Hdrs.picCodingType == BFRAME)
        {
            ...
            /* AVS Plus stuff */
            if (pDecContainer->Hdrs.pbFieldEnhancedFlag &&
                !pDecContainer->ApiStorage.firstField)
            {
            ...
            if(pDecContainer->ApiStorage.firstField)
            {
                ...
                /* delta1 */
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_0, 2);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_1, 0);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_2, 2);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_COL_3, 0);

                /* deltaFw */
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_0, 2);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_1, 0);
                /* deltaBw */
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_2, 0);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_3, 0);
            ...
            else
            {
                ...
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_1, 0);
                /* deltaBw */
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_2,
                    (u32)-2);
                SetDecRegister(pDecContainer->avsRegs, HWIF_REF_DELTA_CUR_3,
                    (u32)-2);
            }
            /* AVS Plus end */
        }
    ...
    if (pDecContainer->Hdrs.picCodingType == PFRAME ||
        (pDecContainer->Hdrs.picCodingType == IFRAME /*&&
         !pDecContainer->ApiStorage.firstField*/)) /* AVS Plus change */
    {
    ...
    if (pDecContainer->ApiStorage.firstField ||
        ...
        SetDecRegister(pDecContainer->avsRegs, HWIF_DIR_MV_BASE,
            ...
            ( ( ( pDecContainer->StrmStorage.frameWidth *
              ...
              4 * sizeof(u32)));
    /* AVS Plus stuff */
    SetDecRegister(pDecContainer->avsRegs, HWIF_DIR_MV_BASE2,
        pDecContainer->StrmStorage.directMvs.busAddress);
    /* AVS Plus end */
...
void AvsPpControl(DecContainer * pDecContainer, u32 pipelineOff)
{
    ...
    /* Connect PP output buffer to decoder output buffer */
    {
        ...
        if(pDecContainer->dpbMode == DEC_DPB_FRAME )
        {
            botLuma = luma + (pDecContainer->StrmStorage.frameWidth * 16);
            botChroma = chroma + (pDecContainer->StrmStorage.frameWidth * 16);
        }
        else if( pDecContainer->dpbMode == DEC_DPB_INTERLACED_FIELD )
        {
            botLuma = luma + ((pDecContainer->StrmStorage.frameWidth * 16) *
        ...
        pDecContainer->PPBufferData(pDecContainer->ppInstance,
            pc->bufferIndex, luma, chroma, botLuma, botChroma);
    }
...
AvsDecRet AvsDecPeek(AvsDecInst decInst, AvsDecPicture * pPicture)
{
    ...
    pPicture->picCodingType = pPic->picCodeType;

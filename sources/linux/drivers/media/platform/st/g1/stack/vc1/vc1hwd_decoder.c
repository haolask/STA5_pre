                       ...
                       const void *dwl )
{
    ...
    {
        if (DWLMallocRefFrmNamed(dwl, size, &pPic[i].data,"AllocateMemories : picture buffer") != 0)
        {
            (void)vc1hwdRelease(dwl, pStorage);
            return (VC1HWD_MEMORY_FAIL);
        }
    ...
    rv = DWLMallocLinearNamed(dwl,
        (pStorage->numOfMbs + 9)/10 * sizeof(u32),
        &pDecCont->bitPlaneCtrl,"AllocateMemories : bit plane Ctrl");
    if ((rv != 0) ||  
    ...
    if(pDecCont->storage.maxBframes)
    {
        ...
        {
            rv = DWLMallocLinearNamed(pDecCont->dwl,
                ((pDecCont->storage.numOfMbs +
                  pDecCont->storage.picWidthInMbs+7) & ~0x7) * 2 * sizeof(u32),
                &pDecCont->directMvs,"AllocateMemories : directMvs odd alignment");
        }
        else
        {
            rv = DWLMallocLinearNamed(pDecCont->dwl,
                ((pDecCont->storage.numOfMbs+7) & ~0x7) * 2 * sizeof(u32),
                &pDecCont->directMvs,"AllocateMemories directMvs even alignment");
        }
        if (rv != 0 ||
        ...
        do {
            ...
            switch (startCode)
            {
                ...
                case SC_FRAME:
                    ...
                    streamData->slice_piclayer_emulation_bits = 0;
                    ...
                    pStorage->picLayer.picHeaderBits -= 
                    streamData->slice_piclayer_emulation_bits ;
...
/* Code */
        ...
        pDecCont->storage.picLayer.fcm == FRAME_INTERLACE )
    {
        ...
        if(pDecCont->storage.picLayer.picType == PTYPE_I)
            ...
            picType = DEC_PIC_TYPE_BI;

        pPic[ workIndex ].picCodeType[0] = picType;
        pPic[ workIndex ].picCodeType[1] = picType;
    }
    else
    {
        pPic[ workIndex ].keyFrame =
                ( pDecCont->storage.picLayer.fieldPicType == FP_I_I ||
                  pDecCont->storage.picLayer.fieldPicType == FP_I_P ||
                  pDecCont->storage.picLayer.fieldPicType == FP_P_I) ?
                HANTRO_TRUE : HANTRO_FALSE;

        switch( pDecCont->storage.picLayer.fieldPicType )
        {
            case FP_I_I:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_I;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_I;
            break;
            case FP_I_P:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_I;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_P;
            break;
            case FP_P_I:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_P;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_I;
            break;
            case FP_P_P:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_P;
            ...
            break;
            case FP_B_B:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_B;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_B;
            break;
            case FP_B_BI:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_B;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_BI;
            break;
            case FP_BI_B:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_BI;
                pPic[ workIndex ].picCodeType[1] = DEC_PIC_TYPE_B;
            break;
            case FP_BI_BI:
                pPic[ workIndex ].picCodeType[0] = DEC_PIC_TYPE_BI;

...
#define DEC_VOPD pDecCont->VopDesc
    ...
    for (i = 0; i < pDecCont->StrmStorage.numBuffers ; i++)
    {
        ret |= DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
            &pDecCont->StrmStorage.data[i],"MP4AllocateBuffers : frame buffers");
    ...
    ret = DWLMallocLinearNamed(pDecCont->dwl,
        ...
        &pDecCont->StrmStorage.directMvs,"MP4AllocateBuffers : directMvs");
    ret |= DWLMallocLinearNamed(pDecCont->dwl, 2*64,
        &pDecCont->StrmStorage.quantMatLinear,"MP4AllocateBuffers : quantMatLinear");
    if(ret)
    {
        return (MP4DEC_MEMFAIL);
    }
...
#define DEC_VOPD pDecCont->VopDesc
    ...
    if (pDecCont->rlcMode)
    {
        ...
        ret |= DWLMallocLinearNamed(pDecCont->dwl, sizeControl,
                               &pDecCont->MbSetDesc.ctrlDataMem,"MP4AllocateRlcBuffers : control");
        ...
        ret |= DWLMallocLinearNamed(pDecCont->dwl, sizeMv,
                               &pDecCont->MbSetDesc.mvDataMem,"MP4AllocateRlcBuffers : motion vector");
        ...
        ret |= DWLMallocLinearNamed(pDecCont->dwl, sizeRlc,
                              &pDecCont->MbSetDesc.rlcDataMem,"MP4AllocateRlcBuffers :  data buffer");
        ...
        ret |= DWLMallocLinearNamed(pDecCont->dwl, sizeDc,
                               &pDecCont->MbSetDesc.DcCoeffMem,"MP4AllocateRlcBuffers : DcCoeffMem");
...
MP4DecRet MP4DecAllocExtraBPic(DecContainer * pDecCont)
{
    ...
    ret = DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
           &pDecCont->StrmStorage.data[2],"MP4DecAllocExtraBPic : B frame buffer");
    ...
    {
        ret = DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
                              &pDecCont->StrmStorage.data[3],"MP4DecAllocExtraBPic : extra B buffer");
                        ...
                        u32 nbrErrMbs)
{
    ...
    else if(vopType == PVOP)
        picType = DEC_PIC_TYPE_P;
    else
        picType = DEC_PIC_TYPE_B;
    pDecCont->StrmStorage.pPicBuf[j].picType = picType;
...
{
    if( (i32)index < 0 || index >= BUFFER_UNDEFINED)
        return NULL;

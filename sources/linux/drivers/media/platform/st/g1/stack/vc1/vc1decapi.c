                      ...
                      void * userCtx)
{
    ...
    *pDecInst = NULL; /* return NULL instance for any error */
    ...
    SetCommonConfigRegs(pDecCont->vc1Regs);
    ...
    if(referenceFrameFormat == DEC_REF_FRM_TILED_DEFAULT)
    {
        ...
        {
            DEC_API_TRC("VC1DecInit# ERROR: Format not supported\n");
            DWLfree(pDecCont);
            (void)DWLRelease(dwl);
            return VC1DEC_FORMAT_NOT_SUPPORTED;
        }
                        ...
                        VC1DecOutput* pOutput )
{
    ...
    else if (decResult == VC1HWD_NOT_CODED_PIC)
    {
        ...
        {
            pDecCont->storage.workOut = pDecCont->storage.workOutPrev;
    ...
    else /* decResult == VC1HWD_OK */
    {
        ...
        if (!pDecCont->asicRunning && pDecCont->storage.partialFreeze)
        {
            PreparePartialFreeze(
                (u8*)pDecCont->storage.pPicBuf[
                pDecCont->storage.workOut].data.virtualAddress,
                pDecCont->storage.picWidthInMbs,
                pDecCont->storage.picHeightInMbs);
        }
                 ...
                 DEC_8190_IRQ_ABORT) )
        {
            if (!pDecCont->storage.partialFreeze ||
                ...
                (u8*)pDecCont->storage.pPicBuf[
                 pDecCont->storage.workOut].data.virtualAddress,
                 pDecCont->storage.work0 != INVALID_ANCHOR_PICTURE ?
                 (u8*)pDecCont->storage.pPicBuf[
                 pDecCont->storage.work0].data.virtualAddress :
                 NULL,
                 pDecCont->storage.picWidthInMbs,
                 pDecCont->storage.picHeightInMbs,
                 pDecCont->storage.partialFreeze == 1))
            {
                ...
                if (!pDecCont->storage.partialFreeze)
                {
                    ...
                    if (firstFrame || (isBPic && !pDecCont->storage.intraFreeze)
                         || pDecCont->storage.slice)
                    {
                        (void)vc1hwdSeekFrameStart(&pDecCont->storage, &streamData);
                        returnValue = VC1DEC_STRM_PROCESSED;
                ...
                else
                {
                    if (!pDecCont->storage.partialFreeze ||
                        ...
                        (u8*)pDecCont->storage.pPicBuf[
                        pDecCont->storage.work1].data.virtualAddress :
                        NULL,
                        pDecCont->storage.picWidthInMbs,
                        pDecCont->storage.picHeightInMbs,
                        pDecCont->storage.partialFreeze == 1))
            ...
            {
                asicStatus &= ~DEC_X170_IRQ_STREAM_ERROR;
                asicStatus &= ~DEC_X170_IRQ_TIMEOUT;
...
void VC1DecRelease(VC1DecInst decInst)
{
    ...
    if(pDecCont->asicRunning)
    {
        DWLWriteReg(dwl, pDecCont->coreID, 1 * 4, 0); /* stop HW */
...
/* Code */
    ...
    if(pDecCont->asicRunning)
    {
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 1 * 4, 0);   /* stop HW */
                ...
                &picId, &errMbs ) == HANTRO_NOK )
    {
        ...
        pPicture->picId = 0;
        pPicture->picCodingType[0] = 0;
        pPicture->picCodingType[1] = 0;
    ...
    else
    {
        ...
        * decode second field and wait PP after that */
        ...
        pPicture->fieldPicture      =
                                (pPic[picIndex].fcm == FIELD_INTERLACE) ? 1 : 0;
                ...
                pDecCont->storage.parallelMode2 )
            {
                if( !pDecCont->storage.pm2AllProcessedFlag )
                {
                    ...
                    /* Connect PP output buffer to decoder output buffer */
                    {
                        ...
                        {
                            botLuma = luma + (pDecCont->storage.picWidthInMbs * 16);
                            botChroma = chroma + (pDecCont->storage.picWidthInMbs * 16);
                        }
                        else if( pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD )
                        {
                            ...
                            botChroma = chroma + ((pDecCont->storage.picWidthInMbs * 16) *
                                        (pDecCont->storage.picHeightInMbs * 4));
                        }
		...
		/* croppedW and croppedH are compared to :
		 * Img->width and Img->height in PPDecStartPp
		 * constraint on width and height is alignement on 0x0F
		 * so these values have also to be aligned on 0x0F
		 * and not 0x07.
		 */
                pDecCont->ppControl.croppedW =
                        ((pPic[picIndex].codedWidth+15) & ~15);
                pDecCont->ppControl.croppedH =
                        ((pPic[picIndex].codedHeight+15) & ~15);
                                            ...
                                            DECPP_PIC_BOT_FIELD_FRAME) )
                {
                    ...
                    pDecCont->ppControl.croppedH =
                        ((pDecCont->ppControl.croppedH + 15) & ~15);
                }

...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    *pDecInst = NULL;   /* return NULL instance for any error */
    ...
    if(referenceFrameFormat == DEC_REF_FRM_TILED_DEFAULT)
    {
        ...
        {
            DEC_API_TRC("H264DecInit# ERROR: Format not supported\n");
            DWLfree(pDecCont);
            (void)DWLRelease(dwl);
            return H264DEC_FORMAT_NOT_SUPPORTED;
        }
                         ...
                         H264DecOutput * pOutput)
{
        ...
        case H264BSD_PIC_RDY:
            {
                ...
                {
                    ERROR_PRINT("IRQ: STREAM ERROR detected\n");
                    if(pDecCont->packetDecoded != HANTRO_TRUE)
                    {
                        DEBUG_PRINT(("reset picStarted\n"));
                        pDecCont->storage.picStarted = HANTRO_FALSE;
    ...
    if (pOutput->dataLeft > pInput->dataLen)
    {
        pOutput->dataLeft = pInput->dataLen;
    }
                              ...
                              H264DecPicture * pOutput, u32 endOfStream)
{
    ...
    if(outPic != NULL)
    {
		...
		pOutput->picOrderCnt[0] = outPic->picOrderCnt[0];
		pOutput->picOrderCnt[1] = outPic->picOrderCnt[1];
		pOutput->isRef = outPic->isRef;

		pOutput->num_reorder_frames = 0;
		{
			vuiParameters_t *vui =
				pDecCont->storage.activeSps->vuiParameters;

			if (vui && vui->bitstreamRestrictionFlag)
				pOutput->num_reorder_frames =
					vui->numReorderFrames;
		}

...
#define DEC_FRAMED pDecCont->FrameDesc
    ...
    ret |= DWLMallocLinearNamed(pDecCont->dwl, MPEG2DEC_TABLE_SIZE,
                           &pDecCont->ApiStorage.pQTableBase,
			   "mpeg2AllocateBuffers : Q-table buffer");
    if(ret)
    ...
    if(!pDecCont->ApiStorage.externalBuffers)
    {
        ...
        for(i = 0; i < pDecCont->StrmStorage.numBuffers ; i++)
        {
            ret |= DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
				   ...
				   "mpeg2AllocateBuffers : PicBuffer");
...
Mpeg2DecRet mpeg2DecAllocExtraBPic(DecContainer * pDecCont)
{
    ...
    ret = DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
			  ...
			  "mpeg2DecAllocExtraBPic : PicBuffer");
    ...
    if(extraBuffer)
    {
        ret = DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
			      ...
			      "mpeg2DecAllocExtraBPic : PicBuffer (Extra)");
                           ...
                           u32 isInter, Mpeg2DecRet returnValue, u32 nbrErrMbs)
{
    ...
    if(pDecCont->FrameDesc.picCodingType == IFRAME)
        picType = DEC_PIC_TYPE_I;
    else if(pDecCont->FrameDesc.picCodingType == PFRAME)
        picType = DEC_PIC_TYPE_P;
    else if(pDecCont->FrameDesc.picCodingType == BFRAME)
        picType = DEC_PIC_TYPE_B;
    else
        picType = DEC_PIC_TYPE_D;

    if((pDecCont->FrameDesc.fieldCodingType[0] == IFRAME &&
        pDecCont->FrameDesc.fieldCodingType[1] == PFRAME) ||
       (pDecCont->FrameDesc.fieldCodingType[0] == PFRAME &&
        pDecCont->FrameDesc.fieldCodingType[1] == IFRAME))
    {
        pDecCont->StrmStorage.pPicBuf[j].picCodeType[0] = DEC_PIC_TYPE_I;
        pDecCont->StrmStorage.pPicBuf[j].picCodeType[1] = DEC_PIC_TYPE_P;
        pDecCont->StrmStorage.pPicBuf[j].picType = 1;
    }
    else
    {
        pDecCont->StrmStorage.pPicBuf[j].picCodeType[0] = picType;
        pDecCont->StrmStorage.pPicBuf[j].picCodeType[1] = picType;
        pDecCont->StrmStorage.pPicBuf[j].picType = !isInter && !bufferB;
    }

    MPEG2DEC_DEBUG(("Field pictype [%d/%d]\n", pDecCont->FrameDesc.fieldCodingType[0], pDecCont->FrameDesc.fieldCodingType[1]));

                         ...
                         void * userCtx)
{
    ...
    *pDecInst = NULL;
    ...
    SetCommonConfigRegs(pDecCont->mpeg2Regs);
    ...
    if(referenceFrameFormat == DEC_REF_FRM_TILED_DEFAULT)
    {
        ...
        {
            MPEG2_API_TRC("Mpeg2DecInit# ERROR: Format not supported\n");
            DWLfree(pDecCont);
            (void)DWLRelease(dwl);
            return MPEG2DEC_FORMAT_NOT_SUPPORTED;
        }
...
#define DEC_FRAMED ((DecContainer *)decInst)->FrameDesc
    ...
    if(pDecCont->StrmStorage.newHeadersChangeResolution)
    {
        ...
        pDecCont->Hdrs.frameRateCode = pDecCont->tmpHdrs.frameRateCode;
        pDecCont->Hdrs.aspectRatioInfo = pDecCont->tmpHdrs.aspectRatioInfo;
        pDecCont->FrameDesc.frameWidth = (pDecCont->tmpHdrs.horizontalSize + 15) >> 4;
        pDecCont->FrameDesc.frameHeight = (pDecCont->tmpHdrs.verticalSize + 15) >> 4;
        pDecCont->FrameDesc.totalMbInFrame =
            pDecCont->FrameDesc.frameWidth * pDecCont->FrameDesc.frameHeight;
    }

    if(API_STOR.DecStat == HEADERSDECODED && !pDecCont->noReallocation)
    {
    ...
    do
    {
        ...
        if(API_STOR.DecStat != HW_PIC_STARTED)
        {
            ...
            case DEC_HDRS_RDY:
                ...
                pDecCont->StrmStorage.errorInHdr = 0;
        ...
        if(API_STOR.DecStat == HW_PIC_STARTED)
        {
            ...
            if(asicStatus & MPEG2_DEC_X170_IRQ_DEC_RDY)
            {
...
#define API_STOR ((DecContainer *)decInst)->ApiStorage
    ...
    {
        /* Release HW */
        DWLDisableHW(pDecCont->dwl, pDecCont->coreID, 4, 0);
...
u32 mpeg2HandleVlcModeError(DecContainer * pDecCont, u32 picNum)
{
    ...
    if(!pDecCont->FrameDesc.frameNumber)
    {
        ...
        pDecCont->StrmStorage.errorInHdr = 1;
            ...
            else if(endOfStream)
            {
                    ...
                    !pDecCont->StrmStorage.pm2AllProcessedFlag)
                {
                    ...
                    /* Connect PP output buffer to decoder output buffer */
                    {
                        ...
                        if(pDecCont->dpbMode == DEC_DPB_FRAME )
                        {
                            botLuma = luma + (pDecCont->FrameDesc.frameWidth * 16);
                            botChroma = chroma + (pDecCont->FrameDesc.frameWidth * 16);
                        }
                        else if( pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD )
                        {
                            botLuma = luma + ((pDecCont->FrameDesc.frameWidth * 16) *
                               ...
                               DecContainer * pDecCont, u32 picIndex)
{
    ...
    pPicture->picCodingType[0] = pPic[picIndex].picCodeType[0];
    pPicture->picCodingType[1] = pPic[picIndex].picCodeType[1];
    ...
    u32 picIndexOverride)
{
    ...
    else if( pDecCont->StrmStorage.work0 < pDecCont->StrmStorage.numBuffers )
    {
        picIndex = pDecCont->StrmStorage.work0;
    }
    else
...
static void Mpeg2PpControl(DecContainer * pDecContainer, u32 pipelineOff)
{
    ...
    /* Connect PP output buffer to decoder output buffer */
    {
        ...
        if(pDecContainer->dpbMode == DEC_DPB_FRAME )
        {
            botLuma = luma + (pDecContainer->FrameDesc.frameWidth * 16);
            botChroma = chroma + (pDecContainer->FrameDesc.frameWidth * 16);
        }
        else if( pDecContainer->dpbMode == DEC_DPB_INTERLACED_FIELD )
        {
            botLuma = luma + ((pDecContainer->FrameDesc.frameWidth * 16) *
...
void Mpeg2CheckReleasePpAndHw(DecContainer *pDecCont)
{
    ...
    {
        (void) DWLReleaseHw(pDecCont->dwl, pDecCont->coreID);
...
Mpeg2DecRet Mpeg2DecPeek(Mpeg2DecInst decInst, Mpeg2DecPicture * pPicture)
{
    ...
    else
    {
        ...
        pPicture->picCodingType[0] = pPic->picCodeType[0];
        pPicture->picCodingType[1] = pPic->picCodeType[1];

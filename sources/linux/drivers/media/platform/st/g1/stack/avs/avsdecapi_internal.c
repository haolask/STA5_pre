...
AvsDecRet AvsAllocateBuffers(DecContainer * pDecCont)
{
    ...
    if(!pDecCont->ApiStorage.externalBuffers)
    {
        ...
        for(i = 0; i < pDecCont->StrmStorage.numBuffers; i++)
        {
            ret |= DWLMallocRefFrmNamed(pDecCont->dwl, sizeTmp,
                                   &pDecCont->StrmStorage.pPicBuf[i].data,"AvsAllocateBuffers : PicBuffer");
    ...
    ret |= DWLMallocLinearNamed(pDecCont->dwl,
        ...
        &pDecCont->StrmStorage.directMvs,"AvsAllocateBuffers directMvs");
                           ...
                           u32 isInter, AvsDecRet returnValue, u32 nbrErrMbs)
{
    ...
    else if(pDecCont->Hdrs.picCodingType == PFRAME)

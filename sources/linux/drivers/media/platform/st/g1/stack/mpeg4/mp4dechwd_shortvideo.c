...
static u32 StrmDec_DecodeSVHPlusHeader(DecContainer * pDecContainer)
{
    ...
    if(ufep == 1)
    {
        ...
        if(customSourceFormat)
        {
            ...
            if(tmp == END_OF_STREAM)
                return (END_OF_STREAM);
            pDecContainer->VopDesc.vopWidth = (customWidth + 15) / 16;
            pDecContainer->Hdrs.videoObjectLayerWidth = customWidth;
            ...
            if(!tmp)
                MP4DEC_UNSUPPORTED;
            pDecContainer->VopDesc.vopHeight = (customHeight + 15) / 16;
            pDecContainer->Hdrs.videoObjectLayerHeight = customHeight;

...
On2RvDecRet On2RvDecInit(void *pRV10Init, void **decoderState, void * userCtx)
{
    ...
    void *pInputParams, void *pOutputParams, void *global)
{
    ...
    * Put in comment and update the code to dyanmically allocate this buffer
    * due to the warning reported at compilation time :
    * "the frame size of 1112 bytes is larger than 1024 bytes
    * [-Wframe-larger-than=]"
    */
    RvDecRet ret;
    u32 more = 0;
    UNUSED(pDecodedFrameBuffer);
    ...
    rvIn.pSliceInfo = (RvDecSliceInfo *) DWLmalloc(128*sizeof(RvDecSliceInfo));
    if (!rvIn.pSliceInfo) {
        RVDEC_DEBUG(("Unable to allocate SliceInfo buffer"));
        return ON2RVDEC_FAIL;
    }

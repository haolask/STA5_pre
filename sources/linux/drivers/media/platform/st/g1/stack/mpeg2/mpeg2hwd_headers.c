...
u32 mpeg2StrmDec_DecodeSequenceHeader(DecContainer * pDecContainer)
{
    ...
    pDecContainer->noReallocation = 0;
    ...
    if(pDecContainer->StrmStorage.strmDecReady)
    {
        ...
        if(pHdr->aspectRatioInfo != pDecContainer->Hdrs.aspectRatioInfo)
        {
            pDecContainer->StrmStorage.strmDecReady = HANTRO_FALSE;
            pDecContainer->noReallocation = 1;
        }
...
u32 mpeg2StrmDec_DecodeExtensionHeader(DecContainer * pDecContainer)
{
    ...
    case SC_SEQ_EXT:
        ...
        if(!pDecContainer->StrmStorage.errorInHdr)
        {
...
u32 mpeg2StrmDec_DecodeSeqExtHeader(DecContainer * pDecContainer)
{
    ...
    /* read parameters from stream */
    tmp = pHdr->profileAndLevelIndication =
        mpeg2StrmDec_GetBits(pDecContainer, 8);
    tmp = pHdr->progressiveSequence =
        mpeg2StrmDec_GetBits(pDecContainer, 1);
    tmp = pHdr->chromaFormat =
        mpeg2StrmDec_GetBits(pDecContainer, 2);
    if(tmp == END_OF_STREAM)
        return (END_OF_STREAM);
    if(tmp != 1)    /* 4:2:0 */
        return (HANTRO_NOK);
    tmp = pHdr->horSizeExtension =
        mpeg2StrmDec_GetBits(pDecContainer, 2);
    tmp = pHdr->verSizeExtension =
        mpeg2StrmDec_GetBits(pDecContainer, 2);
    if(tmp == END_OF_STREAM)
        return (END_OF_STREAM);
    ...
    /* update parameters */
    pHdr->horizontalSize =
        ((pHdr->horSizeExtension << 12) | (pDecContainer->Hdrs.
                                                         horizontalSize &
                                                         DIMENSION_MASK));

    pHdr->verticalSize =
        ((pHdr->verSizeExtension << 12) | (pDecContainer->Hdrs.
                                                         verticalSize &
                                                         DIMENSION_MASK));

    tmp = pHdr->bitRateExtension =
        mpeg2StrmDec_GetBits(pDecContainer, 12);

    /* marker bit ==> flush */
    tmp = mpeg2StrmDec_FlushBits(pDecContainer, 1);

    tmp = pHdr->vbvBufferSizeExtension =
        mpeg2StrmDec_GetBits(pDecContainer, 8);

    tmp = pHdr->lowDelay = mpeg2StrmDec_GetBits(pDecContainer, 1);

    tmp = pHdr->frameRateExtensionN =
        mpeg2StrmDec_GetBits(pDecContainer, 2);
    tmp = pHdr->frameRateExtensionD =
        mpeg2StrmDec_GetBits(pDecContainer, 5);
    if(tmp == END_OF_STREAM)
        return (END_OF_STREAM);
    ...
    {
        /* Update parameters */
        pDecContainer->Hdrs.profileAndLevelIndication = pHdr->profileAndLevelIndication;
        pDecContainer->Hdrs.progressiveSequence = pHdr->progressiveSequence;
        pDecContainer->Hdrs.chromaFormat = pHdr->chromaFormat;
        pDecContainer->Hdrs.horSizeExtension = pHdr->horSizeExtension;
        pDecContainer->Hdrs.verSizeExtension = pHdr->verSizeExtension;
        pDecContainer->Hdrs.bitRateExtension = pHdr->bitRateExtension;
        pDecContainer->Hdrs.vbvBufferSizeExtension = pHdr->vbvBufferSizeExtension;
        pDecContainer->Hdrs.lowDelay = pHdr->lowDelay;
        pDecContainer->Hdrs.frameRateExtensionN = pHdr->frameRateExtensionN;
        pDecContainer->Hdrs.frameRateExtensionD = pHdr->frameRateExtensionD;

        ...
        if(API_STOR.DecStat == HW_VOP_STARTED)
        {
            if(DEC_VOPD.vopCoded)
            {
                    ...
                    pDecCont->StrmStorage.customStrmHeaders )
                {
                    ...
                    /* reset HW */
                    SetDecRegister(pDecCont->mp4Regs, HWIF_DEC_IRQ_STAT, 0);
                    SetDecRegister(pDecCont->mp4Regs, HWIF_DEC_IRQ, 0);
                    SetDecRegister(pDecCont->mp4Regs, HWIF_DEC_E, 0);
                        ...
                        asicStatus & MP4_DEC_X170_IRQ_TIMEOUT)
                {
                    if (!pDecCont->StrmStorage.partialFreeze ||
                        !ProcessPartialFreeze(
                            (u8*)MP4DecResolveVirtual(pDecCont, pDecCont->StrmStorage.workOut),
                            pDecCont->StrmStorage.work0 != INVALID_ANCHOR_PICTURE ?
                                (u8*)MP4DecResolveVirtual(pDecCont, pDecCont->StrmStorage.work0) : NULL,
                            pDecCont->VopDesc.vopWidth,
                            pDecCont->VopDesc.vopHeight,
            ...
            else if (!pDecCont->packedMode)   /* not-coded VOP */
            {
                ...
                if(pDecCont->ppInstance != NULL)
                {
                    i32 id = pDecCont->StrmStorage.workOut;

                    pDecCont->StrmStorage.pPicBuf[id].sendToPp = 1;
...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    *pDecInst = NULL; /* return NULL for any error */    
    ...
    SetCommonConfigRegs(pDecCont->mp4Regs);
...
void HandleVopEnd(DecContainer * pDecCont)
{
    ...
    if(((tmp == SC_VOS_START) || (tmp == SC_VOS_END) ||
        ((u32)(pDecCont->StrmDesc.pStrmCurrPos -
          pDecCont->StrmDesc.pStrmBuffStart) ==
...
u32 RunDecoderAsic(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    if(!pDecContainer->asicRunning)
    {
        ...
        if (pDecContainer->StrmStorage.reloadQuantMatrices)
        {
            MP4SetQuantMatrix(pDecContainer);
            pDecContainer->StrmStorage.reloadQuantMatrices = HANTRO_FALSE;
        }
        ...
        /* Enable HW */
        DWLEnableHW(pDecContainer->dwl, pDecContainer->coreID, 4 * 1, 1);
    }
    else    /* in the middle of VOP, continue decoding */
    {
        ...
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 5,
                    pDecContainer->mp4Regs[5]);
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 6,
                    pDecContainer->mp4Regs[6]);
        DWLWriteReg(pDecContainer->dwl, pDecContainer->coreID, 4 * 12,
                    pDecContainer->mp4Regs[12]);

        DWLEnableHW(pDecContainer->dwl, pDecContainer->coreID, 4 * 1,
                    pDecContainer->mp4Regs[1]);
    }
    ...
    else
    {
        ...
        SetDecRegister(pDecContainer->mp4Regs, HWIF_DEC_E, 0);

        DWLDisableHW(pDecContainer->dwl, pDecContainer->coreID, 4 * 1, 0);
        ...
        DWLReleaseHw(pDecContainer->dwl, pDecContainer->coreID);
    ...
    if(!(asicStatus & MP4_DEC_X170_IRQ_BUFFER_EMPTY))
    {
        DWLDisableHW(pDecContainer->dwl, pDecContainer->coreID, 0x4, 0);
            ...
            if(endOfStream)
            {
                    ...
                    !pDecCont->StrmStorage.pm2AllProcessedFlag)
                {
                    ...
                    /* Connect PP output buffer to decoder output buffer */
                    {
                        ...
                        if(pDecCont->dpbMode == DEC_DPB_FRAME )
                        {
                            botLuma = luma + (pDecCont->VopDesc.vopWidth * 16);
                            botChroma = chroma + (pDecCont->VopDesc.vopWidth * 16);
                        }
                        else if( pDecCont->dpbMode == DEC_DPB_INTERLACED_FIELD )
                        {
                            botLuma = luma + ((pDecCont->VopDesc.vopWidth * 16) *
                ...
                if(picIndex >= pDecCont->StrmStorage.numBuffers)
                {
                  ...
                  pDecCont->VopDesc.vopNumberInSeq = 0;
                  pDecCont->StrmStorage.workOutPrev = pDecCont->StrmStorage.workOut = 0;
                  pDecCont->StrmStorage.work0 = pDecCont->StrmStorage.work1 =
                      INVALID_ANCHOR_PICTURE;
                }
            }
                             ...
                             DecContainer * pDecCont, u32 picIndex)
{
    ...
    pPicture->picCodingType = pPic[picIndex].picType;
...
/*
	...
	if(pDecContainer->VopDesc.trd == 0)
		...
		uint64_t tmp_value;
		tmp_value = (((uint64_t) pDecContainer->VopDesc.trb << 27) +
			pDecContainer->VopDesc.trd - 1);
		do_div(tmp_value,pDecContainer->VopDesc.trd);
		itmp = (u32)tmp_value;
	}
	...
	{
	uint64_t tmp_value;
	tmp_value = (((uint64_t) (2 * pDecContainer->VopDesc.trb + 1) << 27) +
                2 * pDecContainer->VopDesc.trd);
	do_div(tmp_value,(2 * pDecContainer->VopDesc.trd + 1));
	itmp = (u32)tmp_value;
	}
	...
	if(pDecContainer->VopDesc.trd == 0)
		...
		uint64_t tmp_value;
		tmp_value =
			(((uint64_t) (2 * pDecContainer->VopDesc.trb - 1) << 27) +
			2 * pDecContainer->VopDesc.trd -2);
		do_div(tmp_value,(2 * pDecContainer->VopDesc.trd - 1));
		itmp = (u32)tmp_value;
	}
...
static void MP4DecPrepareFieldProcessing(DecContainer * pDecCont, u32 picIndexOverride)
{
    ...
    if(pDecCont->StrmStorage.work0 < pDecCont->StrmStorage.numBuffers )
    {
        pDecCont->ApiStorage.bufferForPp = picIndex+1;
    }
    else
...
static void PPControl(DecContainer * pDecCont, u32 pipelineOff)
{
    ...
    else
    {
        ...
        u32 previousB = pDecCont->StrmStorage.previousB;

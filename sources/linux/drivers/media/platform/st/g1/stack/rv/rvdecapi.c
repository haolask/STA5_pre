...
------------------------------------------------------------------------------*/
RvDecRet RvDecInit(RvDecInst * pDecInst, DecErrorHandling errorHandling,
    ...
    void * userCtx)
{
    ...
    *pDecInst = NULL;
    ...
    SetCommonConfigRegs(pDecCont->rvRegs);
    ...
    if(referenceFrameFormat == DEC_REF_FRM_TILED_DEFAULT)
    {
        ...
        {
            RVDEC_API_DEBUG(("RvDecInit# ERROR: Format not supported\n"));
            DWLfree(pDecCont);
            (void)DWLRelease(dwl);
            return RVDEC_FORMAT_NOT_SUPPORTED;
        }
        ...
        do
        {
            ...
            if(pApiStor->DecStat == HW_PIC_STARTED)
            {
                if (!pDecCont->asicRunning)
                {
                    ...
                    {
                        PreparePartialFreeze((u8*)pDecCont->StrmStorage.pPicBuf[
                ...
                else if(asicStatus & RV_DEC_X170_IRQ_BUS_ERROR)
                {
                    ret = RVDEC_HW_BUS_ERROR;
                         ...
                         (asicStatus & RV_DEC_X170_IRQ_TIMEOUT) ||
                         (asicStatus & RV_DEC_X170_IRQ_ABORT))
                {
                    if (!pDecCont->StrmStorage.partialFreeze ||
                        ...
                        pDecCont->StrmStorage.partialFreeze == 1))
                    {
                        if(asicStatus & RV_DEC_X170_IRQ_TIMEOUT ||
                           asicStatus & RV_DEC_X170_IRQ_ABORT)
                        {
                ...
                if(asicStatus & RV_DEC_X170_IRQ_DEC_RDY)
                {
...
u32 RunDecoderAsic(DecContainer * pDecContainer, u32 strmBusAddress)
{
       ...
       (asicStatus == ID8170_DEC_SYSTEM_ERROR))
    {
        ...
        SetDecRegister(pDecContainer->rvRegs, HWIF_DEC_E, 0);

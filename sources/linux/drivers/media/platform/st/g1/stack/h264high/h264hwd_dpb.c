        ...
        if(!markedAsLongTerm)
        {
            ...
            else
            {
                ...
                for(i = 0; i <= dpb->dpbSize; i++)
                {
                    ...
                    if (dpb->buffer[i].picNum < 0 && dpb->buffer[i].toBeDisplayed)
                    {
                        SET_STATUS(dpb->buffer[i], UNUSED, FRAME);
                        dpb->buffer[i].toBeDisplayed = 0;
                        DpbBufFree(dpb, i);
...
void h264DpbUpdateOutputList(dpbStorage_t * dpb)
{
    ...
    if(dpb->noReordering)
    {
		...
		dpbOut->picOrderCnt[0] = currentOut->picOrderCnt[0];
		dpbOut->picOrderCnt[1] = currentOut->picOrderCnt[1];
		dpbOut->isRef = currentOut->isRef;
...
/* Code */
    ...
    {
        u32 newID = GetFreePicBuffer(dpb->fbList, dpb->currentOut->memIdx);
                   ...
                   struct dpbInitParams *pDpbParams)
{
    ...
    for(i = 0; i < dpb->totBuffers; i++)
    {
        ...
        if(pDpbParams->isHighSupported)
        {
            ...
            pic_buff_size = pDpbParams->picSizeInMbs *
            ...
            dpb->syncMcOffset = pic_buff_size;
            pic_buff_size += 32;
        }
        else
        {
            pic_buff_size = pDpbParams->picSizeInMbs * 384;
        }

        if (pDpbParams->enable2ndChroma && !pDpbParams->monoChrome)
        {
            dpb->ch2Offset = pic_buff_size;
            pic_buff_size += pDpbParams->picSizeInMbs * 128;
        }

        if(DWLMallocRefFrmNamed(dwl, pic_buff_size, dpb->picBuffers + i,
                "picture buffer from h264bsdInitDpb") != 0)
...
/* Code */
    ...
    return h264bsdInitDpb(dwl, dpb, pDpbParams);
}
...
/* Code */
	...
	dpbOut->picOrderCnt[0] = tmp->picOrderCnt[0];
	dpbOut->picOrderCnt[1] = tmp->picOrderCnt[1];
	dpbOut->isRef = tmp->isRef;

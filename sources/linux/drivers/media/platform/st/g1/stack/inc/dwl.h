...
/* missing APIs in DWL to ensure reentrance */
    /* allow globals initialization knowing user context */
    void DWLSetup(void *userCtx);
    /* allow DWL to get access to top user context */
    extern void DWLSetUserContext(const void *instance, void *userCtx);

/* named APIs for contiguous memory tracking */
    i32 DWLMallocLinearNamed(const void *instance, u32 size, DWLLinearMem_t * info, const char * name);
    i32 DWLMallocRefFrmNamed(const void *instance, u32 size, DWLLinearMem_t * info, const char * name);

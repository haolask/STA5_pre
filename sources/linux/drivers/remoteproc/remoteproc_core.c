...
/**
 * Some remote processors will ask us to dynamically allocate them physically
 * contiguous memory regions (which we call "carveouts").
 *
 * They may then ask us to copy objects (e.g. code/data sections) or expose
 * us certain symbols (e.g. their trace buffer) into this memory regions.
 *
 * This function is an internal helper with which we can go over the allocated
 * carveouts and return the referenced memory device address suitable to the
 * requested memory region size.
 */
u32 rproc_get_da_from_carvout(struct rproc *rproc, int len)
{
			      ...
			      int offset, int avail)
{
	...
	/* If @da is set to FW_RSC_ADDR_ANY, then the host has to
	 * dynamically allocate it in a carveout memory area previously
	 * allocated and overwrite this @da with the dynamically allocated
	 * address
	 */
	if (rsc->da == (u32)FW_RSC_ADDR_ANY)
		rsc->da = rproc_get_da_from_carvout(rproc, rsc->len);
				 ...
				 int offset, int avail)
{
	...
	/* If @da is set to FW_RSC_ADDR_ANY, then the host has dynamically
	 * allocated it and it overwrite @da with the dynamically allocated
	 * address
	 */
	if (rsc->da == (u32)FW_RSC_ADDR_ANY)
		rsc->da = dma;
...
static int rproc_fw_boot(struct rproc *rproc, const struct firmware *fw)
{
	...
	if (fw)
		/* Firmware loading required */
		dev_info(dev,
			 "Booting fw image %s, size %zd\n", name, fw->size);
	else
		/* Firmware loading not required */
		dev_info(dev,
			 "Start %s w/o fw image loading\n", rproc->name);
...
static int rproc_trigger_auto_boot(struct rproc *rproc)
{
	...
	/*
	 * The resource table can be extracted:
	 * Either from Device Tree:
	 * - in this case the resource table is already stored in the rproc and
	 *   no firware loading is required.
	 * Or from firmware
	 * - in this case we're initiating an asynchronous firmware loading, so
	 *   we can be built-in kernel code, without hanging the boot process.
	 *
...
static int __rproc_boot(struct rproc *rproc)
{
	...
	if (rproc->firmware == FW_RSC_NOT_REQUIRED) {
		firmware_p = NULL;
	} else {

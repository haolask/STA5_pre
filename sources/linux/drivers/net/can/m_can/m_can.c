...
/* STA SoC m_can base address */
#define STA_MCAN1_ADDR_OFFSET	0x400

/* STA SoC mscr register */
#define STA_MSCR_FDCAN		0x44

/* STA SoC mscr interrupt line enables */
#define STA_MSCR_FDCAN0_IL0	BIT(1)
#define STA_MSCR_FDCAN0_IL1	BIT(4)
#define STA_MSCR_FDCAN1_IL0	BIT(2)
#define STA_MSCR_FDCAN1_IL1	BIT(5)
...
struct m_can_priv {
	...
	u8 soc_ctl_il0_shift;
	u8 soc_ctl_il1_shift;
...
static inline int m_can_unmask_interrupt_lines(const struct m_can_priv *priv)
{
	if (priv->soc_ctl_base &&
	    of_device_is_compatible(priv->device->of_node, "st,m_can")) {
		/* unmask interrupt lines */
		if (regmap_update_bits(priv->soc_ctl_base, STA_MSCR_FDCAN,
				       priv->soc_ctl_il0_shift,
				       priv->soc_ctl_il0_shift))
			return -EIO;
		if (regmap_update_bits(priv->soc_ctl_base, STA_MSCR_FDCAN,
				       priv->soc_ctl_il1_shift,
				       priv->soc_ctl_il1_shift))
			return -EIO;
	}
	return 0;
...
static int m_can_plat_probe(struct platform_device *pdev)
{
	struct net_device *dev;
	struct m_can_priv *priv;
	struct resource *res_ctrl, *res_ram;
	void __iomem *addr;
	void __iomem *mram_addr;
	struct clk *hclk, *cclk;
	int irq, ret;
	struct device_node *np, *syscon_np;
	...
	res_ctrl = platform_get_resource_byname(pdev, IORESOURCE_MEM, "m_can");
	addr = devm_ioremap_resource(&pdev->dev, res_ctrl);
	...
	res_ram = platform_get_resource_byname(pdev, IORESOURCE_MEM,
					       "message_ram");
	if (!res_ram) {
		ret = -ENODEV;
		goto failed_ret;
	...
	mram_addr = devm_ioremap(&pdev->dev, res_ram->start,
				 resource_size(res_ram));
	if (!mram_addr) {
		ret = -ENOMEM;
		goto failed_ret;
	}
	...
	if (of_device_is_compatible(np, "st,m_can")) {
		syscon_np = of_parse_phandle(np, "sta,soc-ctl-syscon", 0);
		if (syscon_np) {
			...
			priv->soc_ctl_base = regmap_init_smc(NULL, &res,
							     &regmap_cfg);
#else
			priv->soc_ctl_base = syscon_node_to_regmap(syscon_np);
#endif
			of_node_put(syscon_np);

			if (IS_ERR(priv->soc_ctl_base)) {
				ret = PTR_ERR(priv->soc_ctl_base);
				if (ret != -EPROBE_DEFER)
					dev_err(&pdev->dev,
						"Can't get syscon: %d\n", ret);
				goto failed_ret;
			}
			if (res_ctrl->start & STA_MCAN1_ADDR_OFFSET) {
				priv->soc_ctl_il0_shift =
					STA_MSCR_FDCAN1_IL0;
				priv->soc_ctl_il1_shift =
					STA_MSCR_FDCAN1_IL1;
			...
			dev_warn(&pdev->dev,
				 "no sta soc ctl base address defined\n");
		}
	}
	ret = m_can_unmask_interrupt_lines(priv);
	if (ret)
		goto failed_ret;
...
static __maybe_unused int m_can_suspend(struct device *dev)
{
	...
	pm_runtime_force_suspend(priv->device);

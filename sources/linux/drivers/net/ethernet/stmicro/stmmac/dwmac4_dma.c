...
static void dwmac4_mtl_operation_mode(void __iomem *ioaddr, int txfifosz,
				      int rxfifosz)
{
	u32 mtl_tx_op, mtl_rx_op;

	/* Set TX and RX fifo size for Queue 0 */
	if (txfifosz) {
		mtl_tx_op = readl(ioaddr + MTL_CHAN_TX_OP_MODE(0));
		mtl_tx_op |= MTL_OP_MODE_TXQ_SIZE(txfifosz);
		writel(mtl_tx_op, ioaddr +  MTL_CHAN_TX_OP_MODE(0));
	}
	if (rxfifosz) {
		mtl_rx_op = readl(ioaddr + MTL_CHAN_RX_OP_MODE(0));
		mtl_rx_op |= MTL_OP_MODE_RXQ_SIZE(rxfifosz);
		writel(mtl_rx_op, ioaddr + MTL_CHAN_RX_OP_MODE(0));
	}
}
...
const struct stmmac_dma_ops dwmac4_dma_ops = {
	...
	.mtl_mode = dwmac4_mtl_operation_mode,

...
retry_clli:
	...
	/* to be sure that clli didn't change */
	clli2 = readl(ch->base + PL080_CH_LLI) & ~PL080_LLI_LM_AHB2;
	if (clli2 != clli)
		goto retry_clli;
	...
	/*
	 * last LLI of cyclic transactions, avoid re-loop
	 */
	if (txd->cyclic && (clli == llis_bus))
		return bytes;
		      ...
		      struct pl08x_dma_chan *virt_chan)
{
	...
	unsigned long flags;
	int i, chn_start, chn_end;
	struct virt_dma_desc *vd = vchan_next_desc(&virt_chan->vc);
	struct pl08x_txd *txd = to_pl08x_txd(&vd->tx);

	if (txd->cyclic) {
		chn_start = 0;
		chn_end = pl08x->pd->cyclic_channels;
	} else {
		chn_start = pl08x->pd->cyclic_channels;
		chn_end = pl08x->vd->channels;
	}

	for (i = chn_start; i < chn_end; i++) {
...
static irqreturn_t pl08x_irq(int irq, void *dev)
{
	...
	err = readl(pl08x->base + PL080_ERR_STATUS) & ~pl08x->locked_chans_mask;
	if (err) {
		dev_err(&pl08x->adev->dev, "%s error interrupt, register value 0x%08x\n",
			__func__, err);
		writel(err, pl08x->base + PL080_ERR_CLEAR);
	}
	tc = readl(pl08x->base + PL080_TC_STATUS) & ~pl08x->locked_chans_mask;
	if (tc)
...
static inline int of_valid_buses(u32 a)
{
	return a && !(a & ~(PL08X_AHB1 | PL08X_AHB2));
}
				       ...
				       struct of_dma *ofdma)
{
	...
	bus = dma_spec->args[1];
	if (of_valid_buses(bus)) {
		plchan->cd->periph_buses = dma_spec->args[1];
	} else {
		dev_warn(&pl08x->adev->dev,
			 "invalid bus: %i, set AHB1|AHB2\n", bus);
		plchan->cd->periph_buses = PL08X_AHB1 | PL08X_AHB2;
	}
			  ...
			  struct device_node *np)
{
	...
	ret = of_property_read_u8(np, "cyclic-channels", &pd->cyclic_channels);
	if (ret) {
		dev_info(&adev->dev, "number of cyclic channels not specified, using 4\n");
		pd->cyclic_channels = 4;
	}
	if (pd->cyclic_channels > pl08x->vd->channels) {
		dev_err(&adev->dev, "invalid number of cyclic channels\n");
		pd->cyclic_channels = 0;
	}

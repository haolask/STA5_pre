...
/*
 * In master mode the maximum SSPCLK frequency is
 * (SSPCLKI frequency) / 2
 */
#define CPSDVR_MIN 0x02
/*
 * In Slave mode to ensure correct device operation,
 * SSPCLKI must be at least 12 times faster than
 * the maximum expected frequency of SSPCLK
 */
#define CPSDVR_MIN_SLAVE 0xC
...
static int calculate_effective_freq(struct pl022 *pl022, int freq,
				    ...
				    struct ssp_clock_params *clk_freq)
{
	...
	case SSP_SLAVE:
		/* cpsdvscr = 12 & scr 0 */
		max_tclk = spi_rate(rate, CPSDVR_MIN_SLAVE, SCR_MIN);
		break;
	...
	if (freq > max_tclk) {
		if (hierarchy == SSP_SLAVE) {
			dev_err(&pl022->adev->dev,
				"Max programmable speed is %d Hz, you requested %d\n",
				max_tclk, freq);
			return -EINVAL;
		}
		dev_warn(&pl022->adev->dev,
...
pl022_platform_data_dt_get(struct device *dev)
{
	...
	pd->cs_internal = of_property_read_bool(np, "pl022,cs-internal");
...
static int pl022_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if (platform_info->num_chipselect > 1 && platform_info->cs_internal) {
		dev_warn(dev, "probe: num-cs should be one if cs-internal selected\n");
		num_cs = 1;
	}
	...
	} else if (IS_ENABLED(CONFIG_OF) && (platform_info->cs_internal)) {
		pl022->chipselects[0] = -1;

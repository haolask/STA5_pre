				...
				unsigned offset, int gpio_mode)
{
	...
	if (WARN_ON(BIT(offset) & nmk_chip->reserved_gpios)) {
		dev_warn(nmk_chip->chip.parent,
			 "%s: pin %i is reserved\n", __func__, offset);
		return;
	}
...
static void __nmk_gpio_irq_handler(struct irq_desc *desc, u32 status)
{
	...
	status &= ~nmk_chip->reserved_gpios;
	while (status) {
...
static int nmk_gpio_probe(struct platform_device *dev)
{
	...
	of_property_read_u32(np, "reserved-gpios", &nmk_chip->reserved_gpios);
	...
	if (nmk_chip->reserved_gpios) {
		nmk_chip->rimsc = readl(nmk_chip->addr + NMK_GPIO_RIMSC);
		nmk_chip->fimsc = readl(nmk_chip->addr + NMK_GPIO_FIMSC);
	}
...
static const struct of_device_id nmk_pinctrl_match[] = {
	...
	},
	{
		...
		.data = (void *)PINCTRL_NMK_STA1295,
	},
	{
		.compatible = "st,sta1385-pinctrl",
		.data = (void *)PINCTRL_NMK_STA1385,
	},
	{},
};
...
static int nmk_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (version == PINCTRL_NMK_STA1295)
		nmk_pinctrl_sta1295_init(&npct->soc);
	if (version == PINCTRL_NMK_STA1385)
		nmk_pinctrl_sta1385_init(&npct->soc);
	...
	if (!npct->prcm_base) {
		if (version == PINCTRL_NMK_STN8815 ||
			version == PINCTRL_NMK_STA1295 ||
			version == PINCTRL_NMK_STA1385) {
			dev_info(&pdev->dev,
				 "No PRCM base, "

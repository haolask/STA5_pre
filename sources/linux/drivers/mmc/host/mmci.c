...
#ifdef CONFIG_MMC_ARMMMCI_SHARED
	if (host->hsem)
		sta_hsem_unlock(host->hsem);
#endif
}
				...
				struct dma_async_tx_descriptor **dma_desc)
{
	...
	/*
	 * In case mmc device is the dma flow controller, dma controller
	 * expects transfer a NULL sg length. So we temporarily set it to 0
	 * until we get the descriptor.
	 */
	if (host->dma_device_fc) {
		sgl = data->sg->length;
		data->sg->length = 0;
	}
	...
	/*
	 * In case mmc device is the dma flow controller,
	 * restore the initial length in scatter list.
	 */
	if (host->dma_device_fc)
		data->sg->length = sgl;
...
#ifdef CONFIG_MMC_ARMMMCI_SHARED
	if (host->hsem)
		while (sta_hsem_trylock(host->hsem) == -EBUSY)
			schedule();
#endif
	spin_lock_irqsave(&host->lock, flags);
...
static int mmci_of_parse(struct device_node *np, struct mmc_host *mmc)
{
	...
	if (of_get_property(np, "st,fbclk-inverted", NULL)) {
	...
	if (of_get_property(np, "st,dma-device-fc", NULL))
		host->dma_device_fc = true;

#ifdef CONFIG_MMC_ARMMMCI_SHARED
	if (of_get_property(np, "st,shared", NULL))
		host->shared = true;
#endif
	...
	const struct amba_id *id)
{
...
#ifdef CONFIG_MMC_ARMMMCI_SHARED
	if (host->shared) {
		host->hsem = sta_hsem_lock_request(HSEM_MMC_ID, 0, NULL, NULL);
		if (!host->hsem) {
			dev_err(&dev->dev, "failed to get hardware semaphore\n");
			return -EINVAL;
		}
		while (sta_hsem_trylock(host->hsem) == -EBUSY)
			schedule();
	...
	if (host->dma_device_fc)
		/*
		 * Mmc device is the dma flow controller.
		 * SGIO is not available.
		 */
		mmc->max_segs = 1;
	else
		/*
...
static void mmci_save(struct mmci_host *host)
{
	...
	writel(0, host->base + MMCIMASK0);
#ifdef CONFIG_MMC_ARMMMCI_SHARED
	if (host->variant->pwrreg_nopower && !host->hsem) {
#else

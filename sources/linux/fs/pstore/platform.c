...
static inline int no_compress(const void *in, void *out, size_t inlen,
			      size_t outlen)
{
	memcpy(out, in, inlen);
	return inlen;
...
static void no_compress_init(void)
{
	workspace = NULL;
}

static struct pstore_zbackend backend_no = {
	.compress	= no_compress,
	.decompress	= (int (*)(void *, void *, size_t, size_t))no_compress,
	.allocate	= no_compress_init,
	.free		= no_compress_init,
	.name		= "no_compress",
};
#endif
...
static struct pstore_zbackend *zbackend =
	...
	&backend_lz4;
#elif defined(CONFIG_PSTORE_NO_COMPRESS)
	&backend_no;
#else
	NULL;
#endif
